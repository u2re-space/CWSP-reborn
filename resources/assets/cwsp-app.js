...
}`}));async function Vi(e){if(Hi===e)return;console.log(`[Veela] Loading variant:`,e);let t=async e=>{typeof e==`string`&&e.length&&await a(e)};if(e===`core`){await t(Li),Hi=e;return}await t(zi),Hi=e}var Hi,Ui=e((()=>{c(),Ri(),Bi(),Hi=null}));async function Wi(e){let t=Gi[e]||Gi[`vl-basic`];if(!t)throw Error(`Unknown style system: ${e}`);if(Ki===e){console.log(`[Styles] Style system '${e}' already loaded`);return}console.log(`[Styles] Loading style system: ${t.name}`),t.initFn&&await t.initFn(),Ki=e,console.log(`[Styles] Style system ${t.name} loaded`)}var Gi,Ki,qi=e((()=>{Ui(),Gi={"vl-advanced":{id:`vl-advanced`,name:`Veela Advanced`,description:`Full-featured CSS framework with design tokens and effects`,variant:`advanced`,initFn:async()=>{try{await Vi(`advanced`),console.log(`[Styles] Veela Advanced loaded`)}catch{}}},"vl-basic":{id:`vl-basic`,name:`Veela Basic Styles`,description:`Lightweight minimal styling for basic functionality`,variant:`basic`,initFn:async()=>{try{await Vi(`basic`),console.log(`[Styles] Veela Basic Styles loaded`)}catch(e){console.warn(`[Styles] Failed to load Veela Basic Styles:`,e)}}},"vl-beercss":{id:`vl-beercss`,name:`Veela BeerCSS`,description:`Beer CSS compatible styling with Material Design 3`,variant:`beercss`,initFn:async()=>{try{await Vi(`beercss`),console.log(`[Styles] Veela BeerCSS loaded`)}catch(e){console.warn(`[Styles] Failed to load Veela BeerCSS:`,e)}}},"vl-core":{id:`vl-core`,name:`Veela Core`,description:`Shared foundation styles for all veela variants`,variant:`core`,initFn:async()=>{try{await Vi(`core`),console.log(`[Styles] Veela Core loaded`)}catch(e){console.warn(`[Styles] Failed to load Veela Core:`,e)}}},raw:{id:`raw`,name:`Raw`,description:`No styling framework, browser defaults`,variant:`core`,initFn:async()=>{console.log(`[Styles] Raw mode - no styles loaded`)}}},Ki=null}));function Ji(e,t,n){let r=new URL(e.includes(`://`)?e:`https://${e}`);r.protocol===`http:`?r.protocol=`ws:`:(r.protocol===`https:`||r.protocol!==`ws:`&&r.protocol!==`wss:`)&&(r.protocol=`wss:`),(!r.pathname||r.pathname===`/`||/^\/socket\.io\/?$/i.test(r.pathname))&&(r.pathname=`/ws`);for(let e of[`EIO`,`transport`,`sid`])r.searchParams.delete(e);return Xi(r,t),Xi(r,n),r.toString()}function Yi(e,t){return new Zi(e,t)}var Xi,Zi,Qi=e((()=>{Xi=(e,t)=>{if(!(!t||typeof t!=`object`))for(let[n,r]of Object.entries(t))!n||r==null||r===``||e.searchParams.set(n,String(r))},Zi=class{url;options;connected=!1;connecting=!1;id=``;ws=null;listeners=new Map;connectTimeout;constructor(e,t={}){this.url=e,this.options=t,this.connect()}connect(){try{let e=Ji(this.url,this.options.query,this.options.auth);this.connecting=!0,this.ws=new WebSocket(e),this.ws.onopen=()=>{this.connected=!0,this.connecting=!1,this.connectTimeout&&clearTimeout(this.connectTimeout),this.emitLocal(`connect`)},this.ws.onclose=e=>{this.connected=!1,this.connecting=!1,this.connectTimeout&&clearTimeout(this.connectTimeout),this.emitLocal(`disconnect`,e.reason||`closed`),this.emitLocal(`close`,e.code,e.reason)},this.ws.onerror=e=>{this.connecting=!1,this.emitLocal(`connect_error`,Error(`WebSocket error`)),this.emitLocal(`error`,e)},this.ws.onmessage=e=>{if(e.data instanceof ArrayBuffer){this.emitLocal(`binary`,e.data);return}if(typeof Blob<`u`&&e.data instanceof Blob){e.data.arrayBuffer().then(e=>this.emitLocal(`binary`,e));return}try{let t=JSON.parse(String(e.data));t.event&&t.payload?this.emitLocal(t.event,t.payload):this.emitLocal(`data`,t)}catch{this.emitLocal(`data`,e.data)}},this.options.timeout&&(this.connectTimeout=setTimeout(()=>{this.connected||(this.connecting=!1,this.ws?.close(),this.emitLocal(`connect_error`,Error(`timeout`)))},this.options.timeout))}catch(e){this.connecting=!1,setTimeout(()=>this.emitLocal(`connect_error`,e),0)}}on(e,t){this.listeners.has(e)||this.listeners.set(e,new Set),this.listeners.get(e).add(t)}off(e,t){this.listeners.get(e)?.delete(t)}send(e){this.connected&&this.ws&&this.ws.send(typeof e==`string`?e:JSON.stringify(e))}sendBinary(e){!this.connected||!this.ws||this.ws.send(e)}emit(e,...t){this.send(t[0])}emitLocal(e,...t){let n=this.listeners.get(e);if(n)for(let e of n)e(...t)}removeAllListeners(){this.listeners.clear()}close(){this.connectTimeout&&clearTimeout(this.connectTimeout),this.ws&&=(this.ws.close(),null),this.connected=!1,this.connecting=!1}disconnect(){this.close()}}}));function $i(e){let t=ea;if(!t)return null;try{return t.querySelector(`#${CSS.escape(e)}`)}catch{return null}}function P(e){let t=ea?.ownerDocument??(typeof document<`u`?document:null);if(!t){console.log(`[LOG]`,e);return}let n=t.createElement(`div`);n.textContent=`[${new Date().toLocaleTimeString()}] ${e}`;let r=na();r&&(r.appendChild(n),r.scrollTop=r.scrollHeight),console.log(`[LOG]`,e)}var ea,ta,na,ra=e((()=>{ea=null,ta=()=>$i(`wsStatus`),na=()=>$i(`logContainer`)}));async function ia(){let e=await sa();if(!e?.Clipboard?.read)return``;try{let t=(await e.Clipboard.read())?.value;if(typeof t==`string`&&t.trim())return t}catch{}return``}async function aa(e){let t=await sa();if(!t?.Clipboard?.write)return!1;try{return await t.Clipboard.write({string:String(e??``),label:`cwsp`}),!0}catch{return!1}}var oa,sa,ca=e((()=>{_(),oa=[`@supernotes/capacitor-clipboard`,`@capacitor/clipboard`],sa=async()=>{try{if(globalThis.document===void 0)return null}catch{return null}for(let e of oa)try{return await v(()=>import(e),[],import.meta.url)}catch{}return null}})),la=t({isCapacitorNativeShell:()=>F,openAppClipboardRelatedSettings:()=>xa,openNativeNotificationSettings:()=>ba,readClipboardTextFromDevice:()=>ya,writeClipboardImageToDevice:()=>ga,writeClipboardTextToDevice:()=>va});async function ua(){let e=await wa(`/service/clipboard?kind=text`);if(!e||e.ok===!1)return null;let t=typeof e.text==`string`&&e.text||typeof e.content==`string`&&e.content||typeof e.data==`string`&&e.data||``;return e.ok===!0||`text`in e||`data`in e?t:null}async function da(e){let t=await wa(`/service/clipboard`,{method:`POST`,body:JSON.stringify({kind:`text`,text:e,content:e,data:e})});return!!(t&&t.ok!==!1)}async function fa(e,t,n){let r=await wa(`/service/clipboard`,{method:`POST`,body:JSON.stringify({kind:`image`,mimeType:t,hash:n||void 0,imageBase64:e,asset:{mimeType:t,hash:n||void 0,data:e,source:`base64`}})});return!!(r&&r.ok!==!1)}async function pa(){if(!fe())return``;try{let e=await O(`clipboard:read-local`,{});return Ta(e)}catch{return``}}async function ma(e,t,n){if(!fe())return!1;try{return!!(await O(`clipboard:write-local-image`,{mimeType:t,hash:n||``,data:e}))?.ok}catch{return!1}}async function ha(e){if(!fe())return!1;try{return!!(await O(`clipboard:write-local`,{text:e}))?.ok}catch{return!1}}async function ga(e,t=`image/png`,n){let r=String(e??``).trim();if(!r)throw Error(`Clipboard image payload empty`);let i=String(t||`image/png`).trim()||`image/png`;if(Oa()){for(let e=0;e<4;e++){if(await fa(r,i,n))return;e+1<4&&await new Promise(t=>globalThis.setTimeout(t,120*(e+1)))}throw Error(`Desktop control clipboard image write failed`)}if(!await fa(r,i,n)&&!await ma(r,i,n)){if(Sa()&&globalThis.navigator?.clipboard?.write)try{let e=Ea(r);if(e?.length){let t=new Blob([e],{type:i}),n=i===`image/png`?t:await Da(t);await globalThis.navigator.clipboard.write([new ClipboardItem({[n.type]:n})]);return}}catch{}throw Error(`Clipboard image write unavailable`)}}async function _a(e,t=4){for(let n=0;n<t;n++){if(await da(e))return!0;n+1<t&&await new Promise(e=>globalThis.setTimeout(e,120*(n+1)))}return!1}async function va(e){let t=String(e??``);if(Oa()){if(await _a(t))return;throw Error(`Desktop control clipboard write failed`)}if(!await da(t)&&!await ha(t)&&!(Sa()&&await aa(t))){if(globalThis.navigator?.clipboard?.writeText){await globalThis.navigator.clipboard.writeText(t);return}throw Error(`Clipboard write unavailable`)}}async function ya(){if(Oa()){for(let e=0;e<4;e++){let t=await ua();if(t!==null)return t;e+1<4&&await new Promise(t=>globalThis.setTimeout(t,120*(e+1)))}throw Error(`Desktop control clipboard read failed`)}let e=await ua();if(e!==null)return e;let t=await pa();if(t)return t;if(Sa()){let e=await ia();if(e)return e}if(globalThis.navigator?.clipboard?.readText)return String(await globalThis.navigator.clipboard.readText());throw Error(`Clipboard read unavailable`)}async function ba(){if(Sa())try{let{NativeSettings:e,AndroidSettings:t,IOSSettings:n}=await v(async()=>{let{NativeSettings:e,AndroidSettings:t,IOSSettings:n}=await import(`capacitor-native-settings`);return{NativeSettings:e,AndroidSettings:t,IOSSettings:n}},[],import.meta.url);await e.open({optionAndroid:t.AppNotification,optionIOS:n.AppNotification})}catch{}}async function xa(){if(Sa())try{let{NativeSettings:e,AndroidSettings:t,IOSSettings:n}=await v(async()=>{let{NativeSettings:e,AndroidSettings:t,IOSSettings:n}=await import(`capacitor-native-settings`);return{NativeSettings:e,AndroidSettings:t,IOSSettings:n}},[],import.meta.url);await e.open({optionAndroid:t.ApplicationDetails,optionIOS:n.App})}catch{}}var Sa,F,Ca,wa,Ta,Ea,Da,Oa,ka=e((()=>{ca(),de(),_(),Sa=()=>{try{let e=globalThis.Capacitor;return typeof e?.isNativePlatform==`function`&&!!e.isNativePlatform()}catch{return!1}},F=()=>Sa(),Ca=()=>{try{let e=globalThis,t=e.__WEBNATIVE_AUTH__||e.__NEUTRALINO_AUTH__;return!t||typeof t.port!=`number`||(e.__CWS_WEBNATIVE_BOOT__||e.__CWS_NEUTRALINO_BOOT__||t.key,!t.key)?null:{port:t.port,key:String(t.key)}}catch{return null}},wa=async(e,t)=>{let n=Ca();if(!n)return null;try{let r=new Headers(t?.headers);r.set(`Content-Type`,`application/json`),r.set(`X-API-Key`,n.key);let i=await fetch(`http://127.0.0.1:${n.port}${e}`,{...t,headers:r,cache:`no-store`});return i.ok?await i.json():null}catch{return null}},Ta=e=>{if(!e||typeof e!=`object`)return``;let t=e,n=t.echo;if(n&&typeof n==`object`){let e=n;if(typeof e.text==`string`)return e.text;if(typeof e.value==`string`)return e.value}return typeof t.text==`string`?t.text:typeof t.value==`string`?t.value:typeof t.data==`string`?t.data:``},Ea=e=>{let t=e.trim();if(!t)return null;if(t.startsWith(`data:`)){let e=t.indexOf(`,`);if(e<0)return null;t=t.slice(e+1)}try{let e=globalThis.atob(t.replace(/\s+/g,``)),n=new Uint8Array(e.length);for(let t=0;t<e.length;t++)n[t]=e.charCodeAt(t);return n}catch{return null}},Da=async e=>{if(e.type===`image/png`)return e;if(typeof createImageBitmap==`function`&&typeof OffscreenCanvas<`u`){let t=await createImageBitmap(e),n=new OffscreenCanvas(t.width,t.height),r=n.getContext(`2d`);return r?(r.drawImage(t,0,0),t.close(),await n.convertToBlob({type:`image/png`})):e}return e},Oa=()=>{try{let e=globalThis;if(e.__CWS_NEUTRALINO_BOOT__||e.__CWS_WEBNATIVE_BOOT__||typeof e.NL_OS==`string`)return!0;let t=e.__WEBNATIVE_AUTH__||e.__NEUTRALINO_AUTH__;return!!(t&&typeof t.port==`number`&&t.key)}catch{return!1}}})),Aa,ja=e((()=>{Aa=4e3})),I,Ma,Na,Pa,Fa,Ia,La,L,Ra,za,Ba,Va=e((()=>{I=1e6,Ma=0,Na=0,Pa=()=>{if(Na=Date.now(),typeof process<`u`&&typeof process.hrtime?.bigint==`function`){Ma=Number(process.hrtime.bigint()/1000n)%I;return}try{let e=globalThis.performance;if(typeof e?.now==`function`){Ma=Math.floor(e.now()%1*I)%I;return}}catch{}Ma=0},Pa(),Fa=()=>{let e=Date.now(),t=0;if(typeof process<`u`&&typeof process.hrtime?.bigint==`function`){let n=e-Na;(n<0||n>6e4)&&Pa(),t=(Ma+Number(process.hrtime.bigint()/1000n))%I}else try{let e=globalThis.performance;typeof e?.now==`function`&&(t=Math.floor(e.now()%1*I)%I)}catch{t=0}let n=String(BigInt(e)*BigInt(I)+BigInt(t));return{ts:e,subUs:t,wireTime64:n,ts64:n,wireTs:n}},Ia=e=>{let t=Fa(),n=String(e.wireTime64??e.ts64??e.wireTs??t.wireTime64);return{...e,ts:Number(e.ts??t.ts),subUs:Number(e.subUs??t.subUs),wireTime64:n,ts64:n,wireTs:n}},La=e=>e&&typeof e==`object`&&!Array.isArray(e)?e:{},L=e=>{let t=Number(e);return Number.isFinite(t)&&t>0?t:0},Ra=e=>{let t=La(e.flags),n=La(e.payload??e.data),r=Ba(e.originTs??t.originTs??n.originTs);return r?.ts?r.ts:L(e.originTs)||L(t.originTs)||L(n.originTs)||L(e.timestamp)||L(e.ts)||L(t.timestamp)||L(t.ts)||L(n.timestamp)||L(n.ts)||0},za=e=>{let t=Fa(),n=Ra(e)||L(e.timestamp)||L(e.ts)||t.ts,r=L(e.timestamp)||L(e.ts)||n,i=Number(e.subUs??t.subUs),a=String(e.wireTime64??e.ts64??e.wireTs??t.wireTime64),o=La(e.flags),s={...o,originTs:o.originTs??e.originTs??n,wireTime64:o.wireTime64??a,ts64:o.ts64??o.wireTime64??a,wireTs:o.wireTs??o.wireTime64??a};return{...e,originTs:e.originTs??n,ts:r,subUs:i,wireTime64:a,ts64:a,wireTs:a,timestamp:r,flags:s}},Ba=e=>{let t=String(e??``).trim();if(!/^\d+$/.test(t))return null;try{let e=BigInt(t),n=Number(e/BigInt(I)),r=Number(e%BigInt(I));return Number.isFinite(n)?{ts:n,subUs:r,wireTime64:t,ts64:t,wireTs:t}:null}catch{return null}}})),Ha,Ua,Wa,Ga,Ka,qa,Ja,Ya,Xa,Za,Qa=e((()=>{Va(),Ha=e=>e&&typeof e==`object`&&!Array.isArray(e)?e:{},Ua=()=>{try{let e=globalThis.performance;if(typeof e?.now==`function`)return e.now()}catch{}return Date.now()},Wa=(e=Ua())=>Math.round(e*10)&65535,Ga=e=>{let t=String(e||``).trim().toLowerCase();return t.startsWith(`mouse:`)||t.startsWith(`keyboard:`)||t.startsWith(`airpad:mouse`)||t.startsWith(`airpad:keyboard`)},Ka=e=>{let t=String(e||``).trim().toLowerCase();return t===`mouse:click`||t===`mouse:down`||t===`mouse:up`||t===`keyboard:tap`||t===`keyboard:type`||t===`keyboard:toggle`},qa=e=>{let t=Ha(e.payload??e.data??e.body),n=String(t.op??t.action??t.type??``).trim().toLowerCase();if(n)return n;let r=t.params;return Array.isArray(r)&&r.length>0?String(r[0]??``).trim().toLowerCase():``},Ja=e=>{if(typeof e==`string`)return Ka(e);let t=String(e.what||e.type||``).trim().toLowerCase();if(Ka(t))return!0;if(t===`airpad:mouse`||t.startsWith(`airpad:mouse`)){let t=qa(e)||`move`;return t===`click`||t===`mouse:click`||t===`down`||t===`mouse:down`||t===`up`||t===`mouse:up`}return!!(t===`airpad:keyboard`||t.startsWith(`airpad:keyboard`))},Ya=e=>{let t=String(e||``).trim().toLowerCase();return t.startsWith(`clipboard:`)||t.startsWith(`airpad:clipboard:`)},Xa=e=>Ga(e)||Ya(e),Za=e=>{let t=Ha(e),n=Number(t.perfTs??Ua());return{...Ia(t),perfTs:n,perfTsLo:Number(t.perfTsLo??Wa(n))}}})),$a,eo,to,no,ro,io,ao,oo,so,co,lo,uo,fo,po,mo,ho,go,_o,vo,yo,bo=e((()=>{ja(),Va(),Qa(),$a=`wireHash`,eo=new Set([`ts`,`subUs`,`wireTime64`,`ts64`,`wireTs`,`perfTs`,`perfTsLo`,$a,`source`,`from`,`clientId`,`userId`,`sender`]),to=e=>e&&typeof e==`object`&&!Array.isArray(e)?e:{},no=e=>{if(!e)return``;let t=5381;for(let n=0;n<e.length;n+=1)t=(t<<5)+t+e.charCodeAt(n)|0;return(t>>>0).toString(36)},ro=e=>String(e??``).replace(/\r\n/g,`
`).trim(),io=e=>{if(e==null)return``;if(typeof e!=`object`)return JSON.stringify(e);if(Array.isArray(e))return`[${e.map(io).join(`,`)}]`;let t=e;return`{${Object.keys(t).filter(e=>!eo.has(e)).sort().map(e=>`${JSON.stringify(e)}:${io(t[e])}`).join(`,`)}}`},ao=e=>{for(let t of[`asset`,`dataAsset`,`file`,`image`]){let n=to(e[t]),r=String(n.hash??``).trim();if(r)return r}return``},oo=(e,t)=>{for(let t of[`text`,`content`,`body`]){let n=e[t];if(typeof n==`string`&&n.trim())return n}for(let e of[`payload`,`data`,`result`]){let n=t[e];if(typeof n==`string`&&n.trim())return n}return``},so=e=>{let t=String(e||``).trim().toLowerCase();return t?t.startsWith(`clipboard:`)||t.startsWith(`airpad:clipboard:`)?`clipboard`:t.startsWith(`mouse:`)||t.startsWith(`keyboard:`)||t.startsWith(`airpad:mouse`)||t.startsWith(`airpad:keyboard`)?`input`:`general`:`general`},co=e=>e===`clipboard`?250:e===`input`?180:400,lo=(e,t)=>!!(t===`ask`||t===`request`||!e||e.endsWith(`:read`)||e.endsWith(`:get`)||e.endsWith(`:isready`)),uo=e=>{let t=to(e.flags),n=to(e.payload??e.data);return String(t.wireHash??e.wireHash??n.wireHash??``).trim()},fo=e=>{let t=String(e.op||`act`).trim().toLowerCase(),n=String(e.what||e.type||``).trim().toLowerCase();if(lo(n,t))return``;let r=String(e.byId||e.from||e.sender||``).trim().toLowerCase(),i=to(e.payload??e.data??e.body??{});if(n.includes(`clipboard`)){let a=oo(i,e),o=a?``:ao(i),s=a?no(ro(a)):o?`asset:${o}`:no(io(i));if(!s)return``;let c=String(e.uuid??``).trim(),l=c?`|u:${c}`:``;return no(`${t}|${n}|${r}|${s}${l}`)}if(so(n)===`input`){let e=i.perfTs??i.perfTsLo??``;return no(`${t}|${n}|${r}|${io(i)}|p:${e}`)}return no(`${t}|${n}|${r}|${io(i)}`)},po=e=>{let t=za(e),n=fo(t);if(!n||uo(t)===n)return t;let r={...to(t.flags),[$a]:n},i=t.payload??t.data,a=i;return i&&typeof i==`object`&&!Array.isArray(i)&&(a={...i,[$a]:n}),t.payload===void 0?t.data===void 0?{...t,flags:r,payload:a}:{...t,flags:r,data:a}:{...t,flags:r,payload:a}},mo=class{maxEntries;seen=new Map;constructor(e=512){this.maxEntries=e}shouldSuppress(e,t){let n=String(e.what||e.type||``).trim().toLowerCase(),r=String(e.op||`act`).trim().toLowerCase();if(lo(n,r))return!1;let i=uo(e)||fo(e);if(!i)return!1;let a=t??so(n),o=co(a),s=Date.now(),c=`${a}|${i}`,l=this.seen.get(c);return this.seen.set(c,s),this.prune(s,o),l!==void 0&&s-l<o}clear(){this.seen.clear()}prune(e,t){let n=Math.max(t*4,4e3);for(let[t,r]of this.seen.entries())e-r>n&&this.seen.delete(t);if(this.seen.size<=this.maxEntries)return;let r=[...this.seen.entries()].sort((e,t)=>e[1]-t[1]);for(let e=0;e<r.length-this.maxEntries;e+=1)this.seen.delete(r[e][0])}},ho=new mo,go=class{maxEntries;seen=new Map;constructor(e=512){this.maxEntries=e}shouldSuppress(e){if(yo(e)||Ja(e))return!1;let t=String(e.what||e.type||``).trim().toLowerCase(),n=String(e.op||`act`).trim().toLowerCase();if(lo(t,n))return!1;let r=uo(e)||fo(e);if(!r)return!1;let i=Date.now(),a=`relay|${r}`,o=this.seen.get(a);return this.seen.set(a,i),this.prune(i),o!==void 0&&i-o<4e3}clear(){this.seen.clear()}prune(e){let t=Math.max(Aa*4,4e3);for(let[n,r]of this.seen.entries())e-r>t&&this.seen.delete(n);if(this.seen.size<=this.maxEntries)return;let n=[...this.seen.entries()].sort((e,t)=>e[1]-t[1]);for(let e=0;e<n.length-this.maxEntries;e+=1)this.seen.delete(n[e][0])}},new go,_o=e=>{let t=String(e||``).trim().toLowerCase();return t===`mouse:move`||t===`mouse:scroll`},vo=e=>{let t=to(e.payload??e.data??e.body),n=String(t.op??t.action??t.type??``).trim().toLowerCase();if(n)return n;let r=t.params;return Array.isArray(r)&&r.length>0?String(r[0]??``).trim().toLowerCase():``},yo=e=>{if(typeof e==`string`)return _o(e);let t=String(e.what||e.type||``).trim().toLowerCase();if(_o(t))return!0;if(t!==`airpad:mouse`&&!t.startsWith(`airpad:mouse`))return!1;let n=vo(e)||`move`;return n===`move`||n===`mouse:move`||n===`scroll`||n===`mouse:scroll`}}));function xo(e){So=e}var So,Co=e((()=>{})),R,z,wo,B,To,Eo,Do,Oo,ko,Ao,jo,Mo,No,Po,Fo,Io=e((()=>{de(),p(),Qa(),ct(),Xc(),R=!1,z=0,wo=1200,B=()=>zc()&&fe()&&Be(),To=6e3,Eo=async()=>{if(!B())return R=!1,!1;try{let e=await h(O(`coordinator:status`,{}),To,`coordinator:status timed out`),t=!!(e.echo?.connected??e.ok);return R=t,z=Date.now(),t}catch{return R=!1,z=Date.now(),!1}},Do=async()=>{if(!B())return!1;try{return(await h(O(`runtime:reload-settings`,{}),To,`runtime:reload-settings timed out`))?.ok?(R=!1,z=0,Eo()):(R=!1,z=Date.now(),!1)}catch{return R=!1,z=Date.now(),!1}},Oo=()=>B()?(Date.now()-z>wo&&Eo(),R):!1,ko=(e,t)=>!Xa(e)||!t||typeof t!=`object`||Array.isArray(t)?t??{}:Za(t),Ao=async e=>{if(!B())return!1;let t=e instanceof Uint8Array?e:new Uint8Array(e),n=``;for(let e=0;e<t.length;e++)n+=String.fromCharCode(t[e]??0);let r=btoa(n);try{let e=await O(`coordinator:binary`,{data:r,encoding:`base64`}),t=!!(e?.sent??e.echo?.sent??e.ok);return t&&(R=!0,z=Date.now()),t}catch{return R=!1,z=Date.now(),!1}},jo=async e=>{if(!B())return!1;try{let t=await O(e?`airmouse:start`:`airmouse:stop`,{}),n=t.echo??{},r=!!t.ok;return r&&(R=!0,z=Date.now()),e?r&&n.active!==!1:r}catch{return!1}},Mo=()=>jo(!0),No=()=>jo(!1),Po=async e=>{let t=(await O(`coordinator:dispatch`,{what:e.what,payload:ko(e.what,e.payload),nodes:e.nodes??[],uuid:e.uuid??``,op:e.op})).echo??{};if(t.result!==void 0)return t.result;if(typeof t.body==`string`&&t.body.trim())try{let e=JSON.parse(t.body);return e.result??e.payload??e.data??t.body}catch{return t.body}return t.result??null},Fo=async e=>{if(!B())return!1;let t=e.op===`ask`?`coordinator:ask`:`coordinator:act`;try{let n=await O(t,{what:e.what,payload:ko(e.what,e.payload),nodes:e.nodes??[],uuid:e.uuid??``,op:e.op}),r=!!(n.echo?.sent??n.ok);return r&&(R=!0,z=Date.now()),r}catch{return R=!1,z=Date.now(),!1}}})),Lo=t({connectWS:()=>G,disconnectWS:()=>ls,getLastServerClipboard:()=>Uo,getWS:()=>Ro,initWebSocket:()=>us,isWSConnected:()=>zo,markTransportDisconnected:()=>Ho,onServerClipboardUpdate:()=>Wo,onVoiceResult:()=>Go,onWSConnectionChange:()=>Bo,reconnectNativeCoordinatorTransport:()=>Do,reconnectTransportAfterLifecycleResume:()=>cs,refreshNativeCoordinatorStatus:()=>Eo,refreshTransportConnectionStatus:()=>Vo,sendCoordinatorAct:()=>ts,sendCoordinatorAsk:()=>rs,sendCoordinatorRequest:()=>is,sendWsBinary:()=>ns,shouldUseNativeCoordinatorTransport:()=>B,startNativeAirMouse:()=>Mo,stopNativeAirMouse:()=>No});function Ro(){return K}function zo(){return B()?Oo():fs}function Bo(e){As.add(e);try{e(zo())}catch{}return()=>As.delete(e)}async function Vo(){if(B()){let e=await Eo();return W(e),e}let e=!!(fs||K?.connected);return W(e),e}function Ho(){W(!1)}function Uo(){return js}function Wo(e){return Ms.add(e),()=>Ms.delete(e)}function Go(e){return Ns.add(e),()=>Ns.delete(e)}function Ko(e,t){for(let n of Ms)try{n(e,t)}catch{}}async function qo(){if(!K?.connected||!Le()||!Re())return;let e=Qe();if(e.length)try{let t=await ya(),n=String(t??``).trim();if(!n)return;let r=Date.now();if(r<Xs&&n===Ys||n===qs&&r-Js<Ks)return;qs=n,Js=r;let i=groupWireTargetsByAccessToken(e,_c());for(let e of i)ts(`clipboard:update`,{text:n},e.nodeIds,{accessToken:e.accessToken})}catch{}}async function Jo(e,t){if(!Le())return;let n=typeof e==`string`?e:``,r=n.trim();if(r.toLowerCase().startsWith(`data:image/`)){await Yo({mimeType:`image/png`,data:r},t);return}let i=Date.now();if(!(r&&r===Zs&&i-Qs<Ks)&&(Zs=r,Qs=i,js=n,Ko(n,t),!(!Ue()||!r)&&!(r===Ys&&i<Xs)))try{await va(r),Ys=r,qs=r,Js=i,Xs=i+Ks}catch(e){console.warn(`[cwsp:clipboard] device write failed`,{length:n.length,source:t?.source,error:cc(e)})}}async function Yo(e,t){if(!Le())return;let n=String(e.data??``).trim();if(!n)return;let r=String(e.mimeType||`image/png`).trim()||`image/png`,i=e.hash?.trim()||n.slice(0,96),a=Date.now();if(!(i&&i===Zs&&a-Qs<Ks)&&(Zs=i,Qs=a,Ko(``,t),Ue()&&!(i===Ys&&a<Xs)))try{await ga(n,r,e.hash),Ys=i,qs=i,Js=a,Xs=a+Ks}catch(n){console.warn(`[cwsp:clipboard] device image write failed`,{mimeType:r,hash:e.hash,source:t?.source,error:cc(n)})}}function Xo(e){try{return JSON.stringify(e)}catch{return String(e)}}function Zo(){return it()===`secure`?`secure`:`plaintext`}function V(e){let t=e.trim();return/^l-/i.test(t)?t.slice(2).trim():t}function Qo(e){let t=V(e.trim()).toLowerCase();return t===`localhost`||t===`127.0.0.1`||t===`::1`}function $o(e){if(!e)return!1;let t=V(e);return t===`localhost`||e===`localhost`||e.endsWith(`.local`)?!0:/^\d{1,3}(?:\.\d{1,3}){3}$/.test(t)?t.startsWith(`10.`)||t.startsWith(`192.168.`)||/^172\.(1[6-9]|2\d|3[01])\./.test(t)||t.startsWith(`127.`)||/^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(t):!1}async function es(e,t){if(!(!e||!t)&&$o(t)&&location.protocol===`https:`&&!xs.has(e)){xs.add(e);try{await fetch(`${e}/lna-probe`,{method:`GET`,mode:`cors`,cache:`no-store`,credentials:`omit`,targetAddressSpace:`local`})}catch(e){P(`LNA probe: ${String(e?.message||e||``)||`request failed`}`)}}}function ts(e,t,n,r){let i=Fc(e,t),a=Ec(`act`,e,i,{nodes:n,accessToken:r?.accessToken});return Tc(a)?!0:Ws(e)?(G(),!1):(Vs.length>=Hs&&Vs.shift(),Vs.push(a),G(),!0)}function ns(e){if(B())return Ao(e),Oo();if(!K?.connected)return!1;let t=K;return typeof t.sendBinary==`function`?(t.sendBinary(e),!0):!1}function rs(e,t,n){return new Promise((r,i)=>{(async()=>{if(B()){try{if(!await Gs()){i({ok:!1,error:`Native WS not connected`});return}r(await Po({op:`ask`,what:e,payload:Fc(e,t),nodes:n}))}catch(e){i({ok:!1,error:String(e?.message||e)})}return}if(!await Gs()||!K?.connected){i({ok:!1,error:`WS not connected`});return}let a=yc(),o=globalThis.setTimeout(()=>{Q.delete(a),i({ok:!1,error:`Timeout waiting for ${e}`})},Ds);Q.set(a,{resolve:r,reject:i,timeoutId:o}),Tc(Ec(`ask`,e,Fc(e,t),{nodes:n,uuid:a}))})()})}function is(e,t,n){return new Promise((r,i)=>{(async()=>{if(B()){try{if(!await Gs()){i({ok:!1,error:`Native WS not connected`});return}r(await Po({op:`act`,what:e,payload:Fc(e,t),nodes:n}))}catch(e){i({ok:!1,error:String(e?.message||e)})}return}if(!await Gs()||!K?.connected){i({ok:!1,error:`WS not connected`});return}let a=yc(),o=globalThis.setTimeout(()=>{Q.delete(a),i({ok:!1,error:`Timeout waiting for ${e}`})},Ds);Q.set(a,{resolve:r,reject:i,timeoutId:o}),Tc(Ec(`act`,e,Fc(e,t),{nodes:n,uuid:a}))})()})}function H(){if(ms){if(q||K&&K.connected===!1){ms.textContent=`WS…`;return}fs||K&&K.connected?ms.textContent=`WS ✓`:ms.textContent=`WS ↔`}}function U(e,t){let n=t.trim();P(`[ws-state] event=${e}${n?` ${n}`:``}`)}function as(e){let t=ta();t&&(t.textContent=F()?`TLS failed — install your CA in Android Settings → Security → Encryption & credentials (or use Remote host = name on the cert). Try HTTP :8080 if the server allows. ${e}`:`Untrusted cert — open ${e} in this browser, accept, then retry`,t.classList.add(Ic),t.classList.remove(`ws-status-ok`),t.classList.add(`ws-status-bad`))}function os(e){let t=ta();t&&(t.textContent=`TLS name mismatch for raw IP — set Remote host to ${e} (name on certificate), keep ports as needed`,t.classList.add(Ic),t.classList.remove(`ws-status-ok`),t.classList.add(`ws-status-bad`))}function W(e){fs=e,e&&Us();let t=ta();t&&(t.classList.remove(Ic),e?(t.textContent=`connected`,t.classList.remove(`ws-status-bad`),t.classList.add(`ws-status-ok`)):(t.textContent=`disconnected`,t.classList.remove(`ws-status-ok`),t.classList.add(`ws-status-bad`))),H();for(let t of As)try{t(e)}catch{}}function ss(e){if(e.type===`voice_result`||e.type===`voice_error`){let t=e.error||e.message||`Actions: `+JSON.stringify(e.actions||[]);for(let n of Ns)try{n({text:t,type:e.type===`voice_error`?`voice_error`:`voice_result`,actions:e.actions,error:e.error})}catch{}P(`Voice result: `+t)}}function cs(e){if(globalThis.window){U(`lifecycle-reconnect`,e),ec(),Os(),J+=1,gs=!1;for(let[e,t]of Q.entries())clearTimeout(t.timeoutId),t.reject({ok:!1,error:`Disconnected before response for ${e}`}),Q.delete(e);for(let e of[...Y])X(e),e.removeAllListeners(),e.close(),Y.delete(e);if(q=!1,K)try{K.removeAllListeners(),K.disconnect()}catch{}K=null,ps(null),W(!1),_s=0,ho.clear(),G()}}function G(){if(ot()){P(`WS skip: Node clipboard-hub owns fleet /ws (WebView must not connect)`);return}if(zc()){P(`WS skip: Java CwspBridgeService owns fleet /ws (WebView must not connect)`);return}if(q||K&&(K.connected||K.connecting)||Y.size>0)return;Os(),J+=1;let e=J;gs=!1;let t=qe().trim(),n=$e().trim(),r=t||n||``,i=Ve(),a=e=>!!e&&/^\d{1,3}(?:\.\d{1,3}){3}$/.test(e),o=e=>!e||!a(e)?!1:e.startsWith(`10.`)||e.startsWith(`192.168.`)||/^172\.(1[6-9]|2\d|3[01])\./.test(e)||/^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(e),s=e=>a(e)&&e.startsWith(`192.168.0.`),c=e=>V(e).trim().toLowerCase()===`192.168.0.200`,l=e=>V(e).trim().toLowerCase().includes(`45.147.`),u=e=>c(e)||l(e),d=location.hostname||``,f=V(d)||d,p=Se(f),m=at().trim(),h=Me(m,n||t)||m,g=he(n||t,h),_=me(h,n||t,He()),v=je(f),y=p||Ee(f)||ve(n,f)||g&&!v||De(n)&&p,b=e=>{let t=[],n=[],r=[],i=[],u=[],d=[],f=[];for(let p of e)a(p.host)?c(p.host)?i.push(p):l(p.host)?u.push(p):s(p.host)||p.host===`127.0.0.1`?r.push(p):o(p.host)?f.push(p):d.push(p):p.source===`page`?n.push(p):t.push(p);return y?[...u,...i,...t,...d,...r,...n,...f]:v?[...r,...i,...u,...t,...n,...d,...f]:[...u,...i,...r,...t,...n,...d,...f]},ee=e=>/^\d{1,5}$/.test(e),x=e=>e.trim().replace(/^[a-z][a-z0-9+.-]*:\/\//i,``).split(`/`)[0],S=e=>{let t=x(e).trim();if(!t)return null;let n=t.lastIndexOf(`:`);if(n<=0)return{host:t};let r=t.slice(0,n),i=t.slice(n+1);return!r||!ee(i)?{host:t}:{host:r,port:i}},C=ge(t).map(e=>S(e)).filter(e=>!!e&&!!e.host);if(p&&De(n)&&!g){let e=C.filter(e=>{let t=V(e.host).trim();return t?u(t)?!0:!(a(t)&&s(t)):!1});e.length&&(C=e)}if(!C.length&&n){let e=S(n);e?.host&&(C=[e])}let w=(C[0]?.port||``).trim(),te=h?S(h):void 0,T=d,E=/^(localhost|127\.0\.0\.1)$/.test(T)||/^\d{1,3}(?:\.\d{1,3}){3}$/.test(T)&&(T.startsWith(`10.`)||T.startsWith(`192.168.`)||/^172\.(1[6-9]|2\d|3[01])\./.test(T));if(location.protocol===`https:`&&i===`http`&&!F()){P(`WebSocket error: browser blocks ws/http from https page (mixed content). Open Airpad via http:// or use valid HTTPS cert on endpoint.`),q=!1,W(!1),H();return}let D=C[0],ne=D?.host||r,re=D?.port,ie=(()=>{if(Oe(h))return Ne(h);if(Oe(m))return Ne(m);if(we(h)||Ce(h))return Ne(h);if(g&&we(m))return Ne(m);if(_||De(n)||De(t))return ke;let e=te?.host||``;return e&&Te(e)?Ne(e):e||h||``})(),ae=(te?.port||``).trim(),oe=(ne||r||``).trim(),se=oe.length>0?V(oe)||oe:``,ce=(()=>{let e=se.trim();if(!e)return``;let t=e.lastIndexOf(`:`);return t>0&&ee(e.slice(t+1))?e.slice(0,t):e})(),le=i===`http`||i===`https`?i:w===`443`||w===`8434`||w===`8444`?`https`:w===`80`||w===`8080`||w===`8081`?`http`:F()&&location.protocol===`https:`&&ce&&a(ce)&&o(ce)?`https`:F()&&location.protocol!==`https:`&&ce&&a(ce)&&o(ce)?`http`:location.protocol===`https:`?`https`:`http`,ue=oe,de=V(ue)||ue,fe=w||(le===`https`?`8434`:`8080`);if(es(`${le}://${de}:${fe}`,de),T&&Qo(de)&&!Qo(T)&&$o(T)){let e=V(T)||T;es(`${le}://${e}:${fe}`,e)}let pe=le===`https`?`http`:`https`,O={http:[...ye],https:[..._e]},be=location.port?.trim?.()||``,xe=be||(location.protocol===`https:`?`443`:location.protocol===`http:`?`80`:``),Ae=i===`http`?[`http`]:i===`https`?[`https`]:[le,pe],Pe=e=>_e.includes(e),Fe=e=>ye.includes(e),Ie=(e,t)=>{let n=[],r=(t&&ee(t)?t:``)||(w&&ee(w)?w:``);if(r&&(e===`https`?(Pe(r)||i===`https`||i===`auto`)&&n.push(r):(Fe(r)||i===`http`||i===`auto`)&&n.push(r),!n.length&&i===e&&n.push(r),n.length))return n.filter((e,t)=>n.indexOf(e)===t);for(let t of O[e])n.push(t);return be&&n.push(be),n.filter((e,t)=>n.indexOf(e)===t)},Le=e=>V(e.trim())||e.trim(),Re=[];for(let e of C){let t=Le(e.host);t&&Re.push({host:t,source:`remote`,preferPort:e.port})}if(C.length===0&&t){let e=Le(t);e&&Re.push({host:e,source:`remote`})}let Be=new Set;for(let e of C)e.host&&Be.add(e.host.toLowerCase());if(C.length===0&&t.trim())for(let e of ge(t.trim())){let t=S(e);t?.host&&Be.add(t.host.toLowerCase())}let Ue=()=>{for(let e of Be){let t=V(e).toLowerCase();if(t===`localhost`||t===`127.0.0.1`||a(t)&&o(t))return!0}return!1},Ge=T.toLowerCase(),Ke=V(T)||T,Je=String(location.protocol||``).toLowerCase(),Ye=Je===`chrome-extension:`||Je===`moz-extension:`||Je===`safari-web-extension:`||/^[a-p]{32}$/.test(Ge),Xe=!!Ge&&Ee(Ke)&&!Be.has(Ge),Qe=!!T&&Be.size>0&&Ue()&&!E&&!Be.has(Ge),tt=p&&!!T&&Qo(T);location.hostname&&!Ye&&!Qe&&!Xe&&!tt&&Re.push({host:location.hostname,source:`page`,...xe?{preferPort:xe}:{}});let rt=new Map;for(let e of Re)e.host&&!rt.has(e.host)&&rt.set(e.host,e);let it=Array.from(rt.values()),st=b(it),ct=[];for(let e of Ae){if(location.protocol===`https:`&&e===`http`)continue;let t=e===`https`?st:it;for(let n of t){let{host:t,source:r,preferPort:i}=n,s=T&&t===T&&xe&&(!i||i===xe)?xe:i;for(let n of Ie(e,s)){let i=V(t).trim()||t.trim(),s=a(i)&&o(i),c=location.protocol===`https:`&&!E&&s,l=F()&&s||location.protocol===`https:`&&E&&s||c&&s;ct.push({url:`${e}://${t}:${n}`,protocol:e,host:t,source:r,port:n,privateLanHint:l})}}}let k=ct.filter((e,t)=>ct.findIndex(t=>t.url===e.url)===t);if(k.length===0){q=!1,W(!1),H();return}let lt=k.length>0?bs%k.length:0,A=k.slice(lt).concat(k.slice(0,lt));bs=lt,ys=A,ys.length<=1&&(bs=0);let ut=()=>{ys.length>1&&(bs=(bs+1)%ys.length)};q=!0,H();let j=Le(ne||t||``),dt=ae||re||w||(le===`https`?`8434`:`8080`),ft=ie,pt=ft||j||``,mt=()=>{if(!j)return!0;let e=Ne(ft);if(!e)return!u(j);let t=j.trim().toLowerCase(),n=V(e).toLowerCase();return!n||!t||n===t||e.toLowerCase()===`l-${t}`?!0:(Ce(e),!1)},ht=e=>{let n=e.url,r=gc(),i=_c(),a=et(),o=hc(),s=Ze().trim(),c={};r&&(c.token=r,c.userKey=r),i&&(c.accessToken=i),a&&(c.clientAccessToken=a),o&&(c.clientId=o),s&&(c.peerInstanceId=s,c.deviceInstanceId=s);let l={};s&&(l.peerInstanceId=s,l.deviceInstanceId=s),l.connectionType=We(),l.archetype=nt(),l.cwspEnvelope=`v2`,o&&(l.clientId=o,l.userId=o),r&&(l.token=r,l.userKey=r),l[Z.via]=mt()?e.source||`unknown`:`tunnel`,l[Z.localEndpoint]=mt()?`1`:`0`;let u=ft||(Oe(m)?`L-200`:``)||``,d=u||pt,f=u||ft||j||pt,p=V(e.host||``).trim(),h=V(T||``).trim();return e.source===`page`&&p&&h&&p.toLowerCase()===h.toLowerCase()&&Qo(d)&&(d=p,f=p),d&&(l[Z.route]=d,l[Z.routeTarget]=f),ks()&&(l[Z.hop]=e.host||t||`unknown`,l[Z.host]=e.host||t||``,l[Z.target]=j||``,l[Z.targetPort]=dt,l[Z.viaPort]=e.port||``,l[Z.protocol]=e.protocol||`https`),a&&(l.clientAccessToken=a),i&&(l.accessToken=i),{url:n,clientToken:r,accessToken:i,clientId:o,peerInstanceId:s,handshakeAuth:c,queryParams:l}},gt=(e,t,n,r)=>{K=e,U(`connected`,`candidate=${n+1}/${A.length} candidate_url=${r} transport=${t.protocol} parallel=${Ts}`),q=!1,_s=0,Os(),W(!0),tc(),K.on(`disconnect`,e=>{ec(),U(`disconnected`,`candidate=${n+1}/${A.length} candidate_url=${r} reason=${e||`unknown`}`),q=!1,W(!1),H();let t=gs;gs=!1;for(let[e,t]of Q.entries())clearTimeout(t.timeoutId),t.reject({ok:!1,error:`Disconnected before response for ${e}`}),Q.delete(e);if(K=null,t){_s=0;return}pc(e)&&(ut(),ys.length>1&&P(`WebSocket disconnect reason "${e||`unknown`}", trying next candidate on reconnect`));let i=_s+1;if(!fc(e))return;_s=i;let a=Math.min(Ss*i,5e3);Os(),vs=globalThis.setTimeout(()=>{vs=null,!(q||fs||K&&K.connected||K?.connecting)&&(U(`auto-reconnect`,`attempt=${`${i}/unlimited`} reason=${e||`unknown reason`}`),G())},a)}),K.on(`connect_error`,e=>{U(`socket-connect-error`,`candidate=${n+1}/${A.length} candidate_url=${r} reason=${e?.message||`unknown`}`),q=!1,H()}),K.on(`voice_result`,async e=>{ss(await kc(e))}),K.on(`voice_error`,async e=>{ss(await kc(e))}),K.on(`clipboard:update`,async e=>{let t=await kc(e);if(!ze(oc(t)))return;let n=ac(t);if(n){Yo(n,{source:t?.source});return}Jo(ic(t),{source:t?.source})}),K.on(`data`,async e=>{let t=await kc(e);bc(t)&&wc(t)}),K.on(`message`,async e=>{let t=await kc(e);bc(t)&&wc(t)}),K.on(`network.fetch`,async(e,t)=>{let n=await Pc(e);typeof t==`function`&&t(n)}),ps(K)},_t=(t,n)=>new Promise(r=>{if(e!==J){r(!1);return}let i=A.slice(t,t+Ts);if(!i.length){r(!1);return}if(t===0&&n===0){let e=ta();e&&(e.classList.remove(Ic),e.textContent=`connecting…`)}let s=!1,c=!1,l=0,u=i.length,d=null,f=null,p=(e,t,n,i,a)=>{if(!c){c=!0,s=!0;for(let t of[...Y])t!==e&&(X(t),t.removeAllListeners(),t.close(),Y.delete(t));X(e),Y.delete(e),gt(e,t,n,i),r(!0)}},m=()=>{c||s||(l++,!(l<u)&&(c=!0,d?as(d):f&&os(f),r(!1)))};for(let n=0;n<i.length;n++){let r=i[n],l=t+n,h=ht(r),{url:g,handshakeAuth:_,queryParams:v}=h;U(`connecting`,`batch=${t}-${t+u-1} candidate=${l+1}/${A.length} candidate_url=${g} transport=${r.protocol} source=${r.source} host=${r.host}:${r.port} target=${j}:${dt}`);let y=Yi(g,{auth:_,query:v,timeout:Cs});Y.add(y),y.__cwspProbeTimer=globalThis.setTimeout(()=>{if(e!==J){X(y),y.removeAllListeners(),y.close(),Y.delete(y);return}s||c||y.connected||(X(y),y.removeAllListeners(),y.close(),Y.delete(y),U(`connect-failed`,`candidate=${l+1}/${A.length} candidate_url=${g} reason=probe-hard-timeout`),m())},ws),y.on(`connect`,()=>{if(X(y),e!==J){y.removeAllListeners(),y.close(),Y.delete(y);return}if(s||c){y.removeAllListeners(),y.close(),Y.delete(y);return}p(y,r,l,g,h)}),y.on(`connect_error`,e=>{if(X(y),Y.delete(y),s||c){y.removeAllListeners(),y.close();return}y.removeAllListeners(),y.close();let t=e?.description||e?.context||``,n=String(e?.message||e||``),i=`${n} ${String(t)}`,u=r.protocol===`https`&&o(r.host)&&/xhr poll error|websocket error/i.test(n),p=/certificate|cert\.|ssl|tls|trust|ERR_CERT|ERR_SSL|handshake|authority|SELF_SIGNED|unknown.*cert|invalid.*cert|unable to verify|pkix|hostname|name mismatch/i.test(i),h=/refused|ECONNREFUSED|ENOTFOUND|timed out|timeout|unreachable|ERR_CONNECTION|ADDRESS_UNREACHABLE|NAME_NOT_RESOLVED|INTERNET_DISCONNECTED|network.*lost/i.test(i),_=F();u&&!d&&(p||!_&&!h)&&(d=g);let v=r.protocol===`https`&&a(r.host)&&!o(r.host)&&r.host!==`127.0.0.1`,b=`${n} ${String(t)}`;if(v&&/xhr poll error|websocket error|certificate|CERT|common name|ssl|tls|failed to fetch|name invalid/i.test(b)&&!f){let e=T&&!a(T)&&T!==`localhost`?T:``;e&&(f=e)}r.privateLanHint&&/cors|private network|address space|failed fetch/i.test(n)&&U(`connect-failed`,`candidate=${l+1}/${A.length} candidate_url=${g} reason=${n} hint=private-network-cors`),U(`connect-failed`,`candidate=${l+1}/${A.length} candidate_url=${g} reason=${n} details=${t?Xo(t):`none`}`),m()})}});(async()=>{for(let t=0;t<3;t++){for(let n=0;n<A.length;n+=Ts)if(e!==J||await _t(n,t))return;t+1<3&&(U(`retry`,`round=${t+2}/3 next=0`),await new Promise(e=>globalThis.setTimeout(e,450)))}e===J&&(U(`failed`,`round=3/3 all-candidates`),q=!1,W(!1),H())})()}function ls(){ec(),Os(),J+=1,gs=!0;for(let e of[...Y])X(e),e.removeAllListeners(),e.close(),Y.delete(e);if(q=!1,!K){W(!1),H();return}P(`Disconnecting WebSocket...`),K.disconnect(),K=null,ps(null),W(!1)}function us(e){ms=e,H(),e&&hs!==e&&(hs&&hs.removeEventListener(`click`,ds),hs=e,hs.addEventListener(`click`,ds))}function ds(){q||fs||K&&K.connected||K?.connecting?ls():G()}var K,fs,q,ps,ms,hs,J,Y,gs,_s,vs,ys,bs,xs,Ss,Cs,ws,Ts,Es,Ds,Os,X,Z,ks,As,js,Ms,Ns,Ps,Fs,Is,Ls,Rs,zs,Bs,Q,Vs,Hs,Us,Ws,Gs,Ks,qs,Js,Ys,Xs,Zs,Qs,$s,ec,tc,nc,rc,ic,ac,oc,sc,cc,lc,uc,dc,fc,pc,mc,hc,gc,_c,vc,yc,bc,xc,Sc,Cc,wc,Tc,Ec,Dc,Oc,kc,Ac,jc,Mc,Nc,Pc,Fc,Ic,Lc=e((()=>{Qi(),ra(),ct(),Xc(),ka(),be(),Xe(),Qa(),bo(),Ae(),Va(),Co(),Io(),K=null,fs=!1,q=!1,ps=e=>{try{let t=globalThis;t.__socket=e;let n=t.window;n&&(n.__socket=e)}catch{}},ms=null,hs=null,J=0,Y=new Set,gs=!1,_s=0,vs=null,ys=[],bs=0,xs=new Set,Ss=800,Cs=4800,ws=5600,Ts=3,Es=`CWS_AIRPAD_VERBOSE_QUERY`,Ds=8e3,Os=()=>{vs&&=(globalThis.clearTimeout(vs),null)},X=e=>{let t=e;t.__cwspProbeTimer&&(globalThis.clearTimeout(t.__cwspProbeTimer),delete t.__cwspProbeTimer)},Z={via:`cwsp_via`,localEndpoint:`cwsp_local_endpoint`,route:`cwsp_route`,routeTarget:`cwsp_route_target`,hop:`cwsp_hop`,host:`cwsp_host`,target:`cwsp_target`,targetPort:`cwsp_target_port`,viaPort:`cwsp_via_port`,protocol:`cwsp_protocol`},ks=()=>{try{let e=String(globalThis?.localStorage?.getItem?.(Es)||``).trim().toLowerCase();if([`1`,`true`,`yes`,`on`].includes(e))return!0}catch{}let e=String(globalThis?.[Es]||``).trim().toLowerCase();return[`1`,`true`,`yes`,`on`].includes(e)},As=new Set,js=``,Ms=new Set,Ns=new Set,Ps=`ws`,Fs=`ws`,Is=e=>{let t=String(e||``).trim().toLowerCase();return!t||t===`ws`||t===`wss`||t===`socket`||t===`socket.io`||t===`socketio`?Ps:t},Ls=new TextEncoder,Rs=new TextDecoder,zs=new Map,Bs=new Map,xo(()=>{zs.clear(),Bs.clear()}),Q=new Map,Vs=[],Hs=128,Us=()=>{if(K?.connected)for(;Vs.length>0;){let e=Vs.shift();e&&Tc(e)}},Ws=e=>{let t=String(e||``).trim().toLowerCase();return t===`mouse:move`||t===`mouse:scroll`},Gs=async(e=7e3)=>B()?Oo()||await Eo():K?.connected?!0:(G(),await new Promise(t=>{let n=!1,r=e=>{if(!n){n=!0;try{i?.()}catch{}globalThis.clearTimeout(a),t(e)}},i=Bo(e=>{e&&r(!0)}),a=globalThis.setTimeout(()=>r(!!K?.connected),e)})),Ks=3500,qs=``,Js=0,Ys=``,Xs=0,Zs=``,Qs=0,$s=null,ec=()=>{$s&&=(globalThis.clearInterval($s),null)},tc=()=>{if(ec(),!Re()||!Le())return;let e=Je();$s=globalThis.setInterval(()=>{qo()},e)},nc=e=>{if(typeof e==`string`)return e;if(!e||typeof e!=`object`)return``;let t=e;for(let e of[`text`,`content`,`body`]){let n=t[e];if(typeof n==`string`)return n}if(typeof t.result==`string`)return t.result;let n=t.payload??t.data;if(n&&n!==e){let e=nc(n);if(e)return e}return``},rc=e=>{let t=String(e||``).trim().toLowerCase();return t===`clipboard:update`||t===`clipboard:write`||t.startsWith(`airpad:clipboard:`)},ic=e=>{let t=e.payload??e.data??e.result??e.results;return nc(t)||nc(e)},ac=e=>{let t=[e.payload,e.data,e.result,e.results,e];for(let e of t){if(!e||typeof e!=`object`)continue;let t=e,n=t.asset??t.dataAsset??t.file??t.image;if(!n||typeof n!=`object`)continue;let r=n,i=typeof r.data==`string`?r.data.trim():``;if(!i)continue;let a=typeof r.mimeType==`string`&&r.mimeType.trim()||typeof r.type==`string`&&r.type.trim()||`image/png`;if(a.toLowerCase().startsWith(`image/`))return{hash:typeof r.hash==`string`?r.hash.trim():``,mimeType:a,data:i}}return null},oc=e=>{let t=e;return!t||typeof t!=`object`?``:String(t.from||t.byId||t.sender||``).trim()},sc=e=>{let t=String(e||``).trim().toLowerCase();return t.startsWith(`clipboard:`)?`clipboard`:t.startsWith(`mouse:`)?`mouse`:t.startsWith(`keyboard:`)?`input`:t.startsWith(`airpad:`)?`airpad`:t.startsWith(`sms:`)?`sms`:t.startsWith(`contacts:`)?`contact`:(t.startsWith(`notification:`)||t.startsWith(`notifications:`),`general`)},cc=e=>e?typeof e==`string`?e:e instanceof Error?`${e.name}: ${e.message}`:Xo(e):String(e),lc=e=>{try{let t=atob(e),n=new Uint8Array(t.length);for(let e=0;e<t.length;e+=1)n[e]=t.charCodeAt(e);return n}catch{return null}},uc=e=>typeof e==`object`&&!!e&&typeof e.cipher==`string`&&typeof e.sig==`string`,dc=e=>{if(!e||typeof e!=`string`)return null;try{return JSON.parse(e)}catch{return null}},fc=e=>!e||!(e===`io client disconnect`||e===`forced close`),pc=e=>!e||!(e===`io server disconnect`||e===`io client disconnect`),mc=()=>(Ge()||``).trim(),hc=()=>xe((Ye()||``).trim())||`airpad-client`,gc=()=>(Ke()||``).trim(),_c=()=>(tt()||``).trim(),vc=()=>wireTargetNodeIds(parseWireTargetList(at().trim())),yc=()=>globalThis.crypto?.randomUUID?globalThis.crypto.randomUUID():`airpad-${Date.now()}-${Math.random().toString(16).slice(2)}`,bc=e=>!!e&&typeof e==`object`&&(`op`in e||`what`in e||`uuid`in e||`result`in e||`error`in e),xc=e=>e===`request`?`ask`:e===`response`?`result`:e===`signal`||e===`notify`||e===`redirect`?`act`:e,Sc=e=>e,Cc=e=>{let t=hc(),n=gc(),r=(typeof e.accessToken==`string`&&e.accessToken.trim()?e.accessToken.trim():typeof e.airpadToken==`string`&&e.airpadToken.trim()?e.airpadToken.trim():``)||_c(),i=String(e.sender||e.byId||e.from||t||``).trim()||void 0,a=String(e.from||i||``).trim()||void 0,o=String(e.byId||i||``).trim()||void 0,s=Array.isArray(e.destinations)&&e.destinations.length?e.destinations:Array.isArray(e.nodes)?e.nodes:vc(),c=typeof e.uuid==`string`&&e.uuid.trim()?e.uuid.trim():yc(),l=Date.now();return{...e,op:Sc(e.op),type:String(e.type||e.what||``).trim()||e.what,protocol:Is(e.protocol),transport:String(e.transport||Fs).trim()||Fs,purpose:String(e.purpose||sc(String(e.what||e.type||``))).trim()||`general`,sender:i,byId:o,from:a,nodes:s,destinations:s,ids:typeof e.ids==`object`&&e.ids!=null?e.ids:{byId:o,from:a,sender:i,destinations:s},urls:Array.isArray(e.urls)&&e.urls.length?e.urls:[qe()],tokens:Array.isArray(e.tokens)&&e.tokens.length?e.tokens:n?[n]:[],token:e.token||n||void 0,userKey:typeof e.userKey==`string`&&e.userKey.trim()?e.userKey:n||void 0,accessToken:r||void 0,flags:{...e.flags,canonicalV2:!0},uuid:c,timestamp:Number(e.timestamp||0)>0?Number(e.timestamp):l}},wc=async e=>{let t=xc(e.op),n=(e.what||e.type||``).trim(),r=typeof e.uuid==`string`?e.uuid:``;if(r&&Q.has(r)){let n=Q.get(r);n&&(clearTimeout(n.timeoutId),Q.delete(r),t===`error`||e.error!==void 0?n.reject(e.error??{ok:!1,error:`Unknown coordinator error`}):n.resolve(e.result??e.results));return}if(t===`ask`&&n===`clipboard:get`){try{let t=await ya();Tc({...Ec(`result`,n,null,{uuid:r,nodes:e.from?[e.from]:void 0}),result:typeof t==`string`?t:String(t||``)})}catch(t){Tc({...Ec(`error`,n,null,{uuid:r,nodes:e.from?[e.from]:void 0}),error:t?.message||String(t)})}return}if(t===`act`&&n){let t=rc(n)?`clipboard`:so(n);if(ho.shouldSuppress(e,t))return}if(rc(n)){if(!ze(oc(e)))return;let t=e.payload??e.data??e.result??e.results,n=ac(e);if(n){Yo(n,{source:typeof t==`object`&&t?String(t.source||``):void 0});return}Jo(ic(e),{source:typeof t==`object`&&t?String(t.source||``):void 0})}},Tc=e=>{if(B()){let t=String(e.what||e.type||``),n=e.payload??e.data??{},r=Array.isArray(e.nodes)?e.nodes.map(String):void 0;return Fo({op:e.op===`ask`||e.op===`request`?`ask`:`act`,what:t,payload:n,nodes:r,uuid:typeof e.uuid==`string`?e.uuid:void 0}),Oo()}return!K||!K.connected?!1:(K.send(Cc(e)),!0)},Ec=(e,t,n,r={})=>{let i=hc(),a=gc(),o=r.accessToken===void 0?_c():String(r.accessToken).trim()||_c();return po(za({op:Sc(e),what:t,type:t,purpose:sc(t),protocol:Ps,transport:Fs,payload:n,nodes:r.nodes??vc(),destinations:r.nodes??vc(),uuid:r.uuid,sender:i,byId:i,from:i,ids:{byId:i,from:i,sender:i,destinations:r.nodes??vc()},urls:[qe()],tokens:a?[a]:[],flags:{canonicalV2:!0},token:a||void 0,userKey:a||void 0,accessToken:o||void 0,timestamp:Date.now()}))},Dc=async e=>{if(!e||!globalThis.crypto?.subtle)return null;if(zs.has(e))return zs.get(e)||null;let t=Ls.encode(e),n=await globalThis.crypto.subtle.digest(`SHA-256`,t),r=await globalThis.crypto.subtle.importKey(`raw`,n,`AES-GCM`,!1,[`encrypt`,`decrypt`]);return zs.set(e,r),r},Oc=async e=>{if(!uc(e))return e;let t=mc(),n=lc(e.cipher);if(!n)return e;if(!t||!globalThis.crypto?.subtle){let t=Rs.decode(n);return dc(t)??e}let r=await Dc(t);if(!r)return e;if(n.length<28){let t=Rs.decode(n);return dc(t)??e}let i=n.slice(0,12),a=n.slice(12);try{let t=new Uint8Array(await globalThis.crypto.subtle.decrypt({name:`AES-GCM`,iv:i},r,a)),n=Rs.decode(t);return dc(n)??e}catch{return e}},kc=async e=>!uc(e)||Zo()!==`secure`?e:Oc(e),Ac=()=>{try{return String(new URL(location.href).hostname).toLowerCase()}catch{return``}},jc=e=>{if(!e||typeof e!=`string`)return!1;let t;try{t=new URL(e,location.href)}catch{return!1}let n=t.hostname.toLowerCase(),r=t.protocol.toLowerCase();if(r!==`http:`&&r!==`https:`)return!1;let i=Ac();return $o(n)||n===`localhost`||n===i},Mc=e=>{let t={};if(!e)return t;for(let[n,r]of Object.entries(e))typeof n!=`string`||!n.trim()||typeof r==`string`&&(t[n]=r);return t},Nc=e=>{let t={};return e.forEach((e,n)=>{t[n]=e}),t},Pc=async e=>{let t=typeof e?.requestId==`string`?e.requestId.trim():``,n=typeof e?.method==`string`?e.method.toUpperCase():`GET`,r=typeof e?.url==`string`?e.url:``,i=e&&typeof e.timeoutMs==`number`?e.timeoutMs:12e3,a=Number.isFinite(i)&&i>0?Math.min(Math.max(Math.round(i),1e3),6e4):12e3;if(!t)return{ok:!1,status:400,statusText:`Bad Request`,error:`Missing requestId`};if(!jc(r))return{requestId:t,ok:!1,status:400,statusText:`Bad Request`,error:`URL not allowed`};let o=new AbortController,s=globalThis.setTimeout(()=>o.abort(),a);try{let i=Mc(e?.headers),a=![`GET`,`HEAD`].includes(n),s=e?.body,c=a?typeof s==`string`?s:Xo(s):void 0,l=await fetch(r,{method:n,headers:i,body:c,signal:o.signal}),u=await l.text();return{requestId:t,ok:l.ok,status:l.status,statusText:l.statusText,headers:Nc(l.headers),body:u}}catch(e){return{requestId:t,ok:!1,status:0,statusText:`Network Error`,error:cc(e)}}finally{clearTimeout(s)}},Fc=(e,t)=>!Xa(e)||!t||typeof t!=`object`||Array.isArray(t)?t:Za(t),Ic=`ws-status-tls-hint`})),Rc=t({applyHubSocketFromSettings:()=>Wc,backendOwnsExclusiveHubWebsocket:()=>Vc,installAirpadHubLifecycleRecovery:()=>Uc,nativeShellOwnsExclusiveHubWebsocket:()=>zc,nodeClipboardHubOwnsExclusiveWebsocket:()=>Bc});function zc(){if(!Be())return!1;try{if(globalThis.__CWS_NATIVE__===!0)return!0}catch{}return Yc()}function Bc(){return ot()}function Vc(){return zc()||Bc()}function Hc(){return!(Vc()||!Ie()&&!st()||!qe().trim())}function Uc(){if(Kc||!Jc())return;Kc=!0;let e=globalThis.document,t=globalThis.window;e.addEventListener(`visibilitychange`,()=>{e.visibilityState===`hidden`&&(qc=Date.now())});let n=e=>{globalThis.setTimeout(e,280)},r=()=>{Hc()&&(async()=>{us(null);let e=!!Ro()?.connected;if(qc>0&&Date.now()-qc>=Gc&&(e||zo())){cs(`visibility`);return}!e&&!zo()&&G()})()},i=e=>{Hc()&&(us(null),cs(e))};e.addEventListener(`visibilitychange`,()=>{e.visibilityState===`visible`&&n(r)}),t.addEventListener(`online`,()=>n(()=>i(`online`))),t.addEventListener(`pageshow`,e=>{e.persisted&&n(()=>i(`bfcache`))})}async function Wc(e){Uc(),!await lt(e)&&(rt(e),!zc()&&(Bc()||!Ie()&&!st()||qe().trim()&&(us(null),G())))}var Gc,Kc,qc,Jc,Yc,Xc=e((()=>{k(),ct(),Lc(),Gc=12e3,Kc=!1,qc=0,Jc=()=>{try{let e=globalThis;return!!(e.window&&e.document)}catch{return!1}},Yc=()=>{try{let e=globalThis.Capacitor;return typeof e?.isNativePlatform==`function`&&!!e.isNativePlatform()}catch{return!1}}})),Zc=t({ensureCapacitorPermissions:()=>rl,isCapacitorNative:()=>$c}),Qc,$c,el,tl,nl,rl,il=e((()=>{Qc=()=>{try{let e=globalThis?.Capacitor;return e&&typeof e==`object`?e:null}catch{return null}},$c=()=>{let e=Qc();try{return!!(e?.isNativePlatform?.()??(e?.platform&&e.platform!==`web`))}catch{return!1}},el=e=>{let t=Qc()?.Plugins?.[e];return t&&typeof t==`object`?t:null},tl=async(e,...t)=>{try{return typeof e==`function`?await e(...t):void 0}catch{return}},nl=!1,rl=async()=>{if(!$c())return{native:!1,requested:[]};if(nl)return{native:!0,requested:[]};nl=!0;let e=[],t=el(`Clipboard`);t&&(await tl(t.read),e.push(`clipboard`));let n=el(`CwsPlatform`);if(n)await tl(n.requestRuntimePermissions),e.push(`CwsPlatform.requestRuntimePermissions`);else{let t=el(`DevicePermissions`)||el(`Permissions`);t&&typeof t.requestPermissions==`function`&&(await tl(t.requestPermissions,{permissions:[`POST_NOTIFICATIONS`]}),e.push(`legacy-permissions`))}let r=el(`LocalNotifications`);return r&&typeof r.requestPermissions==`function`&&(await tl(r.requestPermissions),e.push(`notifications`)),{native:!0,requested:e}}})),al,ol,sl,cl,ll,ul=e((()=>{il(),al=()=>{try{let e=globalThis?.Capacitor;return e&&typeof e==`object`?e:null}catch{return null}},ol=e=>{let t=al()?.Plugins?.[e];return t&&typeof t==`object`?t:null},sl=async(e,...t)=>{try{return typeof e==`function`?await e(...t):void 0}catch(e){console.warn(`[capacitor-settings-permissions]`,e);return}},cl=async e=>{let t=[],n=[],r=!1;if(!$c())return{lines:t,results:n,prompted:r};e.shell&&(e.shell.acceptSmsBridgeData=!1,e.shell.enableNativeSms=!1);let i=e.shell||{},a=i.acceptContactsBridgeData===!0,o=(i.bridgeDaemonEnabled??!0)!==!1,s=(i.enableRemoteClipboardBridge??!0)!==!1,c=o||s,l=ol(`CwsPlatform`);if(a||c)if(l?.requestSettingsPermissions){let e=await sl(l.requestSettingsPermissions,{contacts:a,sms:!1,notifications:c,overlay:!1}),i=!1;if(e&&typeof e==`object`){i=e.prompted===!0,r=i;let t=e.results;if(Array.isArray(t)){for(let e of t)if(e&&typeof e==`object`){let t=String(e.permission??``);if(t===`SYSTEM_ALERT_WINDOW`||t===`READ_SMS`||t===`RECEIVE_SMS`||t===`SEND_SMS`)continue;n.push({permission:t,granted:!!e.granted})}}}let o=n.filter(e=>e.granted===!1);o.length?t.push(`Permission denied: ${o.map(e=>e.permission).filter(Boolean).join(`, `)}`):i&&t.push(`Runtime permissions requested`)}else{let e=ol(`DevicePermissions`)||ol(`Permissions`),n=[];a&&n.push(`READ_CONTACTS`),c&&n.push(`POST_NOTIFICATIONS`),e?.requestPermissions&&n.length&&(await sl(e.requestPermissions,{permissions:n}),t.push(`Runtime permissions requested (legacy plugin)`))}return o&&l?.startCwspBridge?(await sl(l.startCwspBridge),t.push(`CWSP foreground service started`)):!o&&l?.stopCwspBridge&&(await sl(l.stopCwspBridge),t.push(`CWSP foreground service stopped`)),{lines:t,results:n,prompted:r}},ll=async e=>{if(!$c()||((e?.shell||{}).bridgeDaemonEnabled??!0)===!1)return!1;e?.shell&&(e.shell.acceptSmsBridgeData=!1,e.shell.enableNativeSms=!1);let t=ol(`CwsPlatform`);return t?.startCwspBridge?(await sl(t.startCwspBridge),!0):!1}})),dl=t({BootLoader:()=>hl,bootLoader:()=>gl,bootMinimal:()=>fl,default:()=>gl});async function fl(e,t=`viewer`,n){let r=mr(t,`viewer`),i=pr(r)?[r]:[`viewer`],a=i[0];return gl.boot(e,{styleSystem:`vl-basic`,shell:`minimal`,defaultView:r,channels:i,channelPriorityId:a,rememberChoice:n?.rememberChoice??!0,skipInitialNavigate:n?.skipInitialNavigate??!1})}var pl,ml,hl,gl,_l=e((()=>{c(),mt(),yt(),Er(),jr(),de(),k(),Fe(),Ii(),qn(),qi(),hr(),Xc(),ul(),_(),pl=e=>e===`faint`?`tabbed`:e===`base`?`immersive`:e,ml={raw:{name:`Raw (No Framework)`,stylesheets:[],description:`No CSS framework, raw browser defaults`,recommendedShells:[`immersive`]},"vl-core":{name:`Core (Shared Foundation)`,stylesheets:[],description:`Shared foundation styles for all veela variants`,recommendedShells:[`immersive`,`minimal`]},"vl-basic":{name:`Basic Veela Styles`,stylesheets:[],description:`Minimal styling for basic functionality`,recommendedShells:[`window`,`tabbed`,`minimal`,`environment`,`immersive`,`content`]},"vl-advanced":{name:`Advanced (Full-Featured Styling)`,stylesheets:[],description:`Full-featured styling with design tokens and effects`,recommendedShells:[`tabbed`,`minimal`,`environment`]},"vl-beercss":{name:`BeerCSS (Beer CSS Compatible)`,stylesheets:[],description:`Beer CSS compatible styling with Material Design 3`,recommendedShells:[`tabbed`]}},hl=class e{static instance;state={phase:`idle`,styleSystem:null,shell:null,view:null,error:null};stateChangeHandlers=new Set;shellInstance=null;implicitBridgeCleanup=null;phaseHandlers=new Map;constructor(){yr()}static getInstance(){return e.instance||=new e,e.instance}async boot(e,t){console.log(`[BootLoader] Starting boot sequence:`,t);try{if(this.shellInstance)try{this.implicitBridgeCleanup?.(),this.implicitBridgeCleanup=null,xr.unload(this.shellInstance.id)}catch(e){console.warn(`[BootLoader] Failed to unload previous shell:`,e)}finally{this.shellInstance=null}Dr(),pe().catch(()=>{});try{let{initFrontendDebugCapture:e}=await v(async()=>{let{initFrontendDebugCapture:e}=await import(`./frontend-debug-capture-BNPCucC4.js`).then(e=>(e.i(),e.t));return{initFrontendDebugCapture:e}},[],import.meta.url);e()}catch{}let n=await ut().catch(e=>(console.warn(`[BootLoader] Failed to load settings:`,e),null)),r=n;if(fe()){let e=await A().catch(()=>null);e&&(r=e)}if(r&&Wc(r).catch(()=>void 0),fe()&&ll(r).catch(e=>{console.warn(`[BootLoader] CWSP bridge daemon auto-start skipped:`,e)}),ji(r??Pe),!(()=>{try{let e=globalThis,t=typeof document<`u`?String(document.documentElement?.dataset?.cwspSurface||``):``;return!!(e.__CWS_SKIP_PWA__||e.__CWS_NEUTRALINO_BOOT__||e.__CWS_WEBNATIVE_BOOT__||e.Neutralino||typeof e.NL_OS==`string`||t===`cwsp-control`||t===`gateway`)}catch{return!1}})())try{let{initIngressPWA:e}=await v(async()=>{let{initIngressPWA:e}=await import(`./sw-handling-D8qTyRtO.js`);return{initIngressPWA:e}},[],import.meta.url);await e()}catch(e){console.warn(`[BootLoader] Share-target / service worker ingress failed (non-fatal):`,e)}await this.loadStyles(t.styleSystem);let i=this.resolveThemeFromSettings(n),a=await this.loadShell(t.shell,e);return a.setTheme(t.theme||i),await a.mount(e),this.implicitBridgeCleanup?.(),this.implicitBridgeCleanup=Un(),t.channels&&t.channels.length>0&&await this.initChannels(t.channels,t.channelPriorityId),t.skipInitialNavigate?this.dismissShellLoadingSpinner(a):await a.navigate(t.defaultView),this.setPhase(`ready`),t.rememberChoice&&this.savePreferences(t),console.log(`[BootLoader] Boot complete`),a}catch(e){throw console.error(`[BootLoader] Boot failed:`,e),this.updateState({phase:`error`,error:e}),e}}resolveThemeFromSettings(e){let t=e?.appearance?.theme||`auto`;return t===`dark`?Tr:t===`light`?wr:Cr}dismissShellLoadingSpinner(e){try{let t=e.getElement().shadowRoot?.querySelector(`.app-shell__loading`);t&&(t.hidden=!0)}catch{}}async loadStyles(e){this.setPhase(`styles`),console.log(`[BootLoader] Loading style system: ${e}`);let t=ml[e]||ml[`vl-basic`];try{await Wi(e)}catch(t){throw console.error(`[BootLoader] Failed to load style system: ${e}`,t),t}for(let e of t.stylesheets)try{await a(e)}catch(t){console.warn(`[BootLoader] Failed to load stylesheet: ${e}`,t)}this.updateState({styleSystem:e}),console.log(`[BootLoader] Style system ${e} loaded`)}async loadShell(e,t){this.setPhase(`shell`);let n=pl(e);n!==e&&console.warn(`[BootLoader] Shell "${e}" is temporarily disabled, redirecting to "${n}"`),console.log(`[BootLoader] Loading shell: ${n}`);let r=await xr.load(n,t);return this.shellInstance=r,this.updateState({shell:n}),console.log(`[BootLoader] Shell ${n} loaded`),r}async initChannels(e,t){this.setPhase(`channels`);let n=[...new Set(e)];if(n.length===0)return;let r=(t&&n.includes(t)?t:null)??n[0],i=n.filter(e=>e!==r);console.log(`[BootLoader] Initializing primary channel:`,r,i.length?`(+${i.length} deferred)`:``);try{await vt.initChannel(r)}catch(e){console.warn(`[BootLoader] Failed to init primary channel ${r}:`,e)}if(i.length===0){console.log(`[BootLoader] Channels initialized`);return}let a=()=>{(async()=>{for(let e of i)try{await vt.initChannel(e)}catch(t){console.warn(`[BootLoader] Failed to init channel ${e}:`,t)}console.log(`[BootLoader] Deferred channels initialized:`,i)})()};typeof globalThis.requestIdleCallback==`function`?globalThis.requestIdleCallback(a,{timeout:5e3}):globalThis.setTimeout?.(a,0)}updateState(e){Object.assign(this.state,e),this.notifyStateChange()}setPhase(e){this.updateState({phase:e});let t=this.phaseHandlers.get(e);if(t)for(let e of t)try{e(this.state)}catch(e){console.error(`[BootLoader] Phase handler error:`,e)}}notifyStateChange(){for(let e of this.stateChangeHandlers)try{e(this.state)}catch(e){console.error(`[BootLoader] State handler error:`,e)}}onStateChange(e){return this.stateChangeHandlers.add(e),()=>{this.stateChangeHandlers.delete(e)}}onPhase(e,t){return this.phaseHandlers.has(e)||this.phaseHandlers.set(e,new Set),this.phaseHandlers.get(e).add(t),()=>{this.phaseHandlers.get(e)?.delete(t)}}getState(){return{...this.state}}getShell(){return this.shellInstance}savePreferences(e){try{let t=pl(e.shell);localStorage.setItem(`rs-boot-style`,e.styleSystem),localStorage.setItem(`rs-boot-shell`,t),localStorage.setItem(`rs-boot-view`,e.defaultView),localStorage.setItem(`rs-boot-remember`,`1`)}catch(e){console.warn(`[BootLoader] Failed to save preferences:`,e)}}loadPreferences(){try{if(localStorage.getItem(`rs-boot-remember`)!==`1`)return null;let e=pl(localStorage.getItem(`rs-boot-shell`)||`minimal`);return{styleSystem:localStorage.getItem(`rs-boot-style`)||void 0,shell:e,defaultView:localStorage.getItem(`rs-boot-view`)||void 0}}catch{return null}}clearPreferences(){try{localStorage.removeItem(`rs-boot-style`),localStorage.removeItem(`rs-boot-shell`),localStorage.removeItem(`rs-boot-view`),localStorage.removeItem(`rs-boot-remember`),localStorage.removeItem(pt)}catch{}}},gl=hl.getInstance()}));function vl(e){if(e&&typeof e.port==`number`)return e;try{let e=globalThis,t=e.__WEBNATIVE_AUTH__||e.__NEUTRALINO_AUTH__;if(t&&typeof t.port==`number`)return t}catch{}return null}function yl(e){if(!e||typeof e!=`object`)return{};let t=$(e.settings||e.portable),n=$(e.snapshot);if(!Object.keys(t).length&&!Object.keys(n).length)return{};let r=$(t.bridge||n.bridge||$(t.core).bridge),i=$(t.shell||n.shell),a=$(t.core),o=Array.isArray(r.endpoints)?r.endpoints.map(e=>String(e||``).trim()).filter(Boolean):[],s=String(a.endpointUrl||r.endpointUrl||i.remoteHost||o[0]||``).trim(),c=String(a.userId||r.userId||r.deviceId||``).trim(),l=String(a.ecosystemToken||a.userKey||r.userKey||i.accessToken||i.clientToken||``).trim(),u=r.allowInsecureTls===void 0?a.allowInsecureTls===void 0?void 0:!!a.allowInsecureTls:!!r.allowInsecureTls,d=$(a.socket),f={...a};return s&&(f.endpointUrl=s),c&&(f.userId=c),l?(f.userKey=l,f.ecosystemToken=l,f.socket={...d,accessToken:l}):Object.keys(d).length&&(f.socket=d),u!==void 0&&(f.allowInsecureTls=u),f.preferBackendSync===void 0&&(f.preferBackendSync=!0),{...t,core:f,shell:{...i}}}function bl(e){if(e&&typeof e.port==`number`)return e;try{let e=globalThis,t=e.__NEUTRALINO_AUTH__;if(t&&typeof t.port==`number`)return t;let n=e.NL_PORT,r=typeof n==`number`?n:n?Number(n):NaN;if(Number.isFinite(r)){let t=e.NL_KEY??e.NL_TOKEN;return{port:r,key:typeof t==`string`?t:void 0}}}catch{}return null}async function xl(e,t){try{let n=new Headers(t?.headers);n.set(`Content-Type`,`application/json`),e.key&&n.set(`X-API-Key`,e.key);let r=t?.signal??(typeof AbortSignal<`u`&&typeof AbortSignal.timeout==`function`?AbortSignal.timeout(2e3):void 0),i=await fetch(`http://127.0.0.1:${e.port}/service/config`,{...t,headers:n,cache:`no-store`,signal:r});return i.ok?await i.json():null}catch{return null}}async function Sl(e,t){try{let n=new Headers(t?.headers);n.set(`Content-Type`,`application/json`),e.key&&n.set(`X-API-Key`,e.key);let r=await fetch(`http://127.0.0.1:${e.port}/neutralino/config`,{...t,headers:n,cache:`no-store`});return r.ok?await r.json():null}catch{return null}}function Cl(e){let t=vl(e);return t?{get:async()=>yl(await xl(t,{method:`GET`})),patch:async e=>yl(await xl(t,{method:`POST`,body:JSON.stringify(e)})),defaults:async()=>(await xl(t,{method:`GET`}))?.defaults??{},snapshot:async()=>(await xl(t,{method:`GET`}))?.snapshot??{}}:null}function wl(e){let t=bl(e);return t?{get:async()=>{let[e,n]=await Promise.all([xl(t,{method:`GET`}),Sl(t,{method:`GET`})]),r=yl(e),i=n?.config??{};return{...r,neutralino:i}},patch:async e=>yl(await xl(t,{method:`POST`,body:JSON.stringify(e)})),defaults:async()=>(await xl(t,{method:`GET`}))?.defaults??{},snapshot:async()=>(await xl(t,{method:`GET`}))?.snapshot??{}}:null}function Tl(){try{globalThis.__CWS_NEUTRALINO_BOOT__=!0}catch{}}function El(){try{globalThis.__CWS_WEBNATIVE_BOOT__=!0}catch{}}var $,Dl=e((()=>{$=e=>typeof e==`object`&&e&&!Array.isArray(e)?e:{}}));function Ol(){let e=globalThis;return e.__CWS_WEBNATIVE_BOOT__||e.__CWS_NEUTRALINO_BOOT__?`webnative`:e.Capacitor===void 0?e.chrome!==void 0&&e.chrome?.runtime?`crx`:`web`:`capacitor`}function kl(e){Hl=e}function Al(e,t){Vl[e]=t}function jl(e){delete Vl[e]}function Ml(){for(let e of Object.keys(Vl))delete Vl[e]}function Nl(e,t){let n={...e};for(let[e,r]of Object.entries(t)){let t=n[e];typeof r==`object`&&r&&!Array.isArray(r)&&typeof t==`object`&&t&&!Array.isArray(t)?n[e]={...t,...r}:n[e]=r}return n}function Pl(e={},t={}){let n={...e};return{get:async()=>({...n}),patch:async e=>(n=Nl(n,e),{...n}),...t}}function Fl(){return Hl()}function Il(){let e=Hl();return Vl[e]||Vl.web||null}async function Ll(){let e=Il();if(!e)return{};try{return await e.get()}catch{return{}}}async function Rl(e){let t=Il();return t?t.patch(e):{}}async function zl(){let e=Il();if(!e?.defaults)return{};try{return await e.defaults()}catch{return{}}}async function Bl(){let e=Il();if(!e?.snapshot)return{};try{return await e.snapshot()}catch{return{}}}var Vl,Hl,Ul=e((()=>{Vl={},Hl=Ol}));function Wl(e){let t={port:e.port,key:e.key},n=globalThis;return n.__WEBNATIVE_AUTH__={port:e.port,key:e.key},n.__NEUTRALINO_AUTH__=t,n.__CWS_WEBNATIVE_BOOT__=!0,n.__CWS_NEUTRALINO_BOOT__=!0,n.__CWS_NODE_CLIPBOARD_HUB__=!0,t}async function Gl(e){try{let{getRemoteHost:t,getAccessToken:n,getAirPadClientId:r}=await v(async()=>{let{getRemoteHost:e,getAccessToken:t,getAirPadClientId:n}=await import(`./config-CZGbDUv1.js`).then(e=>(e.y(),e.n));return{getRemoteHost:e,getAccessToken:t,getAirPadClientId:n}},[],import.meta.url),i=t().trim(),a=n().trim(),o=r().trim(),s={};if(i&&(s.remoteHost=i),a&&(s.accessToken=a,s.clientToken=a),o&&(s.clientId=o),s.reload=!1,!Object.keys(s).filter(e=>e!==`reload`).length)return;let c=typeof AbortSignal<`u`&&typeof AbortSignal.timeout==`function`?AbortSignal.timeout(2e3):void 0;await fetch(`http://127.0.0.1:${e.port}/service/clipboard-hub`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-API-Key":e.key},body:JSON.stringify(s),cache:`no-store`,signal:c});try{let e=globalThis,t=typeof e.NL_PATH==`string`?e.NL_PATH:``,n=e.Neutralino?.filesystem?.writeFile;t&&n&&await n(`${t}/.tmp/cwsp-hub-auth.json`,JSON.stringify({remoteHost:i||void 0,accessToken:a||void 0,clientToken:a||void 0,clientId:o||void 0,writtenAt:new Date().toISOString()},null,2))}catch{}}catch(e){console.warn(`[CWSP Neutralino] clipboard-hub credential sync skipped`,e)}}function Kl(e){try{let t=wl(e);t&&Al(`webnative`,t)}catch(e){console.warn(`[CWSP] settings arm registration skipped`,e)}}function ql(){let e=bl();return e&&typeof e.port==`number`&&e.key&&e.port!==8434&&Number(e.port)>1024?{port:e.port,key:String(e.key)}:{port:Ql,key:$l}}async function Jl(){try{let e=globalThis,t=typeof e.NL_PATH==`string`?e.NL_PATH:``,n=e.Neutralino?.filesystem?.readFile;if(!t||!n)return null;let r=await n(`${t}/.tmp/cwsp-control-auth.json`),i=JSON.parse(r);if(typeof i.port==`number`&&typeof i.key==`string`)return{port:i.port,key:i.key}}catch{}return null}function Yl(e=15e3){let t=Date.now()+e;(async()=>{for(;Date.now()<t;){let e=await Jl();if(e){Kl(Wl(e));try{let t=await fetch(`http://127.0.0.1:${e.port}/service/config`,{headers:{"X-API-Key":e.key},cache:`no-store`});if(t.ok){console.log(`[CWSP Neutralino] control host ready`,e.port);try{let e=await t.json(),n=e.settings||e.portable||{},{syncAirpadRemoteConfigFromAppSettings:r}=await v(async()=>{let{syncAirpadRemoteConfigFromAppSettings:e}=await import(`./config-CZGbDUv1.js`).then(e=>(e.y(),e.n));return{syncAirpadRemoteConfigFromAppSettings:e}},[],import.meta.url);r(n,{persist:!0})}catch(e){console.warn(`[CWSP Neutralino] airpad hydrate from portable skipped`,e)}await Gl(e);return}}catch{}}await new Promise(e=>setTimeout(e,400))}console.warn(`[CWSP Neutralino] control host still warming — clipboard/settings will retry on use`)})()}async function Xl(){let e=ql();try{let t=await Promise.race([Jl(),new Promise(e=>setTimeout(()=>e(null),400))]);t&&(e=t)}catch{}let t=Wl(e);Kl(t);try{let e=Cl();e&&!t&&Al(`webnative`,e)}catch{}Yl(),await fl(document.body,`network`);try{document.getElementById(`cwsp-boot-fallback`)?.remove()}catch{}(async()=>{let e=await Jl()||t;Wl(e),await Gl(e)})()}var Zl,Ql,$l,eu=e((()=>{_l(),Dl(),Ul(),_(),Zl=[`minimal`,`network`,`settings`],Ql=29110,$l=`cwsp-neutralino-local`,document.documentElement.dataset.cwspEnabledViews=Zl.join(`,`),Tl(),El(),Xl().catch(e=>{console.error(`[CWSP Neutralino] minimal-shell boot failed`,e);try{let t=document.getElementById(`cwsp-boot-fallback`);t&&(t.textContent=`CWSP boot failed: ${e instanceof Error?e.message:String(e)}`)}catch{}})}));e((()=>{eu()}))();export{gn as $,la as A,Lr as B,G as C,zo as D,Lc as E,Pi as F,hr as G,Er as H,Mi as I,yn as J,pr as K,Ai as L,va as M,wi as N,Bo as O,ji as P,_n as Q,Rr as R,Xc as S,us as T,ir as U,N as V,fr as W,hn as X,On as Y,mn as Z,cl as _,Bl as a,en as at,$c as b,Nl as c,Pt as ct,Il as d,ft as dt,vn as et,kl as f,mt as ft,ul as g,_l as h,zl as i,tn as it,ka as j,Lo as k,Rl as l,yt as lt,dl as m,Pl as n,sn as nt,Ll as o,rn as ot,jl as p,mr as q,Fl as r,nn as rt,Ul as s,Xt as st,Ml as t,pn as tt,Al as u,vt as ut,Zc as v,ls as w,Rc as x,il as y,Fr as z};