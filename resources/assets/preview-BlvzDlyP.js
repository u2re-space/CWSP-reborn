import{n as e}from"./rolldown-runtime-DaJ6WEGw.js";import{Et as t,Tt as n,a as r}from"./HistoryManager-DVqS4qoS.js";import{d as i,u as a}from"./registry-DBV7KaK9.js";import{n as o,t as s}from"./preload-helper-NDuSAHbO.js";import{t as c}from"./src-yMca1J0Q.js";import{i as l,n as u,r as d,t as f}from"./shells-C8GzwuHo.js";import{t as p}from"./src-DUnEbC_E.js";var m,h=e((()=>{m=`/* Minimal shell: layered tokens + nav/content grid, WCO-safe, shadow slot views. */
/*
    ...
    .app-shell__status {
      display: none !important;
...
}`}));function g(e){return y.has(e)}function _(e){return new b}var v,y,b,x=e((()=>{c(),n(),h(),p(),u(),a(),l(),o(),v=[{id:`viewer`,name:`Markdown`,icon:`eye`},{id:`explorer`,name:`Explorer`,icon:`folder`},{id:`workcenter`,name:`Work Center`,icon:`lightning`},{id:`network`,name:`Network`,icon:`wifi-high`},{id:`settings`,name:`Settings`,icon:`gear`},{id:`history`,name:`History`,icon:`clock-counter-clockwise`}].filter(e=>i(e.id)),y=new Set(v.map(e=>e.id)),b=class extends f{id=`minimal`;name=`Minimal`;layout={hasSidebar:!1,hasToolbar:!0,hasTabs:!1,supportsMultiView:!1,supportsWindowing:!1};createLayout(){let e=r`
        ...
        `;return this.setupNavClickHandlers(e),this.setupConnectionSourceButton(e),this.setupAdminDoorButton(e),e}setupConnectionSourceButton(e){try{if(document.documentElement.dataset.cwspSurface!==`cwsp-control`)return}catch{return}let t=e.querySelector(`[data-shell-toolbar]`);if(!t||t.querySelector(`[data-connection-source]`))return;let n=r`
            ...
            >SRC</button>
        `;t.appendChild(n),n.addEventListener(`click`,()=>{try{window.dispatchEvent(new CustomEvent(`cwsp:open-connection-source`))}catch(e){console.warn(`[MinimalShell] connection source:`,e)}})}setupAdminDoorButton(e){let t=e.querySelector(`[data-shell-toolbar]`);if(!t||t.querySelector(`[data-admin-door]`))return;let n=r`
            <button
                type="button"
            ...
            >ADM</button>
        `;t.appendChild(n),n.addEventListener(`click`,()=>{s(async()=>{let{loadSettings:e}=await import(`./Settings-DP2WRv9J.js`).then(e=>(e.a(),e.t));return{loadSettings:e}},[],import.meta.url).then(({loadSettings:e})=>e()).then(e=>s(async()=>{let{openAdminDoorFromCore:e}=await import(`./admin-doors-BoU4RSfd.js`).then(e=>(e.n(),e.t));return{openAdminDoorFromCore:e}},[],import.meta.url).then(({openAdminDoorFromCore:t})=>{t(e.core,`https`)})).catch(e=>console.warn(`[MinimalShell] admin door:`,e))})}renderNavButtons(){let e=document.createDocumentFragment();for(let t of v){let n=r`
                <button
