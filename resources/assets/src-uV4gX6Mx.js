...
}`}));function ze(){if(pt?.isConnected)return pt;let e=de(),t=e.querySelector(`ui-quick-settings`);return t||(t=document.createElement(`ui-quick-settings`),t.hidden=!0,e.appendChild(t)),pt=t,t}function Be(e){be(M,()=>{let e=ze(),t=document.documentElement.getAttribute(`data-theme`);return(t===`light`||t===`dark`)&&(e.dataset.theme=t,e.style.colorScheme=t),fe(e,M),e.open(),{kind:M,el:e,close:()=>{e.close(),j(M)},contains:t=>t instanceof Node&&e.contains(t)}})}var Ve,M,He,Ue,We,Ge,Ke,qe,Je,Ye,Xe,Ze,Qe,$e,et,tt,nt,rt,it,at,ot,st,ct,lt,ut,dt,ft,pt,mt=e((()=>{v(),u(),D(),b(),xe(),Re(),w(),Ve=d(Le),M=`quick-settings`,He=`data-theme`,Ue=`rs-appearance-theme`,We=`appearance.theme`,Ge=[`rs-settings`,`cwsp-settings`,`u2-settings`],Ke=()=>{try{return matchMedia?.(`(prefers-color-scheme: dark)`)?.matches??!0}catch{return!0}},qe=e=>{for(let t of Ge)try{let n=localStorage.getItem(t);if(!n)continue;let r=JSON.parse(n);if(!r||typeof r!=`object`)continue;r.appearance={...r.appearance??{},theme:e},localStorage.setItem(t,JSON.stringify(r))}catch{}},Je=()=>{try{let e=document.documentElement.getAttribute(He);if(e===`light`||e===`dark`)return e;let t=localStorage.getItem(Ue);if(t===`light`||t===`dark`)return t}catch{}return Ke()?`dark`:`light`},Ye=e=>{let t=document.documentElement;t.setAttribute(`data-scheme`,e),t.setAttribute(He,e),t.style.colorScheme=e;try{document.body&&(document.body.style.colorScheme=e)}catch{}try{document.querySelectorAll(`.env-shell-root, [data-shell], ui-window`).forEach(t=>{let n=t;n.dataset.theme=e,n.style.colorScheme=e;let r=n.shadowRoot?.querySelector?.(`.app-shell`);r&&(r.dataset.theme=e,r.style.colorScheme=e)})}catch{}try{localStorage.setItem(Ue,e),localStorage.setItem(We,e)}catch{}qe(e),t.dispatchEvent(new CustomEvent(`u2-theme-change`,{bubbles:!0,detail:{source:`quick-settings`,theme:e}}))},Xe=e=>{document.documentElement.style.setProperty(`--orientation-lock`,e?`unlocked`:`locked`),document.documentElement.style.setProperty(`--orientation-lock-angle`,e?`0deg`:`90deg`),Promise.try(()=>{try{e?screen.orientation.unlock():screen.orientation.lock(screen.orientation.type||`natural`)}catch(e){console.warn(e)}})?.catch?.(console.warn.bind(console))},Ze=`env-night-filter`,Qe=`2147483001`,$e=`rs-night-filter`,et=`rs-brightness-filter`,tt=e=>Math.max(0,Math.min(100,Number.isFinite(e)?e:0)),nt=()=>{let e=document.getElementById(Ze);if(e instanceof HTMLElement)return e;let t=document.createElement(`div`);return t.id=Ze,t.setAttribute(`aria-hidden`,`true`),t.style.cssText=[`position:fixed`,`inset:0`,`pointer-events:none`,`z-index:${Qe}`,`background-color:rgb(255 140 60)`,`mix-blend-mode:multiply`,`opacity:0`,`visibility:hidden`,`transition:opacity 160ms ease`].join(`;`),(document.body??document.documentElement).appendChild(t),t},rt=e=>{let t=tt(e),n=nt(),r=t/100;n.style.opacity=String(r),n.style.visibility=r>0?`visible`:`hidden`;try{localStorage.setItem($e,String(t))}catch{}},it=e=>{let t=tt(e),n=nt(),r=t<=50?.4+t/50*.6:1+(t-50)/50*.2;n.style.filter=`brightness(${r.toFixed(3)})`;try{localStorage.setItem(et,String(t))}catch{}},at=(e,t)=>{try{let n=localStorage.getItem(e);if(n==null)return t;let r=Number(n);return Number.isFinite(r)?tt(r):t}catch{return t}},ot=()=>{let e=at($e,0),t=at(et,50);return rt(e),it(t),{night:e,brightness:t}},typeof document<`u`&&(document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,()=>ot(),{once:!0}):ot()),st=[`wifi`,`bluetooth`,`focus`,`airplane`,`orientation`],ct={light:`sun`,dark:`moon`},lt={light:`Light`,dark:`Dark`},ut=e=>{let t=e.querySelector(`[data-qs-tile="theme"]`);if(!t)return;let n=Je();t.querySelector(`ui-icon`)?.setAttribute(`icon`,ct[n]);let r=t.querySelector(`[data-qs-tile-sub]`);r&&(r.textContent=lt[n]),t.setAttribute(`aria-pressed`,n===`dark`?`true`:`false`)},dt=e=>{let t=e.shadowRoot,r=t?.querySelector(`.qs-panel`);if(!t||!r||r.hasAttribute(`data-qs-wired`))return;r.setAttribute(`data-qs-wired`,``),ut(t),t.querySelector(`[data-qs-tile="theme"]`)?.addEventListener(`click`,()=>{let e=Je()===`dark`?`light`:`dark`;Ye(e),ut(t)});let i=e=>!!e?.getAttribute?.(`aria-pressed`)&&e?.getAttribute?.(`aria-pressed`)===`true`;t.querySelector?.(`[data-qs-tile="orientation"]`)?.addEventListener?.(`click`,e=>{let t=n((e?.target?.matches?.(`[data-qs-tile="orientation"]`)?e?.target:e?.target?.querySelector?.(`[data-qs-tile="orientation"]`))||e?.target,`[data-qs-tile="orientation"]`),r=i(t);Xe(r);let a=t?.matches?.(`ui-icon`)?t:t?.querySelector?.(`ui-icon`);a&&a.setAttribute?.(`icon`,r?`device-rotate`:`lock`),a&&a.setAttribute?.(`icon-style`,`duotone`)});for(let e of st){let n=t.querySelector(`[data-qs-tile="${e}"]`);n&&n.addEventListener(`click`,()=>{let e=n.getAttribute(`aria-pressed`)!==`true`;n.setAttribute(`aria-pressed`,String(e));let t=n.querySelector(`[data-qs-tile-sub]`);t&&(t.textContent=e?`On`:`Off`)})}let{night:a,brightness:o}=ot(),s=t.querySelector(`[data-qs-slider="night"]`),c=t.querySelector(`[data-qs-slider="brightness"]`);s&&(s.value=String(a),s.addEventListener(`input`,()=>rt(s.valueAsNumber))),c&&(c.value=String(o),c.addEventListener(`input`,()=>it(c.valueAsNumber)))},ft=class extends T{constructor(){super()}styles=()=>Ve;render=()=>s`
...
</div>`;onRender(){return super.onRender(),dt(this),this}open(){ut(this.shadowRoot),this.removeAttribute(`hidden`),this.hidden=!1,this.setAttribute(`open`,``)}close(){this.hidden=!0,this.setAttribute(`hidden`,``),this.removeAttribute(`open`)}toggle(e){this.hasAttribute(`open`)?this.close():this.open()}},ft=C([g(`ui-quick-settings`)],ft),pt=null,Promise.try(()=>{typeof requestAnimationFrame==`function`&&requestAnimationFrame(()=>{Promise.try(()=>{screen?.orientation?.lock?.(`natural`)}).catch(console.warn.bind(console))})}).catch(console.warn.bind(console))})),ht,gt=e((()=>{ht=`/*
...
@layer ui-statusbar {
  ...
  env-shell-container[data-env-native-task] .env-shell-chrome[data-desktop] {
    ...
    </div>`,r=n.querySelector(`.env-status-bar__pct`);return r instanceof HTMLElement&&te(r,{properties:{textContent:e.batteryPct}}),n}function Et(e,t,n){let r=document.createElement(`ui-statusbar`);r.className=`env-ui-statusbar wf-chrome-no-select`,r.setAttribute(`part`,`status-bar`);let i=document.createElement(`div`);i.slot=`left`,i.className=`env-ui-statusbar__left`;let a=document.createElement(`time`);a.className=`env-ui-statusbar__clock`,a.dateTime=``,a.textContent=bt(),a.setAttribute(`role`,`button`),a.setAttribute(`tabindex`,`0`),a.setAttribute(`aria-label`,`Calendar`),a.setAttribute(`aria-haspopup`,`dialog`),a.setAttribute(`data-chrome-flyout-anchor`,`calendar`);let s=document.createElement(`div`);s.className=`env-ui-statusbar__intro`,t&&(s.innerHTML=t),i.append(a,s);let c=document.createElement(`div`);c.slot=`center`;let l=document.createElement(`p`);l.className=`env-status-bar__meta`,c.appendChild(l);let u=document.createElement(`div`);u.slot=`right`,u.className=`env-ui-statusbar__right`;let d=Tt(n,`env-device-tray env-device-tray--footer`);d.setAttribute(`role`,`button`),d.setAttribute(`tabindex`,`0`),d.setAttribute(`aria-label`,`Quick settings`),d.setAttribute(`aria-haspopup`,`dialog`),d.setAttribute(`data-chrome-flyout-anchor`,`quick-settings`),u.appendChild(d);let f=e=>{e.preventDefault(),e.stopPropagation(),ke(a)},p=e=>{e.preventDefault(),e.stopPropagation(),Be(d)};a.addEventListener(`click`,f),a.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&f(e)}),d.addEventListener(`click`,p),d.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&p(e)}),r.append(i,c,u),o(()=>{let t=e.navEcho.value?` │ ${e.navEcho.value}`:``;l.textContent=`doc=${e.selectedPath.value} │ viewer=${e.viewerStatus.value} │ layout=${e.mqLabel.value}${t}`},[e.selectedPath,e.viewerStatus,e.mqLabel,e.navEcho],{triggerImmediately:!0});let ee=()=>{let e=new Date;a.textContent=bt(e),a.dateTime=e.toISOString()};ee();let te=setInterval(ee,15e3);return{element:r,dispose:()=>{clearInterval(te)}}}var Dt,Ot,kt=e((()=>{v(),r(),Ie(),mt(),D(),gt(),u(),w(),Dt=d(ht),Ot=class extends E{constructor(){super()}styles=()=>Dt;render=()=>s`
<div style="background-color: transparent;" part="left"   class="left"  ><slot name="left"  ></slot></div>
        ...
        <div style="background-color: transparent;" part="right"  class="right" ><slot name="right" ></slot></div>`},Ot=C([g(`ui-statusbar`)],Ot)})),At=e((()=>{r(),ne(),v()}));function jt(){let e=document.documentElement,t=String(e.getAttribute(`data-theme`)||``).trim().toLowerCase();if(t===`light`||t===`dark`)return t;let n=String(e.getAttribute(`data-scheme`)||``).trim().toLowerCase();if(n===`light`||n===`dark`)return n;try{let e=String(localStorage.getItem(`rs-appearance-theme`)||``).trim().toLowerCase();if(e===`light`||e===`dark`)return e}catch{}return typeof matchMedia==`function`&&matchMedia(`(prefers-color-scheme: light)`).matches?`light`:`dark`}function Mt(e,t){e.style.setProperty(`position`,`fixed`,V),e.style.setProperty(`box-sizing`,`border-box`,V),e.style.setProperty(`min-width`,t?`188px`:`220px`,V),e.style.setProperty(`max-width`,`min(320px, calc(100vw - 24px))`,V),e.style.setProperty(`padding`,t?`0.3rem`:`0.4rem`,V),e.style.setProperty(`border-radius`,`14px`,V),e.style.setProperty(`pointer-events`,`auto`,V),e.style.setProperty(`-webkit-backdrop-filter`,`none`,V),e.style.setProperty(`backdrop-filter`,`none`,V),e.style.removeProperty(`border`),e.style.removeProperty(`background`),e.style.removeProperty(`color`),e.style.removeProperty(`outline`),e.style.removeProperty(`box-shadow`);let n=jt();e.dataset.theme=n,e.style.setProperty(`color-scheme`,n===`light`?`light only`:`dark only`,V)}function Nt(e){e.style.setProperty(`list-style`,`none`,V),e.style.setProperty(`list-style-type`,`none`,V),e.style.setProperty(`margin`,`0`,V),e.style.setProperty(`padding`,`0`,V),e.style.setProperty(`display`,`flex`,V),e.style.setProperty(`flex-direction`,`column`,V),e.style.setProperty(`align-items`,`stretch`,V),e.style.setProperty(`gap`,`0.2rem`,V),e.style.setProperty(`width`,`100%`,V),e.style.setProperty(`box-sizing`,`border-box`,V),e.style.setProperty(`text-align`,`left`,V)}function Pt(e){e.style.setProperty(`list-style`,`none`,V),e.style.setProperty(`list-style-type`,`none`,V),e.style.setProperty(`margin`,`0`,V),e.style.setProperty(`padding`,`0`,V),e.style.setProperty(`width`,`100%`,V),e.style.setProperty(`display`,`block`,V),e.style.setProperty(`box-sizing`,`border-box`,V)}function Ft(e,t){if(e.style.setProperty(`appearance`,`none`,V),e.style.setProperty(`-webkit-appearance`,`none`,V),e.style.setProperty(`box-sizing`,`border-box`,V),e.style.setProperty(`width`,`100%`,V),e.style.setProperty(`max-width`,`100%`,V),e.style.setProperty(`margin`,`0`,V),e.style.setProperty(`display`,`grid`,V),e.style.setProperty(`grid-template-columns`,`1.375rem minmax(0, 1fr) auto`,V),e.style.setProperty(`align-items`,`center`,V),e.style.setProperty(`justify-items`,`start`,V),e.style.setProperty(`gap`,`0.55rem`,V),e.style.setProperty(`border-style`,`none`,V),e.style.setProperty(`border-width`,`0`,V),e.style.setProperty(`outline`,`none`,V),e.style.setProperty(`border-radius`,`10px`,V),e.style.setProperty(`padding`,`0.5rem 0.6rem`,V),e.style.setProperty(`min-height`,`2.35rem`,V),e.style.setProperty(`font-family`,`inherit`,V),e.style.setProperty(`font-size`,`0.8125rem`,V),e.style.setProperty(`font-weight`,`400`,V),e.style.setProperty(`line-height`,`1.25`,V),e.style.setProperty(`text-align`,`start`,V),e.style.setProperty(`cursor`,`pointer`,V),e.style.removeProperty(`background`),e.style.removeProperty(`background-color`),e.style.removeProperty(`background-image`),e.style.setProperty(`box-shadow`,`none`,V),e.style.setProperty(`transition`,`none`,V),!t)e.style.setProperty(`color`,`inherit`,V);else{let t=jt()===`light`?`#9f1239`:`#fecaca`;e.style.setProperty(`color`,t,V),e.style.setProperty(`--cw-menu-fg`,t,V),e.style.setProperty(`--icon-color`,t,V)}}function It(e){if(typeof customElements<`u`&&typeof customElements.upgrade==`function`)try{customElements.upgrade(e)}catch{}for(let t of e.querySelectorAll(`ui-icon`)){let e=t;e.style.setProperty(`--icon-size`,`1.125rem`,V),e.style.setProperty(`--icon-padding`,`0px`,V),e.style.setProperty(`--icon-color`,`var(--cw-menu-fg)`,V),e.style.setProperty(`color`,`var(--cw-menu-fg)`,V),e.style.setProperty(`width`,`1.125rem`,V),e.style.setProperty(`height`,`1.125rem`,V),e.style.setProperty(`min-width`,`1.125rem`,V),e.style.setProperty(`min-height`,`1.125rem`,V),e.style.setProperty(`display`,`inline-grid`,V),typeof e.updateIcon==`function`&&e.updateIcon.call(t)}}function Lt(e,t){let n=String(t||``).trim();if(!n)return;let r=document.createElement(`ui-icon`);r.setAttribute(`icon`,n),r.setAttribute(`icon-style`,`duotone`),r.setAttribute(`size`,`18`),r.setAttribute(`aria-hidden`,`true`),r.style.setProperty(`--icon-size`,`1.125rem`,V),r.style.setProperty(`--icon-padding`,`0px`,V),r.style.setProperty(`--icon-color`,`var(--cw-menu-fg)`,V),r.style.setProperty(`color`,`var(--cw-menu-fg)`,V),r.style.setProperty(`width`,`1.125rem`,V),r.style.setProperty(`height`,`1.125rem`,V),e.append(r)}var Rt,zt,Bt,N,P,F,I,L,Vt,Ht,Ut,R,Wt,z,B,V,Gt,Kt,qt,Jt,H,Yt,Xt,Zt,U,Qt,$t=e((()=>{u(),b(),v(),At(),S(),Rt=`2147483640`,zt=320,Bt=220,N=0,P=null,F=null,I=[],L=new Map,Vt=()=>{for(let e of L.values())try{e.destroy()}catch{}L.clear()},Ht=e=>{L.get(e)?.destroy(),L.set(e,ae(e))},Ut=e=>{L.get(e)?.destroy(),L.delete(e)},R=new Map,Wt=new Map,z=new Map,B=new Map,typeof CSS<`u`&&(CSS.supports(`position-anchor: --cw-anchor-test`)||CSS.supports(`anchor-name: --cw-anchor-test`)),V=`important`,Gt=()=>{let e=document.getElementById(`cw-unified-context-menu-style`);e||(e=document.createElement(`style`),e.id=`cw-unified-context-menu-style`,document.head.appendChild(e)),e.textContent=`
    ...
    `},Kt=()=>{for(let e of I)try{e()}catch{}I=[]},qt=e=>{for(let[t,n]of Array.from(z.entries()))t>=e&&(clearTimeout(n),z.delete(t));for(let[t,n]of Array.from(B.entries()))t>=e&&(clearTimeout(n),B.delete(t))},Jt=(e,t,n)=>{e.style.left=`${t}px`,e.style.top=`${n}px`;let r=e.getBoundingClientRect(),i=Math.max(8,window.innerWidth-r.width-8),a=Math.max(8,window.innerHeight-r.height-8);e.style.left=`${Math.min(Math.max(8,t),i)}px`,e.style.top=`${Math.min(Math.max(8,n),a)}px`},H=e=>{qt(e);for(let[t,n]of Array.from(R.entries()))t>=e&&(Ut(n),n.remove(),R.delete(t),Wt.delete(t))},Yt=(e,t)=>{let n=t.getBoundingClientRect();Jt(e,Math.round(n.right+4),Math.round(n.top))},Xt=e=>{for(let[t,n]of Array.from(B.entries()))t>=e&&(clearTimeout(n),B.delete(t))},Zt=(e,t,n,r)=>{let i=document.createElement(`div`);i.className=`cw-context-menu${t?` cw-context-menu--compact`:``}`,i.setAttribute(`role`,`menu`),i.dataset.menuDepth=String(n),i.style.zIndex=String(n+1);let a=document.createElement(`ul`);a.className=`cw-context-menu__list`,Nt(a),i.appendChild(a);let o=(e,n,i)=>{if(r!==N||!F?.isConnected||!P?.isConnected||(H(i),!e.children?.length))return;let a=Zt(e.children,t,i,r);a.classList.add(`cw-context-menu--submenu`),P.appendChild(a),R.set(i,a),Wt.set(i,n),Yt(a,n),Ht(a)},s=(e,t,n)=>{let r=z.get(n);r&&clearTimeout(r),Xt(n);let i=setTimeout(()=>{z.delete(n),o(e,t,n)},zt);z.set(n,i)},c=e=>{let t=B.get(e);t&&clearTimeout(t);let n=setTimeout(()=>{B.delete(e),H(e)},Bt);B.set(e,n)};for(let t of e){let e=document.createElement(`button`);e.type=`button`,e.className=`cw-context-menu__item${t.danger?` cw-context-menu__item--danger`:``}`,e.setAttribute(`role`,`menuitem`),e.disabled=!!t.disabled;let i=!!t.children?.length,l=document.createElement(`span`);l.className=`cw-context-menu__icon`,t.icon&&Lt(l,t.icon);let u=document.createElement(`span`);u.className=`cw-context-menu__label`,u.textContent=t.label;let d=document.createElement(`span`);if(d.className=`cw-context-menu__chevron`,i&&Lt(d,`caret-right`),e.append(l,u,d),Ft(e,!!t.danger),i){let i=n+1;e.setAttribute(`aria-haspopup`,`menu`),e.addEventListener(`pointerenter`,()=>s(t,e,i)),e.addEventListener(`pointerleave`,()=>c(i)),e.addEventListener(`click`,n=>{if(n.preventDefault(),n.stopPropagation(),r!==N||!F?.isConnected)return;Xt(i);let a=R.get(i),s=Wt.get(i);if(a?.isConnected&&s===e){H(i);return}o(t,e,i)})}else e.addEventListener(`click`,async e=>{e.preventDefault(),e.stopPropagation(),!(r!==N||!F?.isConnected)&&(U(),!t.disabled&&await t.action())});let f=document.createElement(`li`);Pt(f),f.appendChild(e),a.appendChild(f)}return Mt(i,t),i.addEventListener(`pointerenter`,()=>Xt(n)),i.addEventListener(`pointerleave`,()=>{if(n>0){let e=B.get(n);e&&clearTimeout(e);let t=setTimeout(()=>{B.delete(n),H(n)},Bt);B.set(n,t)}}),i},U=()=>{Kt(),qt(0),H(1),R.clear(),Wt.clear(),Vt(),F?.remove(),F=null,P?.remove(),P=null,N+=1},Qt=e=>{let t=(e.items||[]).filter(e=>e&&e.id&&e.label);if(!t.length){U();return}Gt(),U();let n=N,r=e.resolveOverlayMountPoint?.(e.anchor??null)??se(e.anchor??null),i=document.createElement(`div`);i.className=`cw-context-menu-layer`,i.style.setProperty(`position`,`fixed`,V),i.style.setProperty(`inset`,`0`,V),i.style.setProperty(`z-index`,Rt,V),i.style.setProperty(`pointer-events`,`none`,V),i.style.setProperty(`backdrop-filter`,`none`,V),i.style.setProperty(`-webkit-backdrop-filter`,`none`,V),P=i,r.appendChild(i);let a=Zt(t,!!e.compact,0,n);F=a,i.appendChild(a),Jt(a,e.x,e.y),Ht(a);let o=()=>{n!==N||!a.isConnected||It(a)},s=typeof customElements<`u`&&customElements.whenDefined?customElements.whenDefined(`ui-icon`).then(o).catch(()=>{}):Promise.resolve();queueMicrotask(()=>{s.then(o),requestAnimationFrame(()=>{o(),requestAnimationFrame(o)})});let c=e=>{if(!P?.isConnected||!F)return!1;let t=typeof e.composedPath==`function`?e.composedPath():[],n=Array.isArray(t)&&t.length?t:[];for(let e of n)if(e instanceof Element&&(e===P||e===F||P.contains(e)||e.classList?.contains?.(`cw-context-menu`)||e.closest?.(`.cw-context-menu`)))return!0;let r=e.target;return!!(r instanceof Node&&P.contains(r)||r instanceof Element&&r.closest?.(`.cw-context-menu`))},l=e=>{n!==N||!P?.isConnected||c(e)||U()},u=e=>{if(n!==N||!F?.isConnected)return;let t=e.target;if(!t)return;let r=t.closest?.(`.cw-context-menu__item`);if(!r&&typeof e.composedPath==`function`){for(let t of e.composedPath())if(t instanceof Element&&t.classList?.contains?.(`cw-context-menu__item`)){r=t;break}}if(!r){H(1);return}r.getAttribute(`aria-haspopup`)!==`menu`&&H(1)},d=e=>{n===N&&e.key===`Escape`&&U()},f=()=>U();queueMicrotask(()=>{n===N&&(document.addEventListener(`pointerdown`,l,{capture:!0}),document.addEventListener(`contextmenu`,l,{capture:!0}),document.addEventListener(`keydown`,d),a.addEventListener(`click`,u,{capture:!0}),window.addEventListener(`resize`,f,{passive:!0}),window.addEventListener(`blur`,f,{passive:!0}),I.push(()=>document.removeEventListener(`pointerdown`,l,{capture:!0})),I.push(()=>document.removeEventListener(`contextmenu`,l,{capture:!0})),I.push(()=>document.removeEventListener(`keydown`,d)),I.push(()=>a.removeEventListener(`click`,u,{capture:!0})),I.push(()=>window.removeEventListener(`resize`,f)),I.push(()=>window.removeEventListener(`blur`,f)))})}})),en,tn=e((()=>{en=`/*
...
}`}));function nn(e){return`${dn}${String(e||``).trim().toLowerCase()}`}function rn(){let e=document.querySelector(`.env-shell-chrome`);return e instanceof HTMLElement&&e.hasAttribute(`data-desktop`)?!1:e instanceof HTMLElement&&e.dataset.chromeLayout===`mobile`||typeof matchMedia==`function`&&matchMedia(`(max-width: 640px)`).matches}function an(e=new Date){return{time:e.toLocaleTimeString(void 0,{hour:`2-digit`,minute:`2-digit`}),date:e.toLocaleDateString(void 0,{weekday:`short`,day:`numeric`,month:`short`})}}function on(e){let t=ee([]);oe(t),re(ln,t,{title:`Home`,icon:`house-line`},{},function(){for(let e of t)e!==this&&(e.active=!1);this.active=!0,e.focusedTaskId.value=`home`,e.onHome()});let n=document.createElement(`ui-taskbar`);n.className=`env-shell-taskbar wf-chrome-no-select`,n.setAttribute(`part`,`taskbar`),n.setAttribute(`data-type`,`desktop`);let r=document.createElement(`div`);r.className=`env-shell-taskbar__pins`;let i=document.createElement(`div`);i.className=`env-shell-taskbar__windows`;let a=document.createElement(`ui-task`);a.setAttribute(`title`,`Home`),a.setAttribute(`icon`,`house-line`),a.setAttribute(`data-id`,ln),a.setAttribute(`data-env-home`,``),a.setAttribute(`aria-label`,`Home`),a.setAttribute(`aria-haspopup`,`menu`),a.setAttribute(`aria-keyshortcuts`,`LongPress`),r.append(a);let s=document.createElement(`div`);s.className=`env-shell-taskbar__tray-host`;let c=document.createElement(`div`);c.className=`env-shell-taskbar__clock`,c.setAttribute(`role`,`button`),c.setAttribute(`tabindex`,`0`),c.setAttribute(`aria-label`,`Calendar`),c.setAttribute(`aria-haspopup`,`dialog`),c.setAttribute(`data-chrome-flyout-anchor`,`calendar`);let l=document.createElement(`span`);l.className=`env-shell-taskbar__clock-time`;let u=document.createElement(`span`);u.className=`env-shell-taskbar__clock-date`,c.append(l,u);let d=()=>{let{time:e,date:t}=an();l.textContent=e,u.textContent=t,c.title=`${e} · ${t}`};d();let f=setInterval(d,pn),p=Tt(e.device,`env-device-tray env-device-tray--taskbar`);p.setAttribute(`role`,`button`),p.setAttribute(`tabindex`,`0`),p.setAttribute(`aria-label`,`Quick settings`),p.setAttribute(`aria-haspopup`,`dialog`),p.setAttribute(`data-chrome-flyout-anchor`,`quick-settings`);let te=e=>{e.preventDefault(),e.stopPropagation(),ke(c)},ne=e=>{e.preventDefault(),e.stopPropagation(),Be(p)};c.addEventListener(`click`,te),c.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&te(e)}),p.addEventListener(`click`,ne),p.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&ne(e)}),s.append(p,c);let m=document.createElement(`div`);m.className=`env-shell-navbar__switcher`,m.setAttribute(`role`,`menu`),m.setAttribute(`aria-label`,`Open apps`),m.hidden=!0;let h=document.createElement(`ul`);h.className=`env-shell-navbar__switcher-list`,m.appendChild(h),n.append(r,i,s,m);let g=new Map,_=[],v=null,y=!1,b=!1,x=null,S=[];S.push(()=>clearInterval(f));let se=e=>_.find(t=>String(t.id||``).trim().toLowerCase()===e),C=t=>{let n=String(t||``).trim().toLowerCase();if(!n)return;let r=se(n),i=String(e.focusedTaskId.value||``).trim().toLowerCase(),a=!!r?.focused||i===n||i===`markdown`&&n===`viewer`||i===`viewer`&&(n===`viewer`||n===`markdown`);if(r?.minimized){r.minimized=!1,r.focused=!0,g.get(n)?.toggleAttribute(`data-minimized`,!1),e.focusedTaskId.value=n===`markdown`?`viewer`:n,e.onWindowTask?.(n);return}if(a&&r&&r.visible!==!1){r.minimized=!0,r.focused=!1,g.get(n)?.toggleAttribute(`data-minimized`,!0),e.onMinimizeWindow?.(n);return}e.focusedTaskId.value=n===`markdown`?`viewer`:n,e.onWindowTask?.(n)},w=(t,r,i)=>{if(rn())return;t.preventDefault(),t.stopPropagation();let a=String(r||``).trim().toLowerCase(),o=!!se(a)?.minimized,s=[{id:o?`restore`:`minimize`,label:o?`Restore`:`Minimize`,icon:o?`arrow-square-out`:`minus`,action:()=>{o?(e.focusedTaskId.value=a,e.onWindowTask?.(a)):e.onMinimizeWindow?.(a)}},{id:`close`,label:`Close`,icon:`x`,danger:!0,action:()=>e.onCloseWindow?.(a)}];Qt({x:t.clientX,y:t.clientY,compact:!0,anchor:t.target instanceof Element?t.target:n,items:s})};n.addEventListener(`contextmenu`,t=>{if(rn())return;let r=typeof t.composedPath==`function`?t.composedPath():[];for(let e of r)if(e instanceof Element&&e.closest?.(`ui-task`))return;t.preventDefault(),t.stopPropagation(),Qt({x:t.clientX,y:t.clientY,compact:!0,anchor:n,items:[{id:`show-desktop`,label:`Show desktop`,icon:`desktop`,action:()=>e.onHome()},{id:`home`,label:`Home`,icon:`house-line`,action:()=>e.onHome()}]})});let T=()=>{b=!1,m.hidden=!0,h.replaceChildren(),n.removeAttribute(`data-switcher-open`)},E=()=>{let r=_.filter(e=>String(e.id||``).trim());if(h.replaceChildren(),r.length)for(let n of r){let r=String(n.id||``).trim().toLowerCase(),i=document.createElement(`li`);i.className=`env-shell-navbar__switcher-row`,i.setAttribute(`role`,`none`);let a=document.createElement(`button`);a.type=`button`,a.className=`env-shell-navbar__switcher-item`,a.setAttribute(`role`,`menuitem`),a.toggleAttribute(`data-active`,!!n.focused&&!n.minimized),a.toggleAttribute(`data-minimized`,!!n.minimized);let o=document.createElement(`ui-icon`);o.setAttribute(`icon`,n.icon||`app-window`),o.setAttribute(`icon-style`,`duotone`),o.setAttribute(`aria-hidden`,`true`);let s=document.createElement(`span`);s.className=`env-shell-navbar__switcher-label`,s.textContent=n.title||r,a.append(o,s),a.addEventListener(`click`,n=>{n.preventDefault(),n.stopPropagation(),T(),e.focusedTaskId.value=r;let i=nn(r),a=ie(t,i);a?a.focus=!0:e.onWindowTask?.(r)});let c=document.createElement(`button`);c.type=`button`,c.className=`env-shell-navbar__switcher-close`,c.setAttribute(`aria-label`,`Close ${n.title||r}`),c.title=`Close`;let l=document.createElement(`ui-icon`);l.setAttribute(`icon`,`x`),l.setAttribute(`icon-style`,`bold`),l.setAttribute(`aria-hidden`,`true`),c.appendChild(l),c.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),e.onCloseWindow?.(r),_=_.filter(e=>String(e.id||``).trim().toLowerCase()!==r),g.get(r)?.remove(),g.delete(r),_.length?E():T()}),i.append(a,c),h.appendChild(i)}else{let e=document.createElement(`li`);e.className=`env-shell-navbar__switcher-empty`,e.textContent=`No open apps`,h.appendChild(e)}b=!0,m.hidden=!1,n.setAttribute(`data-switcher-open`,``)},D=()=>{v!=null&&(clearTimeout(v),v=null)},ce=()=>{T(),ie(t,ln).focus=!0};a.addEventListener(`click`,e=>{if(y){e.preventDefault(),e.stopPropagation(),y=!1;return}ce()}),a.addEventListener(`pointerdown`,e=>{if(rn()&&!(e.button!=null&&e.button!==0)){y=!1,D(),v=setTimeout(()=>{v=null,y=!0;try{a.releasePointerCapture?.(e.pointerId)}catch{}E()},fn);try{a.setPointerCapture?.(e.pointerId)}catch{}}},{capture:!0});let O=()=>{D()};a.addEventListener(`pointerup`,O,{capture:!0}),a.addEventListener(`pointercancel`,O,{capture:!0}),a.addEventListener(`contextmenu`,e=>{rn()&&(e.preventDefault(),y=!0,D(),E())});let k=e=>{if(!b)return;let t=typeof e.composedPath==`function`?e.composedPath():[];for(let e of t)if(e===m||e===a||e instanceof Element&&(e===m||m.contains(e)||e===a))return;T()};document.addEventListener(`pointerdown`,k,{capture:!0}),S.push(()=>document.removeEventListener(`pointerdown`,k,{capture:!0}));let A=()=>{let t=String(e.focusedTaskId.value||`home`),n=(e,t)=>{e.toggleAttribute(`data-env-active`,t),e.toggleAttribute(`data-active`,t),e.toggleAttribute(`data-focus`,t)};n(a,t===`home`);for(let[e,r]of g)n(r,t===e)};o(()=>{A()},[e.focusedTaskId],{triggerImmediately:!0});let le=e=>{let n=String(e.id||``).trim().toLowerCase();if(!n||n===`home`)return;let r=nn(n),a=e.title||n,o=String(e.icon||``).trim()||`app-window`,s=g.get(n);if(!s){let e=re(r,null,{title:a,icon:o},{viewId:n},function(){for(let e of t)e!==this&&(e.active=!1);this.active=!0,C(n)});e.list=t,t.push(e),s=document.createElement(`ui-task`),s.setAttribute(`data-id`,r),s.setAttribute(`data-view`,n),s.setAttribute(`title`,a),s.setAttribute(`aria-label`,a),s.setAttribute(`icon`,o),s.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),C(n)}),s.addEventListener(`contextmenu`,e=>{w(e,n,a)}),g.set(n,s),i.appendChild(s)}s.setAttribute(`title`,a),s.setAttribute(`aria-label`,a),s.setAttribute(`icon`,o),s.toggleAttribute(`data-minimized`,!!e.minimized),s.hidden=e.visible===!1},ue=n=>{_=Array.isArray(n)?n.slice():[];let r=new Set;for(let t of n){let n=String(t.id||``).trim().toLowerCase();!n||n===`home`||(r.add(n),le(t),t.focused&&(e.focusedTaskId.value=n))}for(let[e,n]of[...g.entries()]){if(r.has(e))continue;let i=nn(e),a=ie(t,i);if(a){let e=t.indexOf(a);e>=0&&t.splice(e,1)}n.remove(),g.delete(e)}A(),b&&E()},de=n=>{let r=String(n||`home`).toLowerCase(),i=ln;r===`viewer`||r===`markdown`?i=un:r!==`home`&&(i=nn(r));let a=ie(t,i);if(a){for(let e of t)e!==a&&(e.active=!1);a.active=!0}e.focusedTaskId.value=r===`markdown`?`viewer`:r,A()},fe=()=>{rn()?x&&=(x.destroy(),null):!x&&n.isConnected&&(x=ae(n,{className:`env-shell-taskbar-under`,shadowBlur:28,shadowOffsetY:8,shadowColor:`rgba(0, 0, 0, 0.4)`}))};queueMicrotask(fe);let pe=typeof matchMedia==`function`?matchMedia(`(min-width: 641px)`):null,me=()=>fe();return pe?.addEventListener?.(`change`,me),S.push(()=>pe?.removeEventListener?.(`change`,me)),{element:n,taskList:t,setFocusedTaskId:de,syncWindowTasks:ue,dispose:()=>{D(),T(),x?.destroy(),x=null;for(let e of S)try{e()}catch{}S.length=0,g.clear(),i.replaceChildren()}}}var sn,cn,ln,un,dn,fn,pn,mn=e((()=>{b(),v(),r(),$t(),kt(),Ie(),mt(),D(),tn(),u(),w(),sn=d(en),cn=class extends E{constructor(){super()}styles=()=>sn;render=()=>s`<div part="taskbar" class="taskbar"><slot></slot></div>`},cn=C([g(`ui-taskbar`)],cn),ln=`#env-home`,un=`#env-viewer`,dn=`#env-win-`,fn=420,pn=3e4})),hn,gn=e((()=>{hn=`/**
 * Font Styles Index
 *
...
.c-overlaying [data-axis] {
  pointer-events: auto;
}`}));function _n(e){if(typeof Uint8Array.fromBase64==`function`)return Uint8Array.fromBase64(e);let t=atob(e),n=new Uint8Array(t.length);for(let e=0;e<t.length;e++)n[e]=t.charCodeAt(e);return n}async function vn(e,t=`gzip`){if(typeof CompressionStream>`u`)throw Error(`Compression Streams API is not supported in this browser`);let n=new DecompressionStream(t),r=n.writable.getWriter(),i=n.readable.getReader();r.write(e),r.close();let a=[],o=!1;for(;!o;){let{value:e,done:t}=await i.read();o=t,e&&a.push(e)}let s=a.reduce((e,t)=>e+t.length,0),c=new Uint8Array(s),l=0;for(let e of a)c.set(e,l),l+=e.length;return c}async function yn(e,t,n=`font/woff2`){if(wn.has(t))return wn.get(t);let r=new Blob([e],{type:n}),i=URL.createObjectURL(r);return wn.set(t,i),i}async function bn(e){let{base64:t,family:n,style:r=`normal`,weight:i=`normal`,compressed:a=!1}=e,o=`${n}-${r}-${i}`;if(Tn.has(o))return Tn.get(o);let s=_n(t),c=await yn(a?await vn(s):s,o,a?`application/octet-stream`:`font/woff2`),l=new FontFace(n,`url(${c}) format('woff2')`,{style:r,weight:typeof i==`string`?i:`${i}`,display:`swap`});return await l.load(),document.fonts.add(l),Tn.set(o,l),l}async function xn(e){let t=e.map(e=>bn(e));return Promise.all(t)}async function Sn(){return En||(En=h(()=>import(`./font-registry-D4KJRdpC.js`),[],import.meta.url)?.catch?.(e=>{console.error(`Failed to load font registry:`,e)}),En)}async function Cn(){let e=await Sn();return xn(Object.values(e.fontRegistry))}var wn,Tn,En,Dn=e((()=>{m(),wn=new Map,Tn=new Map,En=null})),On,kn=e((()=>{On=`@layer tokens, base, layout, utilities, shells, shell, views, view, viewer, components, ux-layer, markdown, essentials, print, print-breaks, overrides;
@layer components {
  button {
    display: inline-flex;
    align-items: center;
...
}`})),An,jn,Mn=e((()=>{Dn(),u(),gn(),kn(),Dn(),An=`
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
`,jn=async e=>{await f(An)?.catch(()=>void 0),await Cn().catch(()=>void 0),await f(hn)?.catch(()=>void 0),e?.includeGlobalNativeControls&&await f(On)?.catch(()=>void 0)}})),Nn,Pn=e((()=>{Nn=`@function --hsv(--src-color <color>) returns <color> {
  result: hsl(from var(--src-color, black) h calc(calc((calc(l / 100) - calc(calc(l / 100) * (1 - calc(s / 100) / 2))) / clamp(0.0001, min(calc(calc(l / 100) * (1 - calc(s / 100) / 2)), calc(1 - calc(calc(l / 100) * (1 - calc(s / 100) / 2)))), 1)) * 100) calc(calc(calc(l / 100) * (1 - calc(s / 100) / 2)) * 100) / alpha);
}
/*
 * Filename: _tokens.scss
...
:host(ui-task:not([data-active])) {
  opacity: 0.6;
}`})),Fn,In,Ln,Rn,zn=e((()=>{D(),u(),v(),Pn(),w(),Fn=d(Nn),In=e=>{let t=``;if(typeof e==`string`)t=e;else if(typeof e==`object`&&e&&`value`in e){let n=e.value;t=n==null?``:String(n)}else e!=null&&typeof e!=`object`&&(t=String(e));(!t||t===`undefined`||t===`null`||t===`[object Object]`)&&(t=``);let n=t.trim().charAt(0);return n?n.toUpperCase():`?`},Ln=(e,t,n)=>{let r=e.getAttribute(t);return r!=null&&String(r).trim()?String(r).trim():n},Rn=class extends E{title;icon;constructor(){super()}styles=()=>Fn;render=function(){let e=Ln(this,`title`,`Task`),t=Ln(this,`icon`,`app-window`),n=In(e);return s`
        ...
        `}},C([_({source:`attr`})],Rn.prototype,`title`,void 0),C([_({source:`attr`})],Rn.prototype,`icon`,void 0),Rn=C([g(`ui-task`)],Rn)})),Bn,Vn=e((()=>{Bn=`ui-taskbar[data-type=desktop] > ui-task[data-focus] {
...
:host(ui-taskbar[data-type=desktop]) ::slotted(ui-task[data-focus]) {
  ...
  color: --c2-on-surface(0, var(--current));
  color: contrast-color(var(--c2-surface(0, var(--current))));
}`})),Hn=e((()=>{u(),Vn(),d(Bn)})),Un,Wn=e((()=>{Un=`ui-taskbar[data-type=mobile] > ui-task[data-focus] {
  background: --c2-surface(0, var(--current));
  color: --c2-on-surface(0, var(--current));
  color: contrast-color(var(--c2-surface(0, var(--current))));
}
...
:host(ui-taskbar[data-type=mobile]) ::slotted(ui-task[data-focus]) {
  ...
  color: --c2-on-surface(0, var(--current));
  color: contrast-color(var(--c2-surface(0, var(--current))));
}`})),Gn=e((()=>{u(),Wn(),d(Un)}));function Kn(){try{return globalThis.navigator?.windowControlsOverlay??null}catch{return null}}function qn(){if(typeof globalThis.matchMedia!=`function`)return`unknown`;try{if(globalThis.matchMedia(`(display-mode: window-controls-overlay)`).matches)return`window-controls-overlay`;if(globalThis.matchMedia(`(display-mode: fullscreen)`).matches)return`fullscreen`;if(globalThis.matchMedia(`(display-mode: standalone)`).matches)return`standalone`;if(globalThis.matchMedia(`(display-mode: minimal-ui)`).matches)return`minimal-ui`;if(globalThis.matchMedia(`(display-mode: browser)`).matches)return`browser`}catch{}return`unknown`}function Jn(e){if(!e?.visible||typeof e.getTitlebarAreaRect!=`function`)return null;try{let t=e.getTitlebarAreaRect();return t?{x:t.x,y:t.y,width:t.width,height:t.height}:null}catch{return null}}function Yn(e){let t=Kn(),n=!!t?.visible,r=qn(),i=n||r===`standalone`||r===`fullscreen`||r===`window-controls-overlay`||r===`minimal-ui`,a=`off`;return e&&(a=n?`wco`:i?`standalone`:`fallback`),{requested:e,wcoVisible:n,displayMode:r,titlebarRect:Jn(t),isStandaloneLike:i,surface:a}}function Xn(e){let t=()=>{e.onChange(Yn(e.getRequested()))},n=[];if(typeof globalThis.matchMedia==`function`)for(let e of[`(display-mode: window-controls-overlay)`,`(display-mode: standalone)`,`(display-mode: fullscreen)`,`(display-mode: minimal-ui)`,`(display-mode: browser)`])try{n.push(globalThis.matchMedia(e))}catch{}let r=()=>t();for(let e of n)try{e.addEventListener?.(`change`,r)}catch{try{e.addListener?.(r)}catch{}}let i=Kn(),a=()=>t();try{i?.addEventListener?.(`geometrychange`,a)}catch{}return queueMicrotask(t),()=>{for(let e of n)try{e.removeEventListener?.(`change`,r)}catch{try{e.removeListener?.(r)}catch{}}try{i?.removeEventListener?.(`geometrychange`,a)}catch{}}}var Zn=e((()=>{})),W,Qn,$n,G,er,tr,nr,K,rr,q,ir,ar,J,or,sr,cr,Y,lr,ur,dr,X,Z,fr,Q,pr,mr=e((()=>{W=null,Qn=null,$n=null,G=[],er=null,tr=0,nr=null,K=`#cbb8a4`,rr=`__CWSP_NATIVE_THEME_COLOR_OWNED__`,q=e=>{let t=String(e||``).trim().toLowerCase();if(!t)return!1;if(t===`#007acc`||t===`#007accff`||t===`#36c`||t===`#3366cc`)return!0;let n=t.match(/^rgba?\(\s*([\d.]+)\s*[, ]\s*([\d.]+)\s*[, ]\s*([\d.]+)/i);if(n&&(t?.startsWith?.(`#`)||t?.startsWith?.(`rgb`))){let e=Math.round(Number(n[1])),t=Math.round(Number(n[2])),r=Math.round(Number(n[3]));if(e<=20&&t>=100&&t<=140&&r>=180&&r<=220)return!0}return!1},ir=()=>{try{return!!globalThis?.[rr]}catch{return!1}},ar=e=>{er=e;try{globalThis[rr]=!!e}catch{}},J=e=>{if(!e||!e.isConnected||e.hasAttribute(`minimized`))return!1;if(e.hasAttribute(`native-mode`))return!0;let t=e.hasAttribute(`maximized`)||e.hasAttribute(`data-desk-max`)||e.hasAttribute(`data-mobile-max`)||e.hasAttribute(`data-native-active`);if(!t)return!1;try{let t=e.getBoundingClientRect(),n=Math.max(1,globalThis.innerWidth||1),r=Math.max(1,globalThis.innerHeight||1);return t.top<=8&&t.left<=8&&t.width>=n*.92&&t.height>=r*.85}catch{return t}},or=()=>{if(typeof document>`u`)return null;if(er?.isConnected&&J(er))return er;let e=Array.from(document.querySelectorAll(`ui-window[native-mode]:not([minimized])`));if(e.length)return e[e.length-1];let t=Array.from(document.querySelectorAll(`ui-window[data-desk-max]:not([minimized]), ui-window[maximized]:not([minimized]), ui-window[data-mobile-max]:not([minimized])`));for(let e=t.length-1;e>=0;e--){let n=t[e];if(J(n))return n}return null},sr=()=>{Qn||typeof MutationObserver>`u`||typeof document>`u`||(Qn=new MutationObserver(()=>{let e=or();e?.isConnected?Q(e):Z()}),Qn.observe(document.documentElement,{attributes:!0,attributeFilter:[`data-theme`,`class`,`style`,`color-scheme`]}))},cr=e=>{$n||typeof MutationObserver>`u`||($n=new MutationObserver(()=>{if(!ir())return;let t=(e.getAttribute(`content`)||``).toLowerCase(),n=(nr||``).toLowerCase();if(n&&t===n&&!q(t))return;let r=or();r?Q(r):q(t)&&X(K,!0)}),$n.observe(e,{attributes:!0,attributeFilter:[`content`]}))},Y=e=>e,lr=()=>{if(typeof document>`u`)return null;let e=document.querySelector(`meta[data-theme-color]`)||document.querySelector(`meta[name="theme-color"]`);e||(e=document.createElement(`meta`),e.setAttribute(`name`,`theme-color`),e.setAttribute(`data-theme-color`,``),document.head?.appendChild(e));try{let t=Array.from(document.querySelectorAll(`meta[name="theme-color"]`));for(let n of t)n!==e&&n.remove()}catch{}return cr(e),e},ur=(e,t)=>{try{let n=document.createElement(`div`);n.setAttribute(`data-theme-color-probe`,`true`),n.style.cssText=`position:fixed;left:-8px;top:-8px;inline-size:4px;block-size:4px;pointer-events:none;opacity:0;background:${t}`,e.appendChild(n);let r=Y(getComputedStyle(n).backgroundColor);return n.remove(),r}catch{return null}},dr=e=>{let t=e.shadowRoot?.querySelector(`.title-handler`);if(t){let e=ur(t,`var(--ui-win-titlebar-bg, var(--color-surface-container, Canvas))`);if(e)return e;let n=Y(getComputedStyle(t).backgroundColor);if(n)return n}let n=getComputedStyle(e);for(let t of[`--ui-win-titlebar-bg`,`--color-surface-container`,`--color-surface`]){let r=ur(e,`var(${t})`);if(r)return r;let i=n.getPropertyValue(t).trim();if(!i)continue;let a=Y(i);if(a)return a}let r=getComputedStyle(document.documentElement);for(let e of[`--color-surface-container`,`--color-surface`,`--color-surface-container-low`]){let t=ur(document.documentElement,`var(${e})`)||Y(r.getPropertyValue(e).trim());if(t)return t}return null},X=(e,t=!1)=>{let n=lr();if(!n||!e)return;let r=e.toLowerCase();q(r)&&(r=K);let i=(n.getAttribute(`content`)||``).toLowerCase();if(!(i===r&&!t&&!q(i))&&(n.setAttribute(`content`,r),n.setAttribute(`data-theme-color`,``),n.removeAttribute(`media`),nr=r,t||i!==r||q(i)))try{let e=n.parentNode||document.head;e?.removeChild(n),e?.appendChild(n)}catch{}},Z=()=>{if(typeof document>`u`||or())return;ar(null),nr=null;let e=lr();if(!e)return;let t=document.documentElement,n=getComputedStyle(t),r=document.body?getComputedStyle(document.body):null,i=Y(n.getPropertyValue(`--color-surface-container`).trim())||Y(n.getPropertyValue(`--color-surface`).trim())||Y(n.getPropertyValue(`--ui-win-titlebar-bg`).trim())||(r?Y(r.backgroundColor):null)||Y(n.backgroundColor);i?X(i):q(String(e.getAttribute(`content`)||``))&&X(K,!0),sr()},fr=e=>e.hasAttribute(`maximized`)||e.hasAttribute(`data-desk-max`)||e.hasAttribute(`data-mobile-max`)||e.hasAttribute(`data-native-active`),Q=e=>{if(!e||typeof document>`u`||e.hasAttribute(`minimized`)||!e.hasAttribute(`native-mode`)&&!fr(e)&&!J(e))return;let t=lr();if(!t)return;if(W==null){let e=t.getAttribute(`content`)||``;W=q(e)?``:e}ar(e),q(String(t.getAttribute(`content`)||``))&&X(K,!0);let n=++tr,r=(t=!1)=>{if(n!==tr||!e.isConnected||!e.hasAttribute(`native-mode`)&&!fr(e)&&!J(e))return;let r=dr(e)||K;X(r,t)};r(!0),requestAnimationFrame(()=>{r(!1),requestAnimationFrame(()=>r(!0))});for(let e of G)clearTimeout(e);G=[],G.push(setTimeout(()=>r(!0),50),setTimeout(()=>r(!0),160),setTimeout(()=>r(!0),400)),sr()},pr=e=>{if(typeof document>`u`||!document.querySelector(`meta[name="theme-color"]`))return;let t=or();if(t&&t!==e){Q(t);return}let n=Array.from(document.querySelectorAll(`ui-window[native-mode]:not([minimized]), ui-window[data-desk-max]:not([minimized]), ui-window[maximized]:not([minimized]), ui-window[data-mobile-max]:not([minimized])`)).filter(t=>t!==e&&J(t));if(n.length){Q(n[n.length-1]);return}ar(null),nr=null,tr+=1;for(let e of G)clearTimeout(e);G=[],W!=null&&W&&!q(W)?(X(W,!0),W=null):(W=null,Z())},typeof document<`u`&&queueMicrotask(()=>{try{Z()}catch{}})})),hr,gr=e((()=>{hr=`/*
