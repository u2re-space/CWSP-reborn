...
</div>`;onRender(){return super.onRender(),fn(this),this}open(){dn(this.shadowRoot),this.removeAttribute(`hidden`),this.hidden=!1,this.setAttribute(`open`,``)}close(){this.hidden=!0,this.setAttribute(`hidden`,``),this.removeAttribute(`open`)}toggle(e){this.hasAttribute(`open`)?this.close():this.open()}},pn=z([v(`ui-quick-settings`)],pn),mn=null,Promise.try(()=>{typeof requestAnimationFrame==`function`&&requestAnimationFrame(()=>{Promise.try(async()=>{let e=screen?.orientation?.lock;if(typeof e!=`function`)return;let t=e.call(screen.orientation,`natural`);t&&typeof t.catch==`function`&&await t.catch(()=>{})}).catch(()=>{})})}).catch(()=>{});try{Yt()}catch{}})))()}var gn;function _n(){return(_n=e((()=>{gn=`/*
...
`))},Yo=class extends B{styles=()=>Wo;onInitialize(){let e=super.onInitialize();return queueMicrotask(()=>{qo(this),Ko(this),R()?ue().then(e=>Jo(this,e)):Jo(this,null,`Browser / PWA: use Mount folder (showDirectoryPicker).`)}),e??this}render=function(){let e=this,t=R(),n=de();return w`<div class="explorer-settings" part="root">
            ...
            <section class="explorer-settings__card" hidden=${!t}>
                ...
                <div class="explorer-settings__actions">
                    <button class="btn btn--primary" type="button" disabled=${!t} on:click=${()=>{le().then(()=>ue().then(t=>Jo(e,t,`Opened system all-files settings.`)))}}>Allow all files</button>
                    <button class="btn" type="button" disabled=${!t} on:click=${()=>{fe().then(t=>{Jo(e,null,t?`SAF tree: ${t}`:`SAF pick cancelled.`),window.dispatchEvent(new CustomEvent(`cwsp:explorer-mount-change`))})}}>Pick SAF folder</button>
                </div>
            </section>
            <section class="explorer-settings__card">
                <h3 class="explorer-settings__title">
                ...
                <label class="explorer-settings__check">
                    <input type="checkbox" data-explorer-opfs-enabled checked=${re()} disabled=${!k()} on:change=${e=>{A(e.currentTarget.checked),N(),xe(`/user/`),xe(`/idb/`),pe(),window.dispatchEvent(new CustomEvent(`cwsp:explorer-mount-change`))}} />
            ...
            <section class="explorer-settings__card" hidden=${t||!n}>
                ...
                <div class="explorer-settings__actions">
                    <button class="btn" type="button" disabled=${!n} on:click=${()=>{L().then(t=>{t&&(Ce(t),Ko(e),window.dispatchEvent(new CustomEvent(`cwsp:explorer-mount-change`)))})}}>Mount folder</button>
                </div>
                <div data-explorer-mounts class="explorer-settings__mounts"></div>
            </section>
        </div>`}},Yo=z([v(`ui-explorer-settings`)],Yo),Xo=()=>{document.querySelectorAll(`ui-file-manager`).forEach(e=>{let t=e.shadowRoot?.querySelector(`.fm-root`);t?.classList.remove(`fm-root--settings`),t?.querySelector(`ui-explorer-settings`)?.remove()}),document.querySelector(`ui-explorer-settings`)?.remove()},c(window,`keydown`,e=>{e.key===`Escape`&&Xo()})})))()}var Qo,$o,es,ts,ns,rs,is;function as(){return(as=e((()=>{T(),fi(),Zo(),Qo=()=>document.querySelector(`ui-file-manager`),$o=()=>Qo()?.shadowRoot?.querySelector(`ui-explorer-settings`)??document.querySelector(`ui-explorer-settings`),es=()=>m()&&E()?!0:document.querySelector(`.cw-context-menu, .cw-context-menu-layer`)?(ui(),!0):$o()?(Xo(),!0):!1,ts=()=>{let e=globalThis;e.__CWSP_NATIVE_BACK__={handleBackPress:es};let t=e.__CWSP_LAUNCHER_HOME__?.handleBackPress;e.__CWSP_LAUNCHER_HOME__={...e.__CWSP_LAUNCHER_HOME__||{},handleBackPress:()=>es()||typeof t==`function`&&t()}},ns=()=>{let e=globalThis;if(e.__CWSP_CAP_BACK_BOUND__)return;let t=e.Capacitor?.Plugins?.App;if(typeof t?.addListener==`function`){e.__CWSP_CAP_BACK_BOUND__=!0;try{t.addListener(`backButton`,()=>{es()})}catch(e){console.warn(`[overlay-back] Capacitor backButton bind failed`,e)}}},rs=()=>{x({id:`app-menu-overlay`,priority:b.SIDEBAR,isActive:()=>!!document.querySelector(`.env-shell-app-menu[data-open]`),close:()=>(document.querySelector(`.env-shell-app-menu`)?.dispatchEvent(new CustomEvent(`env-app-menu-request-close`,{bubbles:!0})),!0)}),x({id:`speed-dial-editor`,priority:b.MODAL,isActive:()=>!!document.querySelector(`dialog.speed-dial-editor[open], dialog.sd-icon-picker[open]`),close:()=>(document.querySelectorAll(`dialog.speed-dial-editor[open], dialog.sd-icon-picker[open]`).forEach(e=>{try{e.close()}catch{e.remove()}}),!0)})},is=()=>{let e=globalThis;if(e.__CWSP_LAUNCHER_BACK_STACK__){ns(),ts();return}e.__CWSP_LAUNCHER_BACK_STACK__=!0;try{g({preventDefaultNavigation:!0,pushInitialState:!1})}catch{}rs(),ns(),ts()}})))()}var os;function ss(){return(ss=e((()=>{os=`/*
...
}`})))()}function cs(e){return`${_s}${String(e||``).trim().toLowerCase()}`}function ls(){let e=document.querySelector(`.env-shell-chrome`);return e instanceof HTMLElement&&e.hasAttribute(`data-desktop`)?!1:e instanceof HTMLElement&&e.dataset.chromeLayout===`mobile`||typeof matchMedia==`function`&&matchMedia(`(max-width: 640px)`).matches}function us(e,t){if(e.some(e=>{let t=String(e.id||``).trim().toLowerCase();return!t||t===`home`?!1:e.visible!==!1&&!e.minimized}))return!0;let n=String(t.value||`home`).trim().toLowerCase();if(n&&n!==`home`&&n!==`viewer`)return!0;let r=document.querySelector(`.env-shell-workspace`);if(!r)return!1;for(let e of r.querySelectorAll(`ui-window`)){if(!(e instanceof HTMLElement)||e.hidden||e.hasAttribute(`data-minimized`))continue;let t=getComputedStyle(e);if(t.display!==`none`&&t.visibility!==`hidden`&&!(Number.parseFloat(t.opacity||`1`)<=0))return!0}return!1}function ds(e=new Date){return Sn(e)}function fs(e){let t=ie([]);h(t),f(hs,t,{title:`Home`,icon:`house-line`},{},function(){for(let e of t)e!==this&&(e.active=!1);this.active=!0,e.focusedTaskId.value=`home`,e.onHome()}),f(gs,t,{title:`Markdown`,icon:`article`},{},function(){for(let e of t)e!==this&&(e.active=!1);this.active=!0,e.focusedTaskId.value=`viewer`,e.onViewer()});let n=document.createElement(`ui-taskbar`);n.className=`env-shell-taskbar wf-chrome-no-select`,n.setAttribute(`part`,`taskbar`),n.setAttribute(`data-type`,`desktop`);let r=document.createElement(`div`);r.className=`env-shell-taskbar__pins`;let i=document.createElement(`div`);i.className=`env-shell-taskbar__windows`;let a=document.createElement(`ui-task`);a.setAttribute(`title`,`Home`),a.setAttribute(`icon`,`house-line`),a.setAttribute(`data-id`,hs),a.setAttribute(`data-env-home`,``),a.setAttribute(`aria-label`,`Home`),a.setAttribute(`aria-haspopup`,`menu`),r.append(a);let o=document.createElement(`div`);o.className=`env-shell-taskbar__workspaces`,o.setAttribute(`aria-label`,`Workspaces`);let s=()=>{let e=wo(),t=To();o.replaceChildren();for(let n of e){let e=document.createElement(`button`);e.type=`button`,e.className=`env-shell-taskbar__workspace`,e.title=n.label,e.textContent=n.label.replace(/^Side\s+/i,``)||n.id.slice(-1).toUpperCase(),e.toggleAttribute(`data-active`,n.id===t),e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),Mo(n.id)}),o.append(e)}};s(),window.addEventListener(ho,s),r.append(o),is();let c=()=>{let e=ls();a.setAttribute(`title`,e?`Home`:`Start`),a.setAttribute(`aria-label`,e?`Home`:`Start`),a.setAttribute(`icon`,e?`house-line`:`windows-logo`),a.toggleAttribute(`data-env-start`,!e),e?a.setAttribute(`aria-keyshortcuts`,`LongPress`):a.removeAttribute(`aria-keyshortcuts`)};c();let u=document.createElement(`div`);u.className=`env-shell-taskbar__tray-host`;let d=document.createElement(`div`);d.className=`env-shell-taskbar__clock`,d.setAttribute(`role`,`button`),d.setAttribute(`tabindex`,`0`),d.setAttribute(`aria-label`,`Calendar`),d.setAttribute(`aria-haspopup`,`dialog`),d.setAttribute(`data-chrome-flyout-anchor`,`calendar`);let p=document.createElement(`span`);p.className=`env-shell-taskbar__clock-time`;let m=document.createElement(`span`);m.className=`env-shell-taskbar__clock-date`,d.append(p,m);let g=()=>{let{time:e,date:t}=ds();p.textContent=e,m.textContent=t,d.title=`${e} · ${t}`};g();let _=setInterval(g,ys),v=On(e.device,`env-device-tray env-device-tray--taskbar`);v.setAttribute(`role`,`button`),v.setAttribute(`tabindex`,`0`),v.setAttribute(`aria-label`,`Quick settings`),v.setAttribute(`aria-haspopup`,`dialog`),v.setAttribute(`data-chrome-flyout-anchor`,`quick-settings`);let y=e=>{e.preventDefault(),e.stopPropagation(),yt(d)},b=e=>{e.preventDefault(),e.stopPropagation(),It(v)};d.addEventListener(`click`,y),d.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&y(e)}),v.addEventListener(`click`,b),v.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&b(e)}),u.append(v,d);let x=document.createElement(`div`);x.className=`env-shell-navbar__switcher`,x.setAttribute(`role`,`menu`),x.setAttribute(`aria-label`,`Open apps`),x.hidden=!0;let S=document.createElement(`ul`);S.className=`env-shell-navbar__switcher-list`,x.appendChild(S),n.append(r,i,u,x);let ee=Ba(),C=ee?$a():void 0,w=new Map,T=[],E=null,D=!1,O=!1,k=[];k.push(()=>clearInterval(_));let A=e=>T.find(t=>String(t.id||``).trim().toLowerCase()===e),j=t=>{let n=String(t||``).trim().toLowerCase();if(!n)return;let r=A(n),i=String(e.focusedTaskId.value||``).trim().toLowerCase(),a=!!r?.focused||i===n||i===`markdown`&&n===`viewer`||i===`viewer`&&(n===`viewer`||n===`markdown`);if(r?.minimized){r.minimized=!1,r.focused=!0,w.get(n)?.toggleAttribute(`data-minimized`,!1),e.focusedTaskId.value=n===`markdown`?`viewer`:n,e.onWindowTask?.(n);return}if(a&&r&&r.visible!==!1){r.minimized=!0,r.focused=!1,w.get(n)?.toggleAttribute(`data-minimized`,!0),e.onMinimizeWindow?.(n);return}e.focusedTaskId.value=n===`markdown`?`viewer`:n,e.onWindowTask?.(n)},ne=(t,r,i)=>{if(ls())return;t.preventDefault(),t.stopPropagation();let a=String(r||``).trim().toLowerCase(),o=!!A(a)?.minimized,s=[{id:o?`restore`:`minimize`,label:o?`Restore`:`Minimize`,icon:o?`arrow-square-out`:`minus`,action:()=>{o?(e.focusedTaskId.value=a,e.onWindowTask?.(a)):e.onMinimizeWindow?.(a)}},{id:`close`,label:`Close`,icon:`x`,danger:!0,action:()=>e.onCloseWindow?.(a)}];di({x:t.clientX,y:t.clientY,compact:!0,anchor:t.target instanceof Element?t.target:n,items:s})};n.addEventListener(`contextmenu`,t=>{if(ls())return;let r=typeof t.composedPath==`function`?t.composedPath():[];for(let e of r)if(e instanceof Element&&e.closest?.(`ui-task`))return;t.preventDefault(),t.stopPropagation(),di({x:t.clientX,y:t.clientY,compact:!0,anchor:n,items:[{id:`show-desktop`,label:`Show desktop`,icon:`desktop`,action:()=>e.onHome()},{id:`home`,label:`Home`,icon:`house-line`,action:()=>e.onHome()}]})});let M=()=>{O=!1,x.hidden=!0,S.replaceChildren(),n.removeAttribute(`data-switcher-open`)},N=()=>{let r=T.filter(e=>String(e.id||``).trim());if(S.replaceChildren(),r.length)for(let n of r){let r=String(n.id||``).trim().toLowerCase(),i=document.createElement(`li`);i.className=`env-shell-navbar__switcher-row`,i.setAttribute(`role`,`none`);let a=document.createElement(`button`);a.type=`button`,a.className=`env-shell-navbar__switcher-item`,a.setAttribute(`role`,`menuitem`),a.toggleAttribute(`data-active`,!!n.focused&&!n.minimized),a.toggleAttribute(`data-minimized`,!!n.minimized);let o=document.createElement(`ui-icon`);o.setAttribute(`icon`,n.icon||`app-window`),o.setAttribute(`icon-style`,`duotone`),o.setAttribute(`aria-hidden`,`true`);let s=document.createElement(`span`);s.className=`env-shell-navbar__switcher-label`,s.textContent=n.title||r,a.append(o,s),a.addEventListener(`click`,n=>{n.preventDefault(),n.stopPropagation(),M(),e.focusedTaskId.value=r;let i=cs(r),a=l(t,i);a?a.focus=!0:e.onWindowTask?.(r)});let c=document.createElement(`button`);c.type=`button`,c.className=`env-shell-navbar__switcher-close`,c.setAttribute(`aria-label`,`Close ${n.title||r}`),c.title=`Close`;let u=document.createElement(`ui-icon`);u.setAttribute(`icon`,`x`),u.setAttribute(`icon-style`,`bold`),u.setAttribute(`aria-hidden`,`true`),c.appendChild(u),c.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),e.onCloseWindow?.(r),T=T.filter(e=>String(e.id||``).trim().toLowerCase()!==r),w.get(r)?.remove(),w.delete(r),T.length?N():M()}),i.append(a,c),S.appendChild(i)}else{let e=document.createElement(`li`);e.className=`env-shell-navbar__switcher-empty`,e.textContent=`No open apps`,S.appendChild(e)}O=!0,x.hidden=!1,n.setAttribute(`data-switcher-open`,``)},re=()=>{E!=null&&(clearTimeout(E),E=null)},P=()=>{n.toggleAttribute(`data-app-menu-open`,!!C?.isOpen())},ae=()=>{M(),C?.close(),P(),l(t,hs).focus=!0,e.onHome()},oe=()=>{M(),C?.toggle(),P(),l(t,hs).focus=!0,e.focusedTaskId.value=`home`,R()},F=()=>{!C||C.isOpen()||(M(),C.open(),P(),l(t,hs).focus=!0,e.focusedTaskId.value=`home`,R())},se=()=>{C&&(M(),C.openPage(),P(),l(t,hs).focus=!0,e.focusedTaskId.value=`home`,R())};try{let e=globalThis;e.__CWSP_LAUNCHER_HOME__={...e.__CWSP_LAUNCHER_HOME__||{},openAppMenu:F,openAppMenuPage:se}}catch{}let I=()=>{if(us(T,e.focusedTaskId)){ae();return}if(C?.isOpen()){C.close(),P();return}oe()};a.addEventListener(`click`,e=>{if(D){e.preventDefault(),e.stopPropagation(),D=!1;return}if(ee&&C){e.preventDefault(),e.stopPropagation(),I();return}ae()}),a.addEventListener(`pointerdown`,e=>{if(ls()&&(e.button==null||e.button===0)){D=!1,re(),E=setTimeout(()=>{E=null,D=!0;try{a.releasePointerCapture?.(e.pointerId)}catch{}N()},vs);try{a.setPointerCapture?.(e.pointerId)}catch{}}},{capture:!0});let ce=()=>{re()};a.addEventListener(`pointerup`,ce,{capture:!0}),a.addEventListener(`pointercancel`,ce,{capture:!0}),a.addEventListener(`contextmenu`,e=>{ls()&&(e.preventDefault(),D=!0,re(),N())});let L=e=>{if(!O)return;let t=typeof e.composedPath==`function`?e.composedPath():[];for(let e of t)if(e===x||e===a||e instanceof Element&&(e===x||x.contains(e)||e===a))return;M()};document.addEventListener(`pointerdown`,L,{capture:!0}),k.push(()=>document.removeEventListener(`pointerdown`,L,{capture:!0}));let R=()=>{let t=String(e.focusedTaskId.value||`home`),n=(e,t)=>{e.toggleAttribute(`data-env-active`,t),e.toggleAttribute(`data-active`,t),e.toggleAttribute(`data-focus`,t)};n(a,t===`home`);for(let[e,r]of w)n(r,t===e)};te(()=>{R()},[e.focusedTaskId],{triggerImmediately:!0});let le=e=>{let n=String(e.id||``).trim().toLowerCase();if(!n||n===`home`)return;let r=cs(n),a=e.title||n,o=String(e.icon||``).trim()||`app-window`,s=w.get(n);if(!s){let e=f(r,null,{title:a,icon:o},{viewId:n},function(){for(let e of t)e!==this&&(e.active=!1);this.active=!0,j(n)});e.list=t,t.push(e),s=document.createElement(`ui-task`),s.setAttribute(`data-id`,r),s.setAttribute(`data-view`,n),s.setAttribute(`title`,a),s.setAttribute(`aria-label`,a),s.setAttribute(`icon`,o),s.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),j(n)}),s.addEventListener(`contextmenu`,e=>{ne(e,n,a)}),w.set(n,s),i.appendChild(s)}s.setAttribute(`title`,a),s.setAttribute(`aria-label`,a),s.setAttribute(`icon`,o),s.toggleAttribute(`data-minimized`,!!e.minimized),s.hidden=e.visible===!1},ue=n=>{T=Array.isArray(n)?n.slice():[];let r=new Set;for(let t of n){let n=String(t.id||``).trim().toLowerCase();!n||n===`home`||(r.add(n),le(t),t.focused&&(e.focusedTaskId.value=n))}for(let[e,n]of[...w.entries()]){if(r.has(e))continue;let i=cs(e),a=l(t,i);if(a){let e=t.indexOf(a);e>=0&&t.splice(e,1)}n.remove(),w.delete(e)}R(),O&&N()},de=n=>{let r=String(n||`home`).toLowerCase(),i=hs;r===`viewer`||r===`markdown`?i=gs:r!==`home`&&(i=cs(r));let a=l(t,i);if(a){for(let e of t)e!==a&&(e.active=!1);a.active=!0}e.focusedTaskId.value=r===`markdown`?`viewer`:r,R()};if(queueMicrotask(c),C){let e=()=>P();n.addEventListener(`env-app-menu-open`,e),n.addEventListener(`env-app-menu-close`,e),k.push(()=>{n.removeEventListener(`env-app-menu-open`,e),n.removeEventListener(`env-app-menu-close`,e)})}return{element:n,taskList:t,setFocusedTaskId:de,syncWindowTasks:ue,appMenu:C,openAppMenu:C?F:void 0,openAppMenuPage:C?se:void 0,isSwitcherOpen:()=>O,closeSwitcher:M,dispose:()=>{re(),M(),C?.dispose();for(let e of k)try{e()}catch{}k.length=0,w.clear(),i.replaceChildren()}}}var ps,ms,hs,gs,_s,vs,ys;function bs(){return(bs=e((()=>{I(),ne(),j(),T(),fi(),Mn(),Et(),hn(),oo(),No(),as(),V(),ss(),y(),ps=u(os),ms=class extends Ze{constructor(){super()}styles=()=>ps;render=()=>w`<div part="taskbar" class="taskbar"><slot></slot></div>`},ms=z([v(`ui-taskbar`)],ms),hs=`#env-home`,gs=`#env-viewer`,_s=`#env-win-`,vs=420,ys=3e4})))()}function xs(e){if(typeof Uint8Array.fromBase64==`function`)return Uint8Array.fromBase64(e);let t=atob(e),n=new Uint8Array(t.length);for(let e=0;e<t.length;e++)n[e]=t.charCodeAt(e);return n}async function Ss(e,t=`gzip`){if(typeof CompressionStream>`u`)throw Error(`Compression Streams API is not supported in this browser`);let n=new DecompressionStream(t),r=n.writable.getWriter(),i=n.readable.getReader();r.write(e),r.close();let a=[],o=!1;for(;!o;){let{value:e,done:t}=await i.read();o=t,e&&a.push(e)}let s=a.reduce((e,t)=>e+t.length,0),c=new Uint8Array(s),l=0;for(let e of a)c.set(e,l),l+=e.length;return c}async function Cs(e,t,n=`font/woff2`){if(Os.has(t))return Os.get(t);let r=new Blob([e],{type:n}),i=URL.createObjectURL(r);return Os.set(t,i),i}async function ws(e){let{base64:t,family:n,style:r=`normal`,weight:i=`normal`,compressed:a=!1}=e,o=`${n}-${r}-${i}`;if(ks.has(o))return ks.get(o);let s=xs(t),c=await Cs(a?await Ss(s):s,o,a?`application/octet-stream`:`font/woff2`),l=new FontFace(n,`url(${c}) format('woff2')`,{style:r,weight:typeof i==`string`?i:`${i}`,display:`swap`});return await l.load(),document.fonts.add(l),ks.set(o,l),l}async function Ts(e){let t=e.map(e=>ws(e));return Promise.all(t)}async function Es(){return As||(As=ae(()=>import(`./font-registry-BefiOUvg.js`).then(e=>typeof e.loadFontRegistryShards==`function`?e.loadFontRegistryShards().then(e=>({fontRegistry:e})):{fontRegistry:e.fontRegistry}),[],import.meta.url).catch(e=>{console.error(`Failed to load font registry:`,e)}),As)}async function Ds(){let e=await Es();return Ts(Object.values(e.fontRegistry))}var Os,ks,As;function js(){return(js=e((()=>{P(),Os=new Map,ks=new Map,As=null})))()}var Ms;function Ns(){return(Ns=e((()=>{Ms=`/*
 * Filename: index.scss
...
}`})))()}var Is;function Ls(){return(Ls=e((()=>{js(),y(),Ns(),Fs(),Is=async e=>{await Ds().catch(()=>void 0),await d(Ms)?.catch(()=>void 0),e?.includeGlobalNativeControls&&await d(Ps)?.catch(()=>void 0)}})))()}var Rs;function zs(){return(zs=e((()=>{Rs=`:host(ui-task), :host(ui-task) * {
  box-sizing: border-box;
  user-select: none;
  touch-action: manipulation;
  -webkit-user-drag: none;
...
:host(ui-task:not([data-active])) {
  opacity: 0.6;
}`})))()}var Bs,Vs,Hs,Us;function Ws(){return(Ws=e((()=>{V(),y(),T(),zs(),Bs=u(Rs),Vs=e=>{let t=``;if(typeof e==`string`)t=e;else if(typeof e==`object`&&e&&`value`in e){let n=e.value;t=n==null?``:String(n)}else e!=null&&typeof e!=`object`&&(t=String(e));(!t||t===`undefined`||t===`null`||t===`[object Object]`)&&(t=``);let n=t.trim().charAt(0);return n?n.toUpperCase():`?`},Hs=(e,t,n)=>{let r=e.getAttribute(t);return r!=null&&String(r).trim()?String(r).trim():n},Us=class extends Ze{title;icon;constructor(){super()}styles=()=>Bs;render=function(){let e=Hs(this,`title`,`Task`),t=Hs(this,`icon`,`app-window`),n=Vs(e);return w`
        ...
        `}},z([S({source:`attr`})],Us.prototype,`title`,void 0),z([S({source:`attr`})],Us.prototype,`icon`,void 0),Us=z([v(`ui-task`)],Us)})))()}var Gs;function Ks(){return(Ks=e((()=>{Gs=`ui-taskbar[data-type=desktop] > ui-task[data-focus] {
...
:host(ui-taskbar[data-type=desktop]) ::slotted(ui-task[data-focus]) {
  ...
  color: --c2-on-surface(0, var(--current));
  color: contrast-color(var(--c2-surface(0, var(--current))));
}`})))()}function qs(){return(qs=e((()=>{D(),y(),Ks(),u(Gs)})))()}var Js;function Ys(){return(Ys=e((()=>{Js=`ui-taskbar[data-type=mobile] > ui-task[data-focus] {
  background: --c2-surface(0, var(--current));
  color: --c2-on-surface(0, var(--current));
  color: contrast-color(var(--c2-surface(0, var(--current))));
}
...
}`})))()}var Pc,Fc,Ic,Lc,Rc,zc,Bc;function Vc(){return(Vc=e((()=>{T(),D(),y(),V(),I(),nc(),jc(),Nc(),Pc=u(Mc),Fc=`minus`,Ic=`corners-out`,Lc=`corners-in`,Rc=`x`,zc=Object.freeze({w:240,h:160}),Bc=class extends B{titleHandler;contentHandler;footerHandler;resizer;#e=M(0);#t=M(0);#n=null;#r=null;#i=null;#a=null;#o=null;#s=null;#c=null;#l=!1;#u=0;#d=0;#f=null;#p=null;#m=null;styles=function(){return Pc};render=function(){return w`<div class="window-container" part="window-container">
