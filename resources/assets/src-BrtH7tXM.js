import{n as e}from"./rolldown-runtime-DaJ6WEGw.js";import{F as t,Q as n,X as r,t as i}from"./src-CXiRKMoF.js";import{a,c as o,o as s,s as c,t as l}from"./cws-bridge-CRDA1GOm.js";import{F as u,G as d,H as f,I as p,R as m,V as ee}from"./airpad-cwsp-client-parity-BenwfXdR.js";import{a as te,r as ne}from"./SettingsTypes-D6q9S6cT.js";import{C as re,T as h,w as g,y as ie}from"./config-DRIOcGx0.js";import{a as ae,o as _}from"./Settings-oeAinB_2.js";import{C as oe,D as se,E as ce,M as le,O as ue,T as de,j as fe,w as pe}from"./cwsp-app.js";import{i as me,n as he,r as v}from"./frontend-debug-capture-BNPCucC4.js";var y,b,x,ge,_e=e((()=>{m(),y=e=>typeof e==`string`?e.trim():``,b=e=>{let t=y(e).replace(/\/lna-probe\/?$/i,``).replace(/\/+$/,``);if(!t)return``;let n=ee(t);if(!n?.host)return t;let r=n.protocol??`https`;return n.port?`${r}://${n.host}:${n.port}`:`${r}://${n.host}:8434`},x=(e,t,n)=>{let r=new Set(d(n.relay??``).map(e=>b(e))),i=new Set(d(n.direct??``).map(e=>b(e))),a=b(e);return r.has(a)?t===0?`Relay / gateway`:`Relay (alt)`:i.has(a)?`Direct peer`:a.includes(`192.168.0.200`)?`Gateway LAN fallback`:a.includes(`45.147.121.152`)?`Gateway WAN fallback`:a.includes(`127.0.0.1`)||a.includes(`localhost`)?`Loopback`:`Candidate ${t+1}`},ge=(e,t)=>{let n=e.find(e=>e.ok)?.origin;if(n)return b(n);let r=p(t);return r[0]?r[0]:e[0]?.origin?b(e[0].origin):``}}));async function ve(e){if(!o())return null;let t=p(e);try{let n=await c({channel:`network:probe`,payload:{relay:b(S(e.relay)),direct:b(S(e.direct)),candidates:t}}),r=n.echo,i=n.results??r?.results;if(!Array.isArray(i)||!i.length)return null;let a=new Set,o=[];for(let t=0;t<i.length;t++){let n=i[t],r=b(String(n.url??``));if(!r||a.has(r))continue;a.add(r);let s=!!n.reachable,c=typeof n.statusCode==`number`?n.statusCode:void 0;o.push({label:x(r,o.length,e),origin:r,ok:s,status:c,error:E(n,s,c)})}return o.length?o:null}catch{return null}}async function ye(e,t){if(!o())return null;let n=b(e);if(!n)return null;let r=Date.now();try{let e=await c({channel:`network:dispatch-probe`,payload:{origin:n,clientId:S(t.clientId),token:S(t.token),accessToken:S(t.accessToken)}}),i=typeof e.statusCode==`number`?e.statusCode:void 0,a=!!e.ok,o=typeof e.error==`string`?e.error.trim():``,s=typeof e.bodySnippet==`string`?e.bodySnippet:``;return{origin:n,ok:a,status:i,latencyMs:Date.now()-r,bodySnippet:s,error:a?void 0:o||(i==null?`dispatch failed`:`HTTP ${i}`)}}catch{return null}}async function be(e,t={}){let n=await ve(e);if(n?.length)return n;let r=await D(e);if(r?.length)return r;let i=t.timeoutMs??3500,a=t.maxCandidates??6,o=[],s=p(e);for(let t=0;t<s.length;t++){let n=s[t],r=x(n,t,e),c=u(n).slice(0,a);if(!c.length){o.push({label:r,origin:n,ok:!1,error:`invalid host`});continue}for(let e of c){let t=await f(e,{timeoutMs:i});if(o.push({label:r,...t}),t.ok)break}}return o}async function xe(e,t){if(!C())return null;let n=w(`/service/endpoint-probe`);if(!n)return null;let r=globalThis.__WEBNATIVE_AUTH__?.key,i=p(e),a=typeof AbortController<`u`?new AbortController:void 0,o=a?globalThis.setTimeout(()=>a.abort(),12e3):void 0;try{let o=await fetch(n,{method:`POST`,headers:r?{"Content-Type":`application/json`,"X-API-Key":r}:{"Content-Type":`application/json`},body:JSON.stringify({origins:i,dispatch:!0,auth:t}),signal:a?.signal});if(!o.ok)return null;let s=await o.json();return{probes:(s.rows??[]).map((t,n)=>({label:x(t.origin,n,e),origin:t.origin,ok:t.ok,status:t.status,error:t.error,latencyMs:t.latencyMs})),dispatch:s.dispatch?{origin:s.dispatch.origin,ok:s.dispatch.ok,status:s.dispatch.status,error:s.dispatch.error,bodySnippet:s.dispatch.bodySnippet,latencyMs:s.dispatch.latencyMs}:void 0}}catch{return null}finally{o&&clearTimeout(o)}}async function Se(e,t,n,r=8e3){let i=e.filter(e=>e.ok).map(e=>b(e.origin)),a=i.length?i:p(t),o={origin:``,ok:!1,error:`no origin`};for(let e of a)if(o=await Ce(e,n,r),o.ok)return o;return o}async function Ce(e,t,n=8e3){let r=b(e),i=Date.now();if(!r)return{origin:``,ok:!1,error:`no origin`};let a=await ye(r,t);if(a)return a;let o=typeof AbortController<`u`?new AbortController:void 0,s=o&&n>0?globalThis.setTimeout(()=>o.abort(),n):void 0,c=S(t.clientId),l=S(t.token),u=S(t.accessToken),d={"Content-Type":`application/json`};u&&(d[`x-auth-token`]=u),l&&(d[`x-cws-token`]=l);let f={userId:c,byId:c,from:c,clientId:c,userKey:u||l,token:l||u,accessToken:u||l,op:`ask`,what:`debug:isReady`,payload:{}};try{let e=await fetch(`${r}/api/network/dispatch`,{method:`POST`,mode:`cors`,cache:`no-store`,credentials:`omit`,headers:d,body:JSON.stringify(f),signal:o?.signal}),t=await e.text().catch(()=>``),n=Date.now()-i,a=e.ok;return{origin:r,ok:a,status:e.status,statusText:e.statusText,latencyMs:n,bodySnippet:t.slice(0,240),error:a?void 0:`HTTP ${e.status}${e.statusText?` ${e.statusText}`:``}`.trim()}}catch(e){return{origin:r,ok:!1,error:T(e),latencyMs:Date.now()-i}}finally{s&&clearTimeout(s)}}async function we(e,t,n,r={}){let i=e.map(e=>e.trim()).filter(Boolean);if(!i.length)return[];let a=r.timeoutMs??8e3,s=[b(r.originHint||``),...p(t).map(b)].filter(Boolean)[0]||``;if(!s)return i.map(e=>({id:e,ok:!1,origin:``,error:`no gateway origin`}));let l=S(n.clientId),u=S(n.token),d=S(n.accessToken),f=[];for(let e of i){let t=Date.now(),n={"Content-Type":`application/json`};d&&(n[`x-auth-token`]=d),u&&(n[`x-cws-token`]=u);let r={userId:l,byId:l,from:l,clientId:l,userKey:d||u,token:u||d,accessToken:d||u,op:`ask`,what:`clipboard:isReady`,purpose:`clipboard`,nodes:[e],destinations:[e],payload:{probe:!0,destination:e}};if(o())try{let n=await c({channel:`network:dispatch-probe`,payload:{origin:s,clientId:l,token:u,accessToken:d,what:`clipboard:isReady`,nodes:[e],destinations:[e]}}),r=typeof n.statusCode==`number`?n.statusCode:void 0,i=!!n.ok,a=typeof n.error==`string`?n.error.trim():``;f.push({id:e,origin:s,ok:i,status:r,latencyMs:Date.now()-t,bodySnippet:typeof n.bodySnippet==`string`?n.bodySnippet:void 0,error:i?void 0:a||(r==null?`dispatch failed`:`HTTP ${r}`)});continue}catch{}let i=typeof AbortController<`u`?new AbortController:void 0,p=i&&a>0?globalThis.setTimeout(()=>i.abort(),a):void 0;try{let a=await fetch(`${s}/api/network/dispatch`,{method:`POST`,mode:`cors`,cache:`no-store`,credentials:`omit`,headers:n,body:JSON.stringify(r),signal:i?.signal}),o=await a.text().catch(()=>``),c=a.ok;f.push({id:e,origin:s,ok:c,status:a.status,latencyMs:Date.now()-t,bodySnippet:o.slice(0,240),error:c?void 0:`HTTP ${a.status}${a.statusText?` ${a.statusText}`:``}`.trim()})}catch(n){f.push({id:e,origin:s,ok:!1,error:T(n),latencyMs:Date.now()-t})}finally{p&&clearTimeout(p)}}return f}var S,C,w,T,E,D,O,Te=e((()=>{m(),a(),_e(),S=e=>typeof e==`string`?e.trim():``,C=()=>{try{let e=globalThis;return!!(e.__WEBNATIVE_AUTH__||e.__CWS_WEBNATIVE_BOOT__)}catch{return!1}},w=e=>{try{let t=globalThis.__WEBNATIVE_AUTH__;return t?.port?`http://127.0.0.1:${t.port}${e}`:null}catch{return null}},T=e=>{let t=e instanceof Error?e.message:String(e??`fetch failed`);return/abort/i.test(t)?`timeout`:/refused|ECONNREFUSED/i.test(t)?`connection refused`:/ENOTFOUND|NAME_NOT_RESOLVED/i.test(t)?`host not found`:/certificate|cert\.|ssl|tls|ERR_CERT/i.test(t)?`TLS: ${t}`:/failed to fetch/i.test(t)&&o()?`WebView fetch blocked (CORS/TLS) — use native bridge`:t},E=(e,t,n)=>{if(t)return;let r=[];return e.error&&r.push(String(e.error)),n!=null&&n>=0&&n!==204&&r.push(`HTTP ${n}`),r.join(` · `)||`unreachable`},D=async e=>{let t=await xe(e,{});return t?.probes.length?t.probes:null},O=e=>{let t=String(e||``).trim();if(!t)return[];let n=new Set,r=[];for(let e of t.split(/[,;\s\n\r]+/)){let t=e.trim();!t||n.has(t)||(n.add(t),r.push(t))}return r}})),k,A,j,Ee,De,M,Oe,ke,Ae=e((()=>{me(),a(),fe(),k=e=>`${new Date(e.ts).toISOString()} [${e.level}] (${e.scope}) ${e.msg}`,A=(e=400)=>{let t=he()?.tail(e)??[];return t.length?t.map(k).join(`
`)+`
...
`},Ee=async(e=400)=>{try{let t=await c({channel:`debug:frontend`,payload:{limit:e}}),n=t.echo,r=typeof t.text==`string`?t.text:typeof n?.text==`string`?n.text:``;if(r.trim())return r.endsWith(`
`)?r:`${r}\n`}catch{}return``},De=async(e,t=``)=>{let n=[`CWSP Network diagnostics export`,`generated: ${new Date().toISOString()}`,`userAgent: ${navigator.userAgent}`,``].join(`
`),r=A(500),i=await Ee(500),a=await j(500);return[n,`=== Page log ===`,e||`(empty)`,``,t?`=== Probe summary ===
`+t+`
`:``,`=== Frontend log (WebView ring) ===`,r,i.trim()?`=== Frontend log (native ring) ===
`+i+`
`:``,`=== Logcat (native) ===`,a].filter(Boolean).join(`
`)},M=async e=>{let t=e||`(empty log)`;try{return await le(t),!0}catch{return!1}},Oe=(e,t)=>{let n=new Blob([t],{type:`text/plain;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=e,i.rel=`noopener`,document.body.append(i),i.click(),i.remove(),URL.revokeObjectURL(r)},ke=e=>`${e}-${new Date().toISOString().replace(/[:.]/g,`-`)}.txt`})),N,P,je,Me,Ne=e((()=>{N={root:{selector:`.cw-network-view`,role:`main`,label:`CWSP Network`},statusGrid:{selector:`.cw-network-status-grid`,role:`status`,ariaLive:`polite`,ariaAtomic:`false`},activityLog:{selector:`[data-log]`,ariaLive:`polite`,ariaRelevant:`additions text`,role:`log`},probeList:{selector:`[data-probe-list]`,role:`list`},actions:[{action:`test`,label:`Run network test`},{action:`reconnect`,label:`Reconnect WebSocket`},{action:`open-settings`,label:`Open settings`},{action:`copy-frontend-log`,label:`Copy frontend log`},{action:`copy-logcat`,label:`Copy logcat`},{action:`save-page-logs`,label:`Save page logs`}],minTouchTargetPx:44},P=e=>{let{root:t,statusGrid:n,activityLog:r,probeList:i,actions:a}=N;e.setAttribute(`role`,t.role),!e.getAttribute(`aria-label`)&&!e.getAttribute(`aria-labelledby`)&&e.setAttribute(`aria-label`,t.label);let o=e.querySelector(`h1`);o&&!o.id&&(o.id=`cw-network-view-title`,e.setAttribute(`aria-labelledby`,o.id),e.removeAttribute(`aria-label`));let s=e.querySelector(n.selector);s instanceof HTMLElement&&(s.setAttribute(`role`,n.role),s.setAttribute(`aria-live`,n.ariaLive),s.setAttribute(`aria-atomic`,n.ariaAtomic));let c=e.querySelector(r.selector);c instanceof HTMLElement&&(c.setAttribute(`role`,r.role),c.setAttribute(`aria-live`,r.ariaLive),c.setAttribute(`aria-relevant`,r.ariaRelevant));let l=e.querySelector(i.selector);l instanceof HTMLElement&&l.setAttribute(`role`,i.role);for(let t of a){let n=e.querySelector(`[data-action="${t.action}"]`);n instanceof HTMLElement&&(!n.getAttribute(`aria-label`)&&!n.textContent?.trim()&&n.setAttribute(`aria-label`,t.label),n instanceof HTMLButtonElement&&!n.type&&(n.type=`button`))}},je=e=>{let t=[],{root:n,statusGrid:r,activityLog:i,probeList:a,actions:o}=N;e.getAttribute(`role`)!==n.role&&t.push({code:`root-role`,message:`root role must be "${n.role}"`}),!e.getAttribute(`aria-label`)&&!e.getAttribute(`aria-labelledby`)&&t.push({code:`root-label`,message:`root needs aria-label or aria-labelledby`});let s=e.querySelector(r.selector);s?(s.getAttribute(`role`)!==r.role&&t.push({code:`status-role`,message:`status grid role must be "${r.role}"`}),s.getAttribute(`aria-live`)!==r.ariaLive&&t.push({code:`status-live`,message:`status grid aria-live must be "${r.ariaLive}"`})):t.push({code:`status-grid-missing`,message:`status grid missing`});let c=e.querySelector(i.selector);c?(c.getAttribute(`aria-live`)!==i.ariaLive&&t.push({code:`log-live`,message:`log aria-live must be "${i.ariaLive}"`}),c.getAttribute(`role`)!==i.role&&t.push({code:`log-role`,message:`log role must be "${i.role}"`})):t.push({code:`log-missing`,message:`activity log [data-log] missing`});let l=e.querySelector(a.selector);l?l.getAttribute(`role`)!==a.role&&t.push({code:`probe-list-role`,message:`probe list role must be "${a.role}"`}):t.push({code:`probe-list-missing`,message:`probe list missing`});for(let n of o){let r=e.querySelector(`[data-action="${n.action}"]`);if(!r){t.push({code:`action-missing:${n.action}`,message:`action button data-action="${n.action}" missing`});continue}r.getAttribute(`aria-label`)||r.textContent?.trim()||t.push({code:`action-label:${n.action}`,message:`action "${n.action}" needs accessible name`})}return t},Me=(e=document)=>{let t=e.createElement(`div`);return t.className=`cw-network-view`,t.dataset.view=`network`,t.innerHTML=`
    ...
    `,P(t),t}})),Pe,Fe=e((()=>{Pe=`@layer ui-network {
