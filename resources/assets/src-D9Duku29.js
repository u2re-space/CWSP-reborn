import{n as e}from"./rolldown-runtime-DaJ6WEGw.js";import{a as t,ft as n,kt as r}from"./HistoryManager-5Sa1FlA6.js";import{a as i,c as a,o,s,t as c}from"./cws-bridge-D4zjrwqD.js";import{B as l,J as u,L as d,R as f,U as p,W as ee,q as te}from"./airpad-cwsp-client-parity-CXnZuSSw.js";import{t as ne}from"./src-mG6hxM_E.js";import{a as re,r as ie}from"./SettingsTypes-D6q9S6cT.js";import{C as m,S as ae,v as oe,w as h}from"./remote-connection-runtime-DMQizJyk.js";import{a as se,c as ce,i as le,l as ue,o as de,s as fe}from"./hub-socket-boot-q3HsQ78i.js";import{a as pe,o as g}from"./Settings-BBPxD7yF.js";import{n as me,o as he}from"./clipboard-device-B0k7txdo.js";import{i as _,n as ge,r as v}from"./frontend-debug-capture-CQEyVisE.js";var _e,y,b,ve,ye=e((()=>{l(),_e=e=>typeof e==`string`?e.trim():``,y=e=>{let t=_e(e).replace(/\/lna-probe\/?$/i,``).replace(/\/+$/,``);if(!t)return``;let n=p(t);if(!n?.host)return t;let r=n.protocol??`https`;return n.port?`${r}://${n.host}:${n.port}`:`${r}://${n.host}:8434`},b=(e,t,n)=>{let r=new Set(u(n.relay??``).map(e=>y(e))),i=new Set(u(n.direct??``).map(e=>y(e))),a=y(e);if(r.has(a))return t===0?`Relay / gateway`:`Relay (alt)`;if(i.has(a))return`Direct peer`;if(a.includes(`192.168.0.200`))return`Gateway LAN fallback`;let o=te({relay:n.relay,extras:[n.direct]}).toLowerCase(),s=(()=>{try{return new URL(a).hostname.toLowerCase()}catch{return a.toLowerCase()}})();return s===o||s===`45.147.121.152`||a.includes(`45.147.121.152`)?`Gateway WAN fallback`:a.includes(`127.0.0.1`)||a.includes(`localhost`)?`Loopback`:`Candidate ${t+1}`},ve=(e,t)=>{let n=e.find(e=>e.ok)?.origin;if(n)return y(n);let r=f(t);return r[0]?r[0]:e[0]?.origin?y(e[0].origin):``}}));async function be(e){if(!a())return null;let t=f(e);try{let n=await s({channel:`network:probe`,payload:{relay:y(x(e.relay)),direct:y(x(e.direct)),candidates:t}}),r=n.echo,i=n.results??r?.results;if(!Array.isArray(i)||!i.length)return null;let a=new Set,o=[];for(let t=0;t<i.length;t++){let n=i[t],r=y(String(n.url??``));if(!r||a.has(r))continue;a.add(r);let s=!!n.reachable,c=typeof n.statusCode==`number`?n.statusCode:void 0;o.push({label:b(r,o.length,e),origin:r,ok:s,status:c,error:T(n,s,c)})}return o.length?o:null}catch{return null}}async function xe(e,t){if(!a())return null;let n=y(e);if(!n)return null;let r=Date.now();try{let e=await s({channel:`network:dispatch-probe`,payload:{origin:n,clientId:x(t.clientId),token:x(t.token),accessToken:x(t.accessToken)}}),i=typeof e.statusCode==`number`?e.statusCode:void 0,a=!!e.ok,o=typeof e.error==`string`?e.error.trim():``,c=typeof e.bodySnippet==`string`?e.bodySnippet:``;return{origin:n,ok:a,status:i,latencyMs:Date.now()-r,bodySnippet:c,error:a?void 0:o||(i==null?`dispatch failed`:`HTTP ${i}`)}}catch{return null}}async function Se(e,t={}){let n=await be(e);if(n?.length)return n;let r=await E(e);if(r?.length)return r;let i=t.timeoutMs??3500,a=t.maxCandidates??6,o=[],s=f(e);for(let t=0;t<s.length;t++){let n=s[t],r=b(n,t,e),c=d(n).slice(0,a);if(!c.length){o.push({label:r,origin:n,ok:!1,error:`invalid host`});continue}for(let e of c){let t=await ee(e,{timeoutMs:i});if(o.push({label:r,...t}),t.ok)break}}return o}async function Ce(e,t){if(!S())return null;let n=C(`/service/endpoint-probe`);if(!n)return null;let r=globalThis.__WEBNATIVE_AUTH__?.key,i=f(e),a=typeof AbortController<`u`?new AbortController:void 0,o=a?globalThis.setTimeout(()=>a.abort(),12e3):void 0;try{let o=await fetch(n,{method:`POST`,headers:r?{"Content-Type":`application/json`,"X-API-Key":r}:{"Content-Type":`application/json`},body:JSON.stringify({origins:i,dispatch:!0,auth:t}),signal:a?.signal});if(!o.ok)return null;let s=await o.json();return{probes:(s.rows??[]).map((t,n)=>({label:b(t.origin,n,e),origin:t.origin,ok:t.ok,status:t.status,error:t.error,latencyMs:t.latencyMs})),dispatch:s.dispatch?{origin:s.dispatch.origin,ok:s.dispatch.ok,status:s.dispatch.status,error:s.dispatch.error,bodySnippet:s.dispatch.bodySnippet,latencyMs:s.dispatch.latencyMs}:void 0}}catch{return null}finally{o&&clearTimeout(o)}}async function we(e,t,n,r=8e3){let i=e.filter(e=>e.ok).map(e=>y(e.origin)),a=i.length?i:f(t),o={origin:``,ok:!1,error:`no origin`};for(let e of a)if(o=await Te(e,n,r),o.ok)return o;return o}async function Te(e,t,n=8e3){let r=y(e),i=Date.now();if(!r)return{origin:``,ok:!1,error:`no origin`};let a=await xe(r,t);if(a)return a;let o=typeof AbortController<`u`?new AbortController:void 0,s=o&&n>0?globalThis.setTimeout(()=>o.abort(),n):void 0,c=x(t.clientId),l=x(t.token),u=x(t.accessToken),d={"Content-Type":`application/json`};u&&(d[`x-auth-token`]=u),l&&(d[`x-cws-token`]=l);let f={userId:c,byId:c,from:c,clientId:c,userKey:u||l,token:l||u,accessToken:u||l,op:`ask`,what:`debug:isReady`,payload:{}};try{let e=await fetch(`${r}/api/network/dispatch`,{method:`POST`,mode:`cors`,cache:`no-store`,credentials:`omit`,headers:d,body:JSON.stringify(f),signal:o?.signal}),t=await e.text().catch(()=>``),n=Date.now()-i,a=e.ok;return{origin:r,ok:a,status:e.status,statusText:e.statusText,latencyMs:n,bodySnippet:t.slice(0,240),error:a?void 0:`HTTP ${e.status}${e.statusText?` ${e.statusText}`:``}`.trim()}}catch(e){return{origin:r,ok:!1,error:w(e),latencyMs:Date.now()-i}}finally{s&&clearTimeout(s)}}async function Ee(e,t,n,r={}){let i=e.map(e=>e.trim()).filter(Boolean);if(!i.length)return[];let o=r.timeoutMs??8e3,c=[y(r.originHint||``),...f(t).map(y)].filter(Boolean)[0]||``;if(!c)return i.map(e=>({id:e,ok:!1,origin:``,error:`no gateway origin`}));let l=x(n.clientId),u=x(n.token),d=x(n.accessToken),p=[];for(let e of i){let t=Date.now(),n={"Content-Type":`application/json`};d&&(n[`x-auth-token`]=d),u&&(n[`x-cws-token`]=u);let r={userId:l,byId:l,from:l,clientId:l,userKey:d||u,token:u||d,accessToken:d||u,op:`ask`,what:`clipboard:isReady`,purpose:`clipboard`,nodes:[e],destinations:[e],payload:{probe:!0,destination:e}};if(a())try{let n=await s({channel:`network:dispatch-probe`,payload:{origin:c,clientId:l,token:u,accessToken:d,what:`clipboard:isReady`,nodes:[e],destinations:[e]}}),r=typeof n.statusCode==`number`?n.statusCode:void 0,i=!!n.ok,a=typeof n.error==`string`?n.error.trim():``;p.push({id:e,origin:c,ok:i,status:r,latencyMs:Date.now()-t,bodySnippet:typeof n.bodySnippet==`string`?n.bodySnippet:void 0,error:i?void 0:a||(r==null?`dispatch failed`:`HTTP ${r}`)});continue}catch{}let i=typeof AbortController<`u`?new AbortController:void 0,f=i&&o>0?globalThis.setTimeout(()=>i.abort(),o):void 0;try{let a=await fetch(`${c}/api/network/dispatch`,{method:`POST`,mode:`cors`,cache:`no-store`,credentials:`omit`,headers:n,body:JSON.stringify(r),signal:i?.signal}),o=await a.text().catch(()=>``),s=a.ok;p.push({id:e,origin:c,ok:s,status:a.status,latencyMs:Date.now()-t,bodySnippet:o.slice(0,240),error:s?void 0:`HTTP ${a.status}${a.statusText?` ${a.statusText}`:``}`.trim()})}catch(n){p.push({id:e,origin:c,ok:!1,error:w(n),latencyMs:Date.now()-t})}finally{f&&clearTimeout(f)}}return p}var x,S,C,w,T,E,D,De=e((()=>{l(),i(),ye(),x=e=>typeof e==`string`?e.trim():``,S=()=>{try{let e=globalThis;return!!(e.__WEBNATIVE_AUTH__||e.__CWS_WEBNATIVE_BOOT__)}catch{return!1}},C=e=>{try{let t=globalThis.__WEBNATIVE_AUTH__;return t?.port?`http://127.0.0.1:${t.port}${e}`:null}catch{return null}},w=e=>{let t=e instanceof Error?e.message:String(e??`fetch failed`);return/abort/i.test(t)?`timeout`:/refused|ECONNREFUSED/i.test(t)?`connection refused`:/ENOTFOUND|NAME_NOT_RESOLVED/i.test(t)?`host not found`:/certificate|cert\.|ssl|tls|ERR_CERT/i.test(t)?`TLS: ${t}`:/failed to fetch/i.test(t)&&a()?`WebView fetch blocked (CORS/TLS) — use native bridge`:t},T=(e,t,n)=>{if(t)return;let r=[];return e.error&&r.push(String(e.error)),n!=null&&n>=0&&n!==204&&r.push(`HTTP ${n}`),r.join(` · `)||`unreachable`},E=async e=>{let t=await Ce(e,{});return t?.probes.length?t.probes:null},D=e=>{let t=String(e||``).trim();if(!t)return[];let n=new Set,r=[];for(let e of t.split(/[,;\s\n\r]+/)){let t=e.trim();!t||n.has(t)||(n.add(t),r.push(t))}return r}})),O,k,A,Oe,ke,j,Ae,je,Me=e((()=>{_(),i(),me(),O=e=>`${new Date(e.ts).toISOString()} [${e.level}] (${e.scope}) ${e.msg}`,k=(e=400)=>{let t=ge()?.tail(e)??[];return t.length?t.map(O).join(`
`)+`
`:`(no frontend log entries — boot WebView debug capture first)
`},A=async(e=400)=>{try{let t=await s({channel:`debug:logcat`,payload:{limit:e}}),n=t.echo,r=typeof t.text==`string`?t.text:``,i=typeof n?.text==`string`?n.text:``,a=(r||i).trim();if(a)return a.endsWith(`
...
`},Oe=async(e=400)=>{try{let t=await s({channel:`debug:frontend`,payload:{limit:e}}),n=t.echo,r=typeof t.text==`string`?t.text:typeof n?.text==`string`?n.text:``;if(r.trim())return r.endsWith(`
