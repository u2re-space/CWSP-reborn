import{n as e}from"./rolldown-runtime-C0FnF6B9.js";import{Rt as t,an as n,ft as r,nn as i,t as a}from"./src-C8p-yYHh.js";import{a as o,n as s}from"./channel-actions-DkmBPovk.js";import{a as c,d as l,f as u,i as d,l as f,m as p,n as m,o as h,p as g,r as _,s as v,t as y,u as b}from"./transfer-history-runtime-DnZ7s_C3.js";var x;function S(){return(S=e((()=>{x=`/*
  ...
  .view-history__item[data-status=declined] .view-history__action-btn {
    opacity: 1;
        ...
        `,this.setupEventHandlers(),this.updateList(!0),requestAnimationFrame(()=>{this.visible&&c(!0)}),this.element}getToolbar(){return null}ensureRuntime(){let e=globalThis;e.__CWS_NEUTRALINO_BOOT__&&v(),e.Capacitor&&h(),m()}bindStore(){this.unsub||=m().subscribe(e=>{this.entries=e,this.scheduleUpdateList(!1)})}refreshFromStore(){this.entries=m().list()}teardown(){this.visible=!1,c(!1),this.listRaf&&=(cancelAnimationFrame(this.listRaf),0);try{this.unsub?.()}catch{}this.unsub=null,this.teardownSheet(),this.element=null,this.lastListSig=``,this.lastStructSig=``}teardownSheet(){try{this._sheet&&n(this._sheet)}catch{}this._sheet=null}renderEntries(){let e=document.createDocumentFragment();if(this.entries.length===0)return e.appendChild(r`
            ...
            `),e;for(let t of this.entries){let n=g(t.status,t.kind)?` is-muted`:``,i=f(t),a=t.kind===`files`&&(t.status===`progress`||typeof t.bytesDone==`number`&&typeof t.totalBytes==`number`&&(t.totalBytes||0)>0&&t.status!==`expired`),o=p(t),s=Math.round(o*100),c=l(t.speedBps),u=t.kind===`files`?`Files`:t.kind===`clipboard-image`?`Image`:`Clipboard`,d=t.kind===`clipboard-image`,m=d?_(t):``,h=!!m,v=b(t.totalBytes),y=``;if(i.length>0){let e=document.createElement(`div`);e.className=`view-history__item-actions`;for(let n of i){let i=r`<button class="view-history__action-btn" type="button"
                        data-action="${n}" data-id="${t.id}">
                        ${C(n,t)}
                    </button>`;i&&e.appendChild(i)}y=e}let x=a?r`
            ...
            `:``,S=d?r`
                <div class="view-history__media">
                    ${h?r`<img class="view-history__thumb" src="${m}" alt="" loading="lazy" decoding="async" />`:r`
                    ...
                    <div class="view-history__media-meta">
                        ...
                        ${t.subtitle?r`<p class="view-history__item-sub">${t.subtitle}</p>`:v?r`<p class="view-history__item-sub">${v}</p>`:``}
                        ${h?``:r`<p class="view-history__item-sub">Preview unavailable — Accept uses retained file when present</p>`}
                    </div>
                </div>
            `:r`
                <p class="view-history__item-desc">${t.title}</p>
