import{n as e}from"./rolldown-runtime-DaJ6WEGw.js";import{Rt as t,Zt as n,a as r,en as i}from"./HistoryManager-DVqS4qoS.js";import{t as a}from"./src-yMca1J0Q.js";import{a as o,n as s}from"./channel-actions-DC9DgSYV.js";import{a as c,dispatchHistoryAction as l,getTransferHistoryStore as u,historyImageSrc as d,i as f,n as p,o as m,r as h,s as g,startCapacitorTransferHistory as _,startNeutralinoTransferHistory as v,t as y}from"./transfer-history-runtime-DsMjdogG.js";var b,x=e((()=>{b=`/*
        ...
        `,this.setupEventHandlers(),this.updateList(),this.element}getToolbar(){return null}ensureRuntime(){let e=globalThis;e.__CWS_NEUTRALINO_BOOT__&&v(),e.Capacitor&&_(),u()}bindStore(){this.unsub||=u().subscribe(e=>{this.entries=e,this.updateList()})}refreshFromStore(){this.entries=u().list()}teardown(){try{this.unsub?.()}catch{}this.unsub=null,this.teardownSheet(),this.element=null}teardownSheet(){try{this._sheet&&i(this._sheet)}catch{}this._sheet=null}renderEntries(){let e=document.createDocumentFragment();if(this.entries.length===0)return e.appendChild(r`
            ...
            `:``,C=u?r`
                <div class="view-history__media">
                    ${v?r`<img class="view-history__thumb" src="${_}" alt="" loading="lazy" decoding="async" />`:r`
                    ...
                    <div class="view-history__media-meta">
                        ...
                        ${t.subtitle?r`<p class="view-history__item-sub">${t.subtitle}</p>`:y?r`<p class="view-history__item-sub">${y}</p>`:``}
                        ${v?``:r`<p class="view-history__item-sub">Preview unavailable — Accept uses retained file when present</p>`}
                    </div>
                </div>
            `:r`
                <p class="view-history__item-desc">${t.title}</p>
                ${t.subtitle?r`<p class="view-history__item-sub">${t.subtitle}</p>`:``}
                ${t.textPreview?r`<pre class="view-history__item-preview">${t.textPreview}</pre>`:``}
            `,w=r`
                <div class="view-history__item${n}${u?` is-image`:``}" data-entry="${t.id}" data-status="${t.status}" data-kind="${t.kind}">
                    ...
                    ${C}
                    ${x}
                    ${t.error?r`<p class="view-history__item-error">${t.error}</p>`:``}
                    ${b}
                </div>
