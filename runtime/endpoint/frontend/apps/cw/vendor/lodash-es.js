...
var hasOwnProperty$12 = objectProto$4.hasOwnProperty;
/**
* Used to resolve the
...
/** Used to check objects for own properties. */
var hasOwnProperty$9 = objectProto$1.hasOwnProperty;
/** Built-in value references. */
...
if (DataView && getTag(new DataView(/* @__PURE__ */ new ArrayBuffer(1))) != dataViewTag$3 || Map && getTag(new Map()) != mapTag$4 || Promise$1 && getTag(Promise$1.resolve()) != promiseTag || Set && getTag(new Set()) != setTag$4 || WeakMap$1 && getTag(new WeakMap$1()) != weakMapTag$1) getTag = function(value) {
...
var _ShadowBlot = class _ShadowBlot {
...
var _LeafBlot = class _LeafBlot extends ShadowBlot {
	/**
...
var _ParentBlot = class _ParentBlot extends ShadowBlot {
	constructor(scroll, domNode) {
...
var _InlineBlot = class _InlineBlot extends ParentBlot$1 {
	static create(value) {
...
var InlineBlot$1 = _InlineBlot;
var _BlockBlot = class _BlockBlot extends ParentBlot$1 {
	static create(value) {
		return super.create(value);
	}
	static formats(domNode, scroll) {
...
var BlockBlot$1 = _BlockBlot;
var _ContainerBlot = class _ContainerBlot extends ParentBlot$1 {
	checkMerge() {
...
var ContainerBlot$1 = _ContainerBlot;
...
var _ScrollBlot = class _ScrollBlot extends ParentBlot$1 {
...
var ScrollBlot$1 = _ScrollBlot;
var _TextBlot = class _TextBlot extends LeafBlot$1 {
	static create(value) {
...
var import_Delta = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	Object.defineProperty(exports, "__esModule", { value: true });
...
var Text$1 = class extends TextBlot$1 {};
var entityMap = {
	"&": "&amp;",
	"<": "&lt;",
	">": "&gt;",
...
var Inline = class Inline extends InlineBlot$1 {
...
var Block = class extends BlockBlot$1 {
	...
	insertBefore(blot, ref) {
		...
		if (head instanceof Break) head.remove();
		this.cache = {};
	}
	length() {
...
var BlockEmbed = class extends EmbedBlot$1 {
	...
	delta() {
		return new import_Delta.default().insert(this.value(), {
			...this.formats(),
...
function blockDelta(blot) {
	...
	}, new import_Delta.default()).insert("\n", bubbleFormats(blot));
}
...
var Cursor = class Cursor extends EmbedBlot$1 {
...
var debug$5 = namespace("quill:events");
[
	"selectionchange",
	"mousedown",
	"mouseup",
...
var debug$4 = namespace("quill:selection");
var Range = class {
	constructor(index) {
...
var Selection = class {
	constructor(scroll, emitter) {
		...
		this.emitter.listenDOM("selectionchange", document, () => {
			if (!this.mouseDown && !this.composing) setTimeout(this.update.bind(this, Emitter.sources.USER), 1);
		});
	...
	update() {
		...
		if (!isEqual$1(oldRange, this.lastRange)) {
			...
			if (source !== Emitter.sources.SILENT) this.emitter.emit(...args);
		}
	}
};
...
var Editor = class {
	...
	applyDelta(delta) {
		...
		const deleteDelta = new import_Delta.default();
		splitOpLines(normalizedDelta.ops.slice()).reduce((index, op) => {
			const length = import_Delta.Op.length(op);
			...
			if (op.insert != null) {
				...
				if (typeof op.insert === "string") {
					...
					if (line instanceof Block) {
						...
						if (leaf) formats = merge(formats, bubbleFormats(leaf));
					}
					attributes = import_Delta.AttributeMap.diff(formats, attributes) || {};
		...
		deleteDelta.reduce((index, op) => {
			...
			return index + import_Delta.Op.length(op);
		}, 0);
	...
	deleteText(index, length) {
		...
		return this.update(new import_Delta.default().retain(index).delete(length));
	}
	formatLine(index, length) {
		...
		this.scroll.optimize();
		const delta = new import_Delta.default().retain(index).retain(length, cloneDeep(formats));
	...
	getHTML(index, length) {
		...
		if (line) {
			...
			if (line.length() >= lineOffset + length && !(lineOffset === 0 && length === lineLength)) return convertHTML(line, lineOffset, length, true);
	...
	insertContents(index, contents) {
		...
		const change = new import_Delta.default().retain(index).concat(normalizedDelta);
		this.scroll.insertContents(index, normalizedDelta);
		return this.update(change);
	}
	insertEmbed(index, embed, value) {
		this.scroll.insertAt(index, embed, value);
		return this.update(new import_Delta.default().retain(index).insert({ [embed]: value }));
	}
	insertText(index, text) {
		...
		return this.update(new import_Delta.default().retain(index).insert(text, cloneDeep(formats)));
	}
	isBlank() {
	...
	removeFormat(index, length) {
		...
		let suffixLength = 0;
		let suffix = new import_Delta.default();
		if (line != null) {
			suffixLength = line.length() - offset;
			suffix = line.delta().slice(offset, offset + suffixLength - 1).insert("\n");
		}
		const diff = this.getContents(index, length + suffixLength).diff(new import_Delta.default().insert(text).concat(suffix));
		const delta = new import_Delta.default().retain(index).concat(diff);
	...
	update(change) {
		...
		if (mutations.length === 1 && mutations[0].type === "characterData" && mutations[0].target.data.match(ASCII) && this.scroll.find(mutations[0].target)) {
			...
			const oldText = new import_Delta.default().insert(oldValue);
			const newText = new import_Delta.default().insert(textBlot.value());
			const relativeSelectionInfo = selectionInfo && {
				oldRange: shiftRange$1(selectionInfo.oldRange, -index),
				newRange: shiftRange$1(selectionInfo.newRange, -index)
			};
			change = new import_Delta.default().retain(index).concat(oldText.diff(newText, relativeSelectionInfo)).reduce((delta, op) => {
			...
			this.delta = this.getDelta();
			if (!change || !isEqual$1(oldDelta.compose(change), this.delta)) change = oldDelta.diff(this.delta, selectionInfo);
		}
		return change;
	}
};
...
function convertHTML(blot, index, length) {
	...
	if (blot instanceof Text$1) return escapeText(blot.value().slice(index, index + length)).replaceAll(" ", "&nbsp;");
...
//#endregion
//#region ../../node_modules/quill/core/utils/createRegistryWithFormats.js
...
var createRegistryWithFormats = (formats, sourceRegistry, debug) => {
...
var Quill = class Quill {
	...
	static import(name) {
		if (this.imports[name] == null) debug$3.error(`Cannot import ${name}. Are you sure it was registered?`);
	...
	static register() {
		if (typeof (arguments.length <= 0 ? void 0 : arguments[0]) !== "string") {
			...
			if (typeof name === "string") this.register(`formats/${name}`, target, overwrite);
			...
			if (this.imports[path] != null && !overwrite) debug$3.warn(`Overwriting ${path} with`, target);
	...
	constructor(container) {
		...
		this.emitter.on(Emitter.events.EDITOR_CHANGE, (type) => {
			if (type === Emitter.events.TEXT_CHANGE) this.root.classList.toggle("ql-blank", this.editor.isBlank());
		});
		...
		this.emitter.on(Emitter.events.SCROLL_EMBED_UPDATE, (blot, delta) => {
			...
			modify.call(this, () => {
				const change = new import_Delta.default().retain(blot.offset(this)).retain({ [blot.statics.blotName]: delta });
	...
	format(name, value) {
		...
		return modify.call(this, () => {
			...
			let change = new import_Delta.default();
			if (range == null) return change;
			if (this.scroll.query(name, Scope.BLOCK)) change = this.editor.formatLine(range.index, range.length, { [name]: value });
	...
	*/
	...
	setContents(delta) {
		...
		return modify.call(this, () => {
			delta = new import_Delta.default(delta);
	...
	setSelection(index, length, source) {
		if (index == null) this.selection.setRange(null, length || Quill.sources.API);
			...
			if (source !== Emitter.sources.SILENT) this.scrollSelectionIntoView();
		}
	}
	setText(text) {
		let source = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : Emitter.sources.API;
		const delta = new import_Delta.default().insert(text);
...
function expandConfig(containerOrSelector, options) {
	...
	const theme = !options.theme || options.theme === Quill.DEFAULTS.theme ? Theme : Quill.import(`themes/${options.theme}`);
	...
	if (registry) {
		if (options.formats) debug$3.warn("Ignoring \"formats\" option because \"registry\" is specified");
	} else registry = options.formats ? createRegistryWithFormats(options.formats, config.registry, debug$3) : config.registry;
	return {
		...config,
		registry,
		container,
...
function modify(modifier, source, index, shift) {
	if (!this.isEnabled() && source === Emitter.sources.USER && !this.allowReadOnlyEdits) return new import_Delta.default();
...
function shiftRange(range, index, lengthOrSource, source) {
	...
	let start;
	let end;
	if (index && typeof index.transformPosition === "function") [start, end] = [range.index, range.index + range.length].map((pos) => index.transformPosition(pos, source !== Emitter.sources.USER));
...
var Scroll = class extends ScrollBlot$1 {
	...
	insertContents(index, delta) {
		const renderBlocks = this.deltaToRenderBlocks(delta.concat(new import_Delta.default().insert("\n")));
		...
		if (first) {
			...
			const delta = first.type === "block" ? first.delta : new import_Delta.default().insert({ [first.key]: first.value });
			insertInlineContents(this, index, delta);
			const newlineCharLength = first.type === "block" ? 1 : 0;
			const lineEndIndex = index + delta.length() + newlineCharLength;
			if (shouldInsertNewlineChar) this.insertAt(lineEndIndex - 1, "\n");
			const formats = bubbleFormats(this.line(index)[0]);
			const attributes = import_Delta.AttributeMap.diff(formats, first.attributes) || {};
			Object.keys(attributes).forEach((name) => {
	...
	lines() {
		...
		const getLines = (blot, blotIndex, blotLength) => {
			...
			blot.children.forEachAt(blotIndex, blotLength, (child, childIndex, childLength) => {
				if (isLine$1(child)) lines.push(child);
				else if (child instanceof ContainerBlot$1) lines = lines.concat(getLines(child, childIndex, lengthLeft));
	...
	optimize() {
		...
		if (mutations.length > 0) this.emitter.emit(Emitter.events.SCROLL_OPTIMIZE, mutations, context);
	}
	path(index) {
	...
	update(mutations) {
		...
		if (mutations.length > 0) this.emitter.emit(Emitter.events.SCROLL_BEFORE_UPDATE, source, mutations);
	...
	deltaToRenderBlocks(delta) {
		const renderBlocks = [];
		let currentBlockDelta = new import_Delta.default();
		delta.forEach((op) => {
			...
			if (typeof insert === "string") {
				...
				splitted.slice(0, -1).forEach((text) => {
					...
					currentBlockDelta = new import_Delta.default();
				});
				...
				if (!key) return;
				if (this.query(key, Scope.INLINE)) currentBlockDelta.push(op);
				else {
	...
	createBlock(attributes, refBlot) {
		...
		Object.entries(attributes).forEach((_ref3) => {
			let [key, value] = _ref3;
			if (this.query(key, Scope.BLOCK & Scope.BLOT) != null) blotName = key;
...
var AlignAttribute = new Attributor("align", "align", config$2);
var AlignClass = new ClassAttributor$1("align", "ql-align", config$2);
var AlignStyle = new StyleAttributor$1("align", "text-align", config$2);
//#endregion
...
var ColorClass = new ClassAttributor$1("color", "ql-color", { scope: Scope.INLINE });
var ColorStyle = new ColorAttributor("color", "color", { scope: Scope.INLINE });
//#endregion
//#region ../../node_modules/quill/formats/background.js
var BackgroundClass = new ClassAttributor$1("background", "ql-bg", { scope: Scope.INLINE });
var BackgroundStyle = new ColorAttributor("background", "background-color", { scope: Scope.INLINE });
//#endregion
//#region ../../node_modules/quill/formats/code.js
var CodeBlockContainer = class extends Container {
	static create(value) {
...
var DirectionAttribute = new Attributor("direction", "dir", config$1);
var DirectionClass = new ClassAttributor$1("direction", "ql-direction", config$1);
var DirectionStyle = new StyleAttributor$1("direction", "direction", config$1);
//#endregion
...
var FontClass = new ClassAttributor$1("font", "ql-font", config);
...
var debug$2 = namespace("quill:keyboard");
var SHORTKEY = /Mac/i.test(navigator.platform) ? "metaKey" : "ctrlKey";
var Keyboard = class Keyboard extends Module {
	...
	listen() {
		this.quill.root.addEventListener("keydown", (evt) => {
			...
			if (evt.keyCode === 229 && (evt.key === "Enter" || evt.key === "Backspace")) return;
	...
	handleBackspace(range, context) {
		...
		let delta = new import_Delta.default().retain(range.index - length).delete(length);
		if (context.offset === 0) {
			...
			if (prev) {
				if (!(prev.statics.blotName === "block" && prev.length() <= 1)) {
					...
					formats = import_Delta.AttributeMap.diff(curFormats, prevFormats) || {};
					if (Object.keys(formats).length > 0) {
						const formatDelta = new import_Delta.default().retain(range.index + line.length() - 2).retain(1, formats);
	...
	handleDelete(range, context) {
		...
		let formats = {};
		const [line] = this.quill.getLine(range.index);
		let delta = new import_Delta.default().retain(range.index).delete(length);
		if (context.offset >= line.length() - 1) {
			const [next] = this.quill.getLine(range.index + 1);
			if (next) {
				const curFormats = line.formats();
				const nextFormats = this.quill.getFormat(range.index, 1);
				formats = import_Delta.AttributeMap.diff(curFormats, nextFormats) || {};
	...
	handleEnter(range, context) {
		...
		const delta = new import_Delta.default().retain(range.index).delete(range.length).insert("\n", lineFormats);
		this.quill.updateContents(delta, Quill.sources.USER);
...
Keyboard.DEFAULTS = { bindings: {
	...
	"outdent backspace": {
		...
		offset: 0,
		handler(range, context) {
			if (context.format.indent != null) this.quill.format("indent", "-1", Quill.sources.USER);
			else if (context.format.list != null) this.quill.format("list", false, Quill.sources.USER);
		}
	},
	...
	"checklist enter": {
		...
		handler(range) {
			...
			const delta = new import_Delta.default().retain(range.index).insert("\n", formats).retain(line.length() - offset - 1).retain(1, { list: "unchecked" });
			this.quill.updateContents(delta, Quill.sources.USER);
			this.quill.setSelection(range.index + 1, Quill.sources.SILENT);
	...
	"table enter": {
		...
		handler(range) {
			...
			if (module) {
				...
				if (shift < 0) {
					const delta = new import_Delta.default().retain(index).insert("\n");
					this.quill.updateContents(delta, Quill.sources.USER);
	...
	"table tab": {
		...
		handler(range, context) {
			...
			if (event.shiftKey) this.quill.setSelection(offset - 1, Quill.sources.USER);
			else this.quill.setSelection(offset + cell.length(), Quill.sources.USER);
		}
	},
	"list autofill": {
		key: " ",
		...
		handler(range, context) {
			...
			this.quill.history.cutoff();
			const delta = new import_Delta.default().retain(range.index - offset).delete(length + 1).retain(line.length() - 2 - offset).retain(1, { list: value });
			this.quill.updateContents(delta, Quill.sources.USER);
			this.quill.history.cutoff();
	...
	"code exit": {
		...
		handler(range) {
			...
			while (cur != null && cur.length() <= 1 && cur.formats()["code-block"]) {
				...
				numLines -= 1;
				if (numLines <= 0) {
					const delta = new import_Delta.default().retain(range.index + line.length() - offset - 2).retain(1, { "code-block": null }).delete(1);
					this.quill.updateContents(delta, Quill.sources.USER);
					this.quill.setSelection(range.index - 1, Quill.sources.SILENT);
					return false;
				}
...
function normalize$2(binding) {
	...
	else if (typeof binding === "object") binding = cloneDeep(binding);
	else return null;
	if (binding.shortKey) {
...
function deleteRange(_ref2) {
	...
	if (lines.length > 1) {
		...
		formats = import_Delta.AttributeMap.diff(lastFormats, firstFormats) || {};
	}
...
var parseListItem = (element, html) => {
	...
	return {
		id,
		indent,
		type: typeMatch && typeMatch[1] === "bullet" ? "bullet" : "ordered",
		element
	};
};
var normalizeListItem = (doc) => {
	...
	msoList.forEach((node) => {
		if ((node.getAttribute("style") || "").match(ignoreRegexp)) ignored.push(node);
...
var NORMALIZERS = [normalize, normalize$1];
...
var debug$1 = namespace("quill:clipboard");
...
var Clipboard = class extends Module {
	...
	convert(_ref2) {
		let { html, text } = _ref2;
		let formats = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		if (formats[CodeBlock.blotName]) return new import_Delta.default().insert(text || "", { [CodeBlock.blotName]: formats[CodeBlock.blotName] });
		if (!html) return new import_Delta.default().insert(text || "", formats);
		const delta = this.convertHTML(html);
		if (deltaEndsWith(delta, "\n") && (delta.ops[delta.ops.length - 1].attributes == null || formats.table)) return delta.compose(new import_Delta.default().retain(delta.length() - 1).delete(1));
		return delta;
	}
	normalizeHTML(doc) {
	...
	dangerouslyPasteHTML(index, html) {
		...
		if (typeof index === "string") {
			...
			this.quill.updateContents(new import_Delta.default().retain(index).concat(paste), source);
	...
	onPaste(range, _ref3) {
		...
		const delta = new import_Delta.default().retain(range.index).delete(range.length).concat(pastedDelta);
		this.quill.updateContents(delta, Quill.sources.USER);
...
function traverse(scroll, node, elementMatchers, textMatchers, nodeMatches) {
	...
	return new import_Delta.default();
}
...
function matchBlot(node, delta, scroll) {
	...
	if (match.prototype instanceof EmbedBlot$1) {
		...
		if (value != null) {
			...
			return new import_Delta.default().insert(embed, match.formats(node, scroll));
		}
	} else {
...
function matchCodeBlock(node, delta, scroll) {
	...
	return applyFormat(delta, "code-block", match && "formats" in match && typeof match.formats === "function" ? match.formats(node, scroll) : true, scroll);
}
...
function matchStyles(node, delta, scroll) {
	...
	if (parseFloat(style.textIndent || 0) > 0) return new import_Delta.default().insert("	").concat(delta);
	return delta;
}
function matchTable(node, delta, scroll) {
	const table = node.parentElement?.tagName === "TABLE" ? node.parentElement : node.parentElement?.parentElement;
	if (table != null) return applyFormat(delta, "table", Array.from(table.querySelectorAll("tr")).indexOf(node) + 1, scroll);
	return delta;
}
function matchText(node, delta, scroll) {
	...
	if (!isPre(node)) {
		...
		if (node.previousSibling == null && node.parentElement != null && isLine(node.parentElement, scroll) || node.previousSibling instanceof Element && isLine(node.previousSibling, scroll)) text = text.replace(/^ /, "");
		if (node.nextSibling == null && node.parentElement != null && isLine(node.parentElement, scroll) || node.nextSibling instanceof Element && isLine(node.nextSibling, scroll)) text = text.replace(/ $/, "");
...
var History = class extends Module {
	...
	restoreSelection(stackItem) {
		if (stackItem.range) this.quill.setSelection(stackItem.range, Quill.sources.USER);
		else {
...
var Uploader = class extends Module {
	constructor(quill, options) {
		super(quill, options);
...
Uploader.DEFAULTS = {
	...
	handler(range, files) {
		...
		Promise.all(promises).then((images) => {
			...
			}, new import_Delta.default().retain(range.index).delete(range.length));
...
var Input = class extends Module {
	constructor(quill, options) {
		super(quill, options);
	...
	replaceText(range) {
		...
		if (text) {
			...
			this.quill.updateContents(new import_Delta.default().retain(range.index).insert(text, formats), Quill.sources.USER);
...
var UINode = class extends Module {
	isListening = false;
...
var ListContainer = class extends Container {};
...
var Italic = class extends Bold {
	static blotName = "italic";
	static tagName = ["EM", "I"];
};
//#endregion
//#region ../../node_modules/quill/formats/link.js
var Link = class extends Inline {
...
var Script = class extends Inline {
...
var TokenAttributor = new ClassAttributor$1("code-token", "hljs", { scope: Scope.INLINE });
var CodeToken = class CodeToken extends Inline {
...
var highlight = (lib, language, text) => {
	if (typeof lib.versionString === "string") {
		...
		if (parseInt(majorVersion, 10) >= 11) return lib.highlight(text, { language }).value;
	}
	return lib.highlight(language, text).value;
};
var Syntax = class extends Module {
	static register() {
	...
	highlightBlot(text) {
		...
		return traverse(this.quill.scroll, container, [(node, delta) => {
			...
			if (value) return delta.compose(new import_Delta.default().retain(delta.length(), { [CodeToken.blotName]: value }));
			return delta;
		}], [(node, delta) => {
...
var TableCell = class TableCell extends Block {
...
var TableBody = class extends Container {
	static blotName = "table-body";
	static tagName = "TBODY";
};
var TableContainer = class extends Container {
...
var Table = class extends Module {
	static register() {
	...
	getTable() {
		...
		if (cell == null || cell.statics.blotName !== TableCell.blotName) return [
			null,
			null,
			null,
			-1
	...
	insertRow(offset) {
		...
		this.quill.update(Quill.sources.USER);
		if (offset > 0) this.quill.setSelection(range, Quill.sources.SILENT);
		else this.quill.setSelection(range.index + row.children.length, range.length, Quill.sources.SILENT);
	}
	insertRowAbove() {
		this.insertRow(0);
	}
	...
	insertTable(rows, columns) {
		...
		}, new import_Delta.default().retain(range.index));
		this.quill.updateContents(delta, Quill.sources.USER);
...
var debug = namespace("quill:toolbar");
var Toolbar = class extends Module {
	constructor(quill, options) {
		super(quill, options);
	...
	attach(input) {
		...
		input.addEventListener(eventName, (e) => {
			...
			if (this.handlers[format] != null) this.handlers[format].call(this, value);
				...
				if (!value) return;
				this.quill.updateContents(new import_Delta.default().retain(range.index).delete(range.length).insert({ [format]: value }), Quill.sources.USER);
...
Toolbar.DEFAULTS = {
	...
	handlers: {
		...
		direction(value) {
			...
			if (value === "rtl" && align == null) this.quill.format("align", "right", Quill.sources.USER);
			else if (!value && align === "right") this.quill.format("align", false, Quill.sources.USER);
...
var ColorPicker = class extends Picker {
...
var IconPicker = class extends Picker {
	...
	buildPickers(selects, icons) {
		this.pickers = Array.from(selects).map((select) => {
			if (select.classList.contains("ql-align")) {
				...
				if (typeof icons.align === "object") return new IconPicker(select, icons.align);
			}
...
BaseTheme.DEFAULTS = merge({}, Theme.DEFAULTS, { modules: { toolbar: { handlers: {
	formula() {
		...
		switch (this.root.getAttribute("data-mode")) {
			...
			case "formula": {
				...
				if (range != null) {
					...
					if (this.root.getAttribute("data-mode") === "formula") this.quill.insertText(index + 1, " ", Emitter.sources.USER);
...
var BubbleTheme = class extends BaseTheme {
	constructor(quill, options) {
		if (options.modules.toolbar != null && options.modules.toolbar.container == null) options.modules.toolbar.container = TOOLBAR_CONFIG$1;
		super(quill, options);
...
BubbleTheme.DEFAULTS = merge({}, BaseTheme.DEFAULTS, { modules: { toolbar: { handlers: { link(value) {
...
SnowTheme.DEFAULTS = merge({}, BaseTheme.DEFAULTS, { modules: { toolbar: { handlers: { link(value) {
	if (value) {
		const range = this.quill.getSelection();
