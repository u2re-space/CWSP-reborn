...
function set_cptable(cptable) {
	...
	debom = function(data) {
		if (data.charCodeAt(0) === 255 && data.charCodeAt(1) === 254) return $cptable.utils.decode(1200, char_codes(data.slice(2)));
		return data;
	};
...
function datenum_local(v, date1904) {
	...
	if (date1904) epoch -= 1262304e5;
	else if (v >= SSFbase1904) epoch += 864e5;
	return (epoch - (SSFdnthresh + (/*#__PURE__*/ v.getTimezoneOffset() - /*#__PURE__*/ SSFbasedate.getTimezoneOffset()) * 6e4)) / 864e5;
}
		...
		for (; di < 10; ++di) {
			...
			if (m) switch (m[1].toLowerCase()) {
				...
				case "content-type":
					ctype = m[2].trim();
					break;
				case "content-transfer-encoding": cte = m[2].trim();
			}
		}
		++di;
...
function xlml_write_docprops(Props, opts) {
	...
	keys(XLMLDocPropsMap).map(function(m) {
		...
		switch (p[2]) {
			case "date": m = new Date(m).toISOString().replace(/\.\d*Z/, "Z");
		}
...
function guess_property_type(val) {
	switch (typeof val) {
		...
		case "object": if (val instanceof Date) return 64;
	}
	return -1;
}
	...
	function write_ws_cell_sylk(cell, ws, R, C) {
		...
		switch (cell.t) {
			...
			case "d":
				o += "\"" + (cell.w || cell.v) + "\"";
				break;
			case "s": o += "\"" + cell.v.replace(/"/g, "").replace(/;/g, ";;") + "\"";
		}
		return o;
	}
...
var PRN = /*#__PURE__*/ (function() {
	...
	function dsv_to_sheet_str(str, opts) {
		...
		outer: for (; end < str.length; ++end) switch (cc = str.charCodeAt(end)) {
			...
			case sepcc:
			case 10:
			case 13: if (!instr && finish_cell()) break outer;
		}
		...
		for (var i = 0; i != defaults.length; ++i) {
			...
			if (w[z[0]] == null) w[z[0]] = z[1];
				...
				case "bool":
					...
					break;
				case "int": if (typeof w[z[0]] == "string") w[z[0]] = parseInt(w[z[0]], 10);
			}
		}
	}
}
	...
	for (var i = 0; i != defaults.length; ++i) {
		...
		if (target[z[0]] == null) target[z[0]] = z[1];
			...
			case "bool":
				...
				break;
			case "int": if (typeof target[z[0]] == "string") target[z[0]] = parseInt(target[z[0]], 10);
		}
	}
}
...
function parse_xlml_data(xml, ss, data, cell, base, styles, csty, row, arrayf, o) {
	...
	switch (data.Type) {
		...
		case "DateTime":
			...
			cell.v = (parseDate(xml) - new Date(Date.UTC(1899, 11, 30))) / 864e5;
						...
						case "visible":
							if (Rn[0].slice(-2) === "/>") {} else if (Rn[1] === "/") switch (str.slice(pidx, Rn.index)) {
								case "SheetHidden":
									wsprops.Hidden = 1;
									break;
								case "SheetVeryHidden": wsprops.Hidden = 2;
							}
							else pidx = Rn.index + Rn[0].length;
							break;
						case "header":
	...
	while (Rn = xlmlregex.exec(str)) switch (Rn[3] = Rn[3].replace(/_.*$/, "")) {
		...
		case "text":
			if (Rn[0].slice(-2) === "/>") break;
				...
				case "number-style":
				case "date-style":
				case "time-style": NF += str.slice(pidx, Rn.index);
			}
			else pidx = Rn.index + Rn[0].length;
			break;
		case "named-range":
...
function write_varint49(v) {
	var usz = /* @__PURE__ */ new Uint8Array(7);
	usz[0] = v & 127;
	var L = 1;
	sz: if (v > 127) {
		usz[L - 1] |= 128;
...
function compress_iwa_file(buf) {
	...
	while (l < buf.length) {
		...
		var frame = /* @__PURE__ */ new Uint8Array(4);
		out.push(frame);
...
function write_new_storage(cell, sst) {
	var out = /* @__PURE__ */ new Uint8Array(32), dv = u8_to_dataview(out), l = 12, flags = 0;
	out[0] = 5;
	switch (cell.t) {
		case "n":
			out[1] = 2;
...
function readSync(data, opts) {
	...
	switch ((n = firstbyte(d, o))[0]) {
		...
		case 137: if (n[1] === 80 && n[2] === 78 && n[3] === 71) throw new Error("PNG Image File is not a spreadsheet");
	}
...
//#endregion
export { CFB, SSF, parse_xlscfb, parse_zip, readSync as read, readFileSync as readFile, readFileSync, set_cptable, set_fs, __stream as stream, utils, version, writeSync as write, writeFileSync as writeFile, writeFileSync, writeFileAsync, writeFileSyncXLSX as writeFileXLSX, writeSyncXLSX as writeXLSX };
