(function() {
	...
	*/
	...
	const UUIDv4 = () => crypto?.randomUUID ? crypto?.randomUUID?.() : "10000000-1000-4000-8000-100000000000".replace(/[018]/g, (c) => (+c ^ getRandomValues?.(/* @__PURE__ */ new Uint8Array(1))?.[0] & 15 >> +c / 4).toString(16));
	...
	*/
	function getWorkerResolveBaseUrl() {
		...
		try {
			if (typeof document !== "undefined" && typeof document.baseURI === "string" && document.baseURI.length > 0) return document.baseURI;
		} catch {}
		return "";
	}
	/** Resolved absolute href for `./x`/`/x`/absolute worker specifiers (delegates trailing `/` normalization to callers). */
	function resolveWorkerSpecifierHref(spec) {
		const base = getWorkerResolveBaseUrl();
		if (!base.length) throw new TypeError("[uniform] No base URL for worker resolution (missing location / document.baseURI)");
	...
	*/
	...
	const workerCode = workerBase.length > 0 ? new URL("../transport/Worker.ts", workerBase) : "";
	...
	*/
	...
	var OPFS_worker_exports = /* @__PURE__ */ __exportAll({
		...
		resolveFileSystemHandle: () => resolveFileSystemHandle
	});
	...
	var init_OPFS_worker = __esmMin((() => {
		...
		resolveFileSystemHandle = async (root, path, create = false) => {
			const parts = normalizePath(path).split("/").filter((p) => p && p !== ".");
			let current = root;
			for (let i = 0; i < parts.length; i++) {
				const part = parts[i];
				if (i === parts.length - 1) try {
				...
				else current = await current.getDirectoryHandle(part, { create });
			}
			return current;
		};
		getDirHandle = async (root, path, create) => {
		...
		handlers = {
			...
			readFile: async ({ rootId, path, type }) => {
				try {
					...
					const file = await (await (await getDirHandle(root, dirPath, false)).getFileHandle(filename, { create: false })).getFile();
			...
			writeFile: async ({ rootId, path, data }) => {
				try {
					...
					const filename = parts.pop();
					const dirPath = parts.join("/");
					const writable = await (await (await getDirHandle(root, dirPath, true)).getFileHandle(filename, { create: true })).createWritable();
			...
			remove: async ({ rootId, path, recursive }) => {
				try {
					...
					const dirPath = parts.join("/");
					await (await getDirHandle(root, dirPath, false)).removeEntry(name, { recursive });
					return true;
				} catch (e) {
					return false;
				}
		...
		SW_BRIDGE_CHANNEL_NAME = "opfs-sw-bridge-v1";
	...
	const initWorker = async () => {
		try {
			const handlers = (await Promise.resolve().then(() => (init_OPFS_worker(), OPFS_worker_exports))).handlers;
			if (handlers) registerWorkerAPI(handlers);
