...
`;
function readInitialUrl(options) {
	...
	try {
		...
		if (/^www\./i.test(raw) || /^[a-z0-9][a-z0-9.-]*\.[a-z]{2,}([/:?#]|$)/i.test(raw)) return new URL(`https://${raw.replace(/^\/+/, "")}`).href;
	} catch {
		return "";
	}
	return "";
...
function createBrowserView(options) {
	...
	hint.textContent = "Embedded page. If it stays blank, the site blocks iframes — use Open ↗.";
