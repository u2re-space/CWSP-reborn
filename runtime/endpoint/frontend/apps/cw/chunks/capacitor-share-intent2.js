...
import { t as isCapacitorNative } from "./capacitor-permissions2.js";
	...
	const handler = (ev) => {
		(async () => {
			...
			try {
				...
				if (row.mode === "process") {
					const { ensureCapacitorBridgeDaemonStarted } = await import("../shells/boot-history-base.js").then((n) => n.r);
