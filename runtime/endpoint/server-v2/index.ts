...
import { createWsServer } from "../server/network/socket/websocket.ts";
...
import { listenHttpsWithHttpUpgrade } from "./protocol/http/https-upgrade.ts";
...
export const createServerV2Runtime = async (options: ServerV2StartOptions = {}) => {
    ...
    const build = async () => {
        ...
        // WHY: Socket.IO (coordinator) + native /ws share the same HTTPS server.
        // createWsServer registers the upgrade handler for pathname === "/ws".
        const nativeWsHub = createWsServer(app);
        (app as any).wsHub = nativeWsHub;
        // WHY: phones dial native /ws; when target is this desk (or unknown→self),
        // apply clipboard:update locally — including image DataAsset (not text-only).
        (nativeWsHub as any).onUnknownTarget = (_userId: string, _deviceId: string, frame: any) => {
            try {
                const payload = frame?.payload || frame?.data || frame;
                const frameType = String(frame?.type || frame?.what || "").toLowerCase();
                if (!frameType.startsWith("clipboard") && !frameType.startsWith("airpad:clipboard")) {
                    return false;
                }
                const asset =
                    payload && typeof payload === "object"
                        ? (payload.asset || payload.dataAsset || payload.file || payload.image)
                        : null;
                if (asset && typeof asset === "object") {
                    void import("../server/io/clipboard.ts").then(({ writeClipboardImage }) =>
                        writeClipboardImage(asset as Record<string, unknown>)
                    );
                    return true;
                }
                const text =
                ...
                if (text && !text.startsWith("content://") && !text.startsWith("file://")) {
                    void import("../server/io/clipboard.ts").then(({ writeClipboard }) => writeClipboard(text));
                    return true;
                }
            } catch (error) {
                console.warn("[server-v2] local clipboard apply failed:", String((error as Error)?.message || error));
            }
            return false;
        };
        // Expose runtime sockets/clipboard so HTTP transport handlers can resolve either
        // `register(app, wsHub)` or `register(app, { sockets, clipboard })` call shapes.
        // The gateway branch also needs the authoritative server-v2 engine settings store.
        const httpRuntimeContext = {
            ...runtimeContext,
            engine
        };
        (app as any).serverV2RuntimeContext = httpRuntimeContext;
    ...
    return {
        ...
        start: async (startOptions: ServerV2StartOptions = {}) => {
            ...
            const primaryPort = isHttps ? httpsPort || httpPort || 8434 : httpPort || httpsPort || 8080;
            ...
            // WHY: HTTPS-only bind rejects plain http://host:8434 with an empty reply.
            // Same-port mux peeks TLS ClientHello (0x16) vs HTTP and 308-redirects the latter.
            if (isHttps) {
                await built.app.ready();
                const httpsServer = built.app.server as HttpsServer;
                for (const port of portsToTry) {
                    try {
                        const upgrade = await listenHttpsWithHttpUpgrade({
                            httpsServer,
                            host: listenHost,
                            port,
                            fallbackAuthority: `localhost:${port}`
                        });
                        boundPort = upgrade.port;
                        listenOk = true;
                        (built as { httpsUpgradeMux?: typeof upgrade.mux }).httpsUpgradeMux = upgrade.mux;
                        break;
                    } catch (error: unknown) {
                        lastErr = error;
                        const code =
            ...
            console.log(
                `[server-v2] listening on ${protocol}://${listenHost}:${boundPort} — Engine.IO/Socket.IO on same port (/socket.io/). ` +
                    (isHttps
                        ? `Plain HTTP on this port redirects 308 → https://. `
                        : "") +
