...
export const registerSettingsHttpHandlers = async (app: FastifyInstance): Promise<void> => {
    const gatewayAuth = (app as any).gatewayAuth;
    const gatewaySessionOptions = gatewayAuth?.requireSession
        ? { preHandler: gatewayAuth.requireSession }
        : {};
    const gatewayMutationOptions = gatewayAuth?.requireSession && gatewayAuth?.requireOrigin
        ? { preHandler: [gatewayAuth.requireSession, gatewayAuth.requireOrigin] }
    ...
    app.get("/core/admin/prefs", gatewaySessionOptions, async () => {
        try {
    ...
    app.post("/core/admin/prefs", gatewayMutationOptions, async (request: FastifyRequest<{ Body: { prefs?: unknown } }>) => {
        try {
    ...
    app.get("/api/config/:filename", gatewaySessionOptions, async (request: FastifyRequest<{ Params: { filename: string } }>, reply) => {
