...
// INVARIANT: always return a scheme-qualified absolute base for `new URL(...)`.
const resolveEndpointUrl = () => {
    const input = $("#endpointUrl");
    const explicit = input?.value?.trim?.() || "";
    const resolved = normalizeEndpointUrl(explicit, currentOrigin());
    // Surface bare-host upgrades (scheme/port) so Register hits :8434, not :443.
    if (input && explicit && explicit !== resolved) {
