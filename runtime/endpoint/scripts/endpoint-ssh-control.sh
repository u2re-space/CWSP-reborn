...
linux_status() {
    run_linux "bash -lc 'cd \"$LINUX_ROOT\"; echo === 200 \(linux\) ===; \
        ...
        ss -ltnp 2>/dev/null | grep -E \"(:8080|:8434)\" || true; \
...
windows_status() {
    run_windows "cmd /c \"cd /d \"$WINDOWS_ROOT\" && echo === 110 (windows) === && if exist \"ecosystem.config.cjs\" (echo [portable] ecosystem.config.cjs: yes) else (echo [portable] ecosystem.config.cjs: no) && if exist run.cmd (echo [portable] run.cmd: yes) else (echo [portable] run.cmd: no) && where pm2 >nul 2>nul && pm2 list || echo pm2: missing && echo -- LISTENERS && netstat -ano | findstr :8080 || true && netstat -ano | findstr :8434 || true\""
}
