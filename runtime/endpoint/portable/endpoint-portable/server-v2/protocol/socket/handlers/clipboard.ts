 ...
 * Reason for changes: Apply image DataAsset to OS clipboard on DST (not text-only).
 *
 * WHY: Image shares arrive as clipboard:update { asset }. Skipping write(undefined)
 * fixed text clobber, but DST still never got the image. Now: persist optional file,
 * write image via platform clipboard helper, and only write text when real text exists.
 */
