    ...
    wss.on("connection", async (ws, req) => {
        ...
        const userId = !isPlaceholderUserId(rawUserId)
            ...
            : !isPlaceholderUserId(rawClientId)
              ? rawClientId
              : rawUserId;
        // COMPAT: phones/web often send ecosystem token as token/accessToken, not userKey.
        const userKey =
            params.get("userKey") ||
            params.get("token") ||
        ...
        ws.on("message", async (data) => {
            ...
            // Pure relay never called writeClipboard / writeClipboardImage → text worked only
            // when another path applied it; images never did after asset-only packets.
