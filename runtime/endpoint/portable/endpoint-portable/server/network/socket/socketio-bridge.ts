...
import { areConnectionTypesCompatible, describeDisplayConnectionType, inferExpectedRemoteConnectionType, isFirstOrderFamily, parseWsConnectionType, supportsForwardServerConnectionType, supportsReverseServerConnectionType, toDisplayTopology } from "../stack/connection-types.ts";
...
export const createSocketIoBridge = (app: FastifyInstance, opts: SocketIoBridgeOptions = {}): SocketIoBridge => {
    ...
    const fanoutToAllExceptSource = (sourceSocket: Socket, payload: any, targetSource: "explicit" | "fallback"): void => {
        sourceSocket.broadcast.emit("message", payload);
        logMsg("OUT(broadcast)", payload, targetSource, sourceSocket);

        const userIdForReverse = (sourceSocket as any).userId || "";
        networkContext?.sendToReverse?.(userIdForReverse, "broadcast", payload);

        const bridgePayload = buildSocketIoBridgePayload(sourceSocket, payload, "broadcast", targetSource);
        if (bridgePayload) {
    ...
    const routeMessage = (sourceSocket: Socket, msg: any): void => {
        ...
        // Default behavior: two-directional fanout for implicit targets, with self-echo guard.
        if (!hasExplicitTarget) {
            fanoutToAllExceptSource(sourceSocket, processed, targetSource);
            return;
        }
        ...
        if (candidateTargets.length > 0) {
            ...
            for (const candidate of candidateTargets) {
                if (candidate.socket === sourceSocket) continue;
    ...
    const multicastMessage = (sourceSocket: Socket, msg: any, deviceIds?: string[]): void => {
        ...
        if (!deviceIds || deviceIds.length === 0) {
            fanoutToAllExceptSource(sourceSocket, processed, "explicit");
            return;
        }
    ...
    io.on("connection", (socket: Socket) => {
        ...
        const runtimeRoles = (config as any)?.roles;
        const supportsLocalForwardServer = supportsForwardServerConnectionType(runtimeRoles);
        const supportsLocalReverseServer = supportsReverseServerConnectionType(runtimeRoles);
        if (!supportsLocalForwardServer && !supportsLocalReverseServer) {
            console.warn(`[Server] AirPad socket rejected: neither forward nor reverse server role is enabled`, buildSocketConnectionTypeLogPayload(socket, remoteConnectionType));
        ...
        if (isFirstOrderFamily(activeConnectionType)) {
            // Socket.IO server side is fixed to responser-initiated mode.
            activeConnectionType = "requestor-initiator";
            console.log(`[Server] AirPad socket accepted first-order/exchanger connection, acting as ${activeConnectionType}`);
        }
