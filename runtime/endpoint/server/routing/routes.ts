...
const resolveSourceEndpointPolicy = (app: any, requestBody: any, request: any): { sourceId: string; targets: string[] } | null => {
    ...
    const rawPolicyMap = Object.keys(runtimeRaw).length > 0 ? runtimeRaw : rawClientsPolicyFallback;
...
export const buildClipboardBroadcastPayload = (app: any, requestBody: any, text: string, request: any) => {
    ...
    const clipboardPayload: Record<string, any> = {
        text
    };
    const resolvedSource = sourceId || resolvedClientId || String(requestBody?.source || requestBody?.from || requestBody?.userId || "").trim();
    if (resolvedSource) {
        clipboardPayload.from = resolvedSource;
        clipboardPayload.source = resolvedSource;
        clipboardPayload.userId = resolvedSource;
        clipboardPayload.clientId = resolvedSource;
    }
    if (resolvedToken) {
        clipboardPayload.token = resolvedToken;
        clipboardPayload.userKey = resolvedToken;
    }
