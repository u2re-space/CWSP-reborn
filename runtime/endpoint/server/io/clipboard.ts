 ...
 * FullPath: runtime/cwsp/endpoint/server/io/clipboard.ts
 * Change date and time: 22.50.00_11.07.2026
 * Reason for changes: Gateway L-200 must be clipboard relay-only — never write OS paste
 *   SoT or poll/fan-out local buffer (that rewrote Android text as L-200 bounce).
 */
...
/**
 * INVARIANT: central gateway (L-200) relays clipboard packets over /ws — it must not
 * own a paste SoT. Writing + polling here turned inbound Android text into an L-200
 * rebroadcast that overwrote phones.
 *
 * Opt-in override: CWS_CLIPBOARD_RELAY_ONLY=1
 * Opt-out (force local SoT on gateway): CWS_CLIPBOARD_RELAY_ONLY=0
 */
export const isClipboardRelayOnly = (): boolean => {
    const forced = process.env.CWS_CLIPBOARD_RELAY_ONLY;
    if (forced === "1" || forced === "true") return true;
    if (forced === "0" || forced === "false") return false;
    const id = resolveClipboardSourceId().toLowerCase().replace(/^l-/, "");
    if (!id) return false;
    // Short + long fleet forms for the LAN/WAN gateway coordinator.
    return (
        id === "200" ||
        id === "192.168.0.200" ||
        id === "l-200" ||
        id.endsWith(".200") && id.includes("192.168.0.")
    );
};
...
export function startClipboardPolling() {
    ...
    // WHY: gateway must not poll/fan-out its own OS clipboard as a peer rewrite.
    if (isClipboardRelayOnly()) {
        logClipboard("info", "[Clipboard] relay-only host — OS poll/fan-out disabled", {
            node: resolveClipboardNodeLabel()
        });
        return;
    }
...
export async function writeClipboard(text: string): Promise<boolean> {
    ...
    // WHY: relay gateway acknowledges without mutating OS paste SoT.
    if (isClipboardRelayOnly()) {
        lastNetworkClipboard = text;
        lastClipboard = text;
        logClipboard("info", "[ClipboardRx] relay-only — skipped OS write", {
            node: resolveClipboardNodeLabel(),
            text: summarizeClipboardText(text)
        });
        return true;
...
/**
 * Apply an image DataAsset to the OS clipboard (Windows SetImage / Linux xclip).
 * WHY: share-target and remote clipboard:update { asset } never reach clipboardy.
 */
export async function writeClipboardImage(asset: Record<string, unknown>): Promise<boolean> {
    if (!clipboardFeatureEnabled) return false;
    if (isClipboardRelayOnly()) {
        logClipboard("info", "[ClipboardRx] relay-only — skipped OS image write", {
            node: resolveClipboardNodeLabel(),
            hash: String((asset as any)?.hash || "")
        });
        return true;
    }
    try {
        const { writeClipboardImageAsset } = await import("../../server-v2/inputs/drivers/clipboard-image.ts");
        const ok = await writeClipboardImageAsset(asset);
        if (ok) {
            logClipboard("info", "[ClipboardRx] Image asset applied to OS clipboard", {
                node: resolveClipboardNodeLabel(),
                hash: String(asset.hash || ""),
                mime: String(asset.mimeType || asset.type || "")
            });
        }
        return ok;
    } catch (err) {
        logClipboard("warn", "[ClipboardRx] Image asset apply failed", {
            node: resolveClipboardNodeLabel(),
