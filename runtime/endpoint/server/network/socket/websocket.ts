...
export const createWsServer = (app: FastifyInstance): WsHub => {
    ...
    // WHY: phones dial mode=push (forward) with clientId=L-196/L-210. Reverse-only
    // resolution made them invisible to HTTP dispatch / clipboard routing via gateway.
    const resolveClientByTarget = (userId: string, target: string): ClientInfo | undefined => {
        const reverseHit = resolveReverseClientByTarget(userId, target);
        if (reverseHit) return reverseHit;
        const normalizedTargets = resolveTargetAlias(target);
        if (normalizedTargets.length === 0) return undefined;
        for (const entry of clients.values()) {
            if (entry.reverse) continue;
            const aliases = [
                ...resolveTargetAlias(entry.peerId || ""),
                ...resolveTargetAlias(entry.userId || ""),
                ...resolveTargetAlias(entry.deviceId || "")
            ];
            if (aliases.some((alias) => normalizedTargets.includes(alias))) return entry;
        }
        return undefined;
    };
    ...
    const verify = async (userId?: string, userKey?: string) => {
        ...
        try {
            ...
            // fall through to endpoint policy tokens (ecosystem token)
        }
        // WHY: fleet phones authenticate with shared ecosystem token in endpointIDs.tokens,
        // not the local users.json store. loadUserSettings throws "Invalid credentials" for them.
    ...
    wss.on("connection", async (ws, req) => {
        ...
        const rawUserId =
            ...
            params.get("from") ||
            undefined;
        const rawClientId = params.get("clientId") || params.get("peerId") || params.get("byId") || "";
        // WHY: web AirPad falls back to userId=airpad-client when settings.core.userId is empty,
        // while clientId query still carries the real L-196. Prefer a real node id for auth/routing.
        const isPlaceholderUserId = (value: string | undefined): boolean => {
            const v = String(value || "").trim().toLowerCase();
            return !v || v === "airpad-client" || v === "anonymous" || v === "guest" || v === "client";
        };
        const userId = !isPlaceholderUserId(rawUserId)
            ? rawUserId
            : !isPlaceholderUserId(rawClientId)
              ? rawClientId
              : rawUserId;
        // COMPAT: phones/web/Neutralino hub send ecosystem token under several aliases.
        // WHY: clipboard-hub historically sent only `clientToken` — accept it too.
        const userKey =
            params.get("userKey") ||
            params.get("token") ||
            params.get("accessToken") ||
            params.get("clientToken") ||
            params.get("ecosystemToken") ||
            params.get("clientAccessToken") ||
            undefined;
        ...
        if (!settings || !userId || !userKey) {
            console.warn(
                `[ws] Invalid credentials userId=${userId || "-"} hasKey=${Boolean(userKey)} keyAliases=${[
                    params.get("userKey") ? "userKey" : "",
                    params.get("token") ? "token" : "",
                    params.get("accessToken") ? "accessToken" : "",
                    params.get("ecosystemToken") ? "ecosystemToken" : ""
                ]
                    .filter(Boolean)
                    .join(",") || "none"}`
            );
        ...
        // WHY: Android/first-order peers dial mode=push with clientId=L-196 (not reverse).
        // Without peerId/deviceId they never appear in getConnectedDevices / sendToDevice.
        const forwardPeerId = !isReverse
            ? (requestedPeerId || normalizeSocketPeer(userId) || normalizedDeviceId || "")
            : "";
        const peerLabel = isReverse
            ? normalizePeerLabel(userId || "", normalizedDeviceId, requestedPeerId)
            : (requestedLabel || forwardPeerId || userId || undefined);
        const info: ClientInfo = {
            ...
            deviceId: isReverse ? normalizedDeviceId : (normalizedDeviceId || undefined),
            peerLabel,
            peerId: isReverse ? requestedPeerId : (forwardPeerId || undefined),
        ...
        console.log(
            formatInboundWsConnection(
                req,
                userId || "",
                (isReverse ? normalizedDeviceId : forwardPeerId) || "",
                String(activeConnectionType || ""),
                String(localConnectionType || ""),
                mode,
                (isReverse ? requestedPeerId : forwardPeerId) || "",
                peerLabel || ""
            )
        );
        console.log(
        ...
        // Register both reverse devices and first-order peers under a shared profile map
        // keyed by owner userIdKey (and also under peer id for cross-user gateway routing).
        const profileDeviceKey = isReverse
            ? (normalizedDeviceId || requestedPeerId || "")
            : (forwardPeerId || normalizedDeviceId || "");
        if (profileDeviceKey) {
            const labels = reversePeerProfiles.get(info.userIdKey) ?? new Map<string, { label: string; peerId: string }>();
            labels.set(profileDeviceKey, {
                label: peerLabel || profileDeviceKey,
                peerId: (isReverse ? requestedPeerId : forwardPeerId) || profileDeviceKey
            });
            reversePeerProfiles.set(info.userIdKey, labels);
            // Also index under peer identity so getConnectedPeerProfiles() without filter sees phones.
            if (!isReverse && forwardPeerId && forwardPeerId !== info.userIdKey) {
                const peerBucket = reversePeerProfiles.get(forwardPeerId) ?? new Map<string, { label: string; peerId: string }>();
                peerBucket.set(forwardPeerId, {
                    label: peerLabel || forwardPeerId,
                    peerId: forwardPeerId
                });
                reversePeerProfiles.set(forwardPeerId, peerBucket);
            }
        }
        ws.send(JSON.stringify({
            ...
            id: info.id,
            userId,
            deviceId: info.deviceId || deviceId || undefined,
        ...
        ws.on("message", async (data) => {
            ...
            // Use it to refresh forward-peer identity when query params were incomplete.
            if (frameType === "hello") {
                const helloClientId = normalizeSocketPeer(
                    String(parsed?.clientId || parsed?.peerId || parsed?.userId || "")
                );
                const helloDeviceId = normalizeSocketPeer(String(parsed?.deviceId || ""));
                if (helloClientId && !info.reverse) {
                    info.peerId = helloClientId;
                    info.peerLabel = info.peerLabel || helloClientId;
                    if (helloDeviceId) info.deviceId = helloDeviceId;
                    const labels = reversePeerProfiles.get(info.userIdKey) ?? new Map<string, { label: string; peerId: string }>();
                    labels.set(helloClientId, { label: info.peerLabel || helloClientId, peerId: helloClientId });
                    reversePeerProfiles.set(info.userIdKey, labels);
                }
                return;
            }
            ...
            // on desk DST hosts. Gateway L-200 is relay-only (isClipboardRelayOnly) — write
            // + poll there rebroadcast old/gateway text to Android as a rewrite.
            const applyLocalClipboardIfNeeded = (): boolean => {
                const frameType = String(type || frame?.what || "").toLowerCase();
                if (!frameType.startsWith("clipboard") && !frameType.startsWith("airpad:clipboard")) {
                    return false;
                }
                const body = (payload && typeof payload === "object" ? payload : {}) as Record<string, unknown>;
                const asset = body.asset || body.dataAsset || body.file || body.image;
                if (asset && typeof asset === "object") {
                    void import("../../../io/clipboard.ts")
                        .then(({ writeClipboardImage }) => writeClipboardImage(asset as Record<string, unknown>))
                        .catch(() => undefined);
                    return true;
                }
                const text =
                    ...
                    "";
                if (text && !text.startsWith("content://") && !text.startsWith("file://")) {
                    void import("../../../io/clipboard.ts")
                        .then(({ writeClipboard }) => writeClipboard(text))
                        .catch(() => undefined);
                    return true;
                }
                return false;
            ...
            const selfAliases = new Set(
                [
                    ...
                    String(process.env.CWS_BRIDGE_USER_ID || "").trim(),
                    String(process.env.CWS_BRIDGE_DEVICE_ID || "").trim(),
                    String((config as any)?.bridge?.userId || "").trim(),
                    String((config as any)?.bridge?.deviceId || "").trim(),
                    String((config as any)?.bridge?.clientId || "").trim()
                ]
                    .filter(Boolean)
        ...
        ws.on("close", () => {
            ...
            if (info.reverse && info.deviceId) {
                ...
                const labels = reversePeerProfiles.get(info.userIdKey);
                if (labels) {
                    labels.delete(info.peerId);
                    if (labels.size === 0) reversePeerProfiles.delete(info.userIdKey);
                }
                const peerBucket = reversePeerProfiles.get(info.peerId);
                if (peerBucket) {
                    peerBucket.delete(info.peerId);
                    if (peerBucket.size === 0) reversePeerProfiles.delete(info.peerId);
                }
            }
        });
    });
    ...
    const sendToDevice = (userId: string, deviceId: string, payload: any): boolean => {
        ...
        const target = resolveClientByTarget(normalizedUser, normalizedTarget);
    ...
    const requestToDevice = async (userId: string, deviceId: string, payload: any, waitMs = 15000) => {
        ...
        const pendingTarget = normalizeSocketPeer(target.deviceId || target.peerId || normalizedTarget);
    ...
    // WHY: ignore owner userId filter for live peer listing — each phone authenticates as its
    // own clientId (L-196, L-210, …) under separate namespaces; gateway routing needs all of them.
    const getConnectedDevices = (_userId?: string): string[] => {
        const devices = new Set<string>();
        for (const entry of clients.values()) {
            if (entry.reverse) {
                if (entry.deviceId) devices.add(normalizeSocketPeer(entry.deviceId));
                if (entry.peerId) devices.add(normalizeSocketPeer(entry.peerId));
            } else {
                const peer = normalizeSocketPeer(entry.peerId || entry.userId || "");
                if (peer) devices.add(peer);
            }
        }
    ...
    const getConnectedPeerProfiles = (userId?: string): Array<{ id: string; label: string; peerId?: string }> => {
        if (!userId) {
            const byId = new Map<string, { id: string; label: string; peerId?: string }>();
            for (const entry of clients.values()) {
                const id = normalizeSocketPeer(entry.peerId || (entry.reverse ? entry.deviceId : entry.userId) || "");
                if (!id) continue;
                byId.set(id, { id, label: entry.peerLabel || id, peerId: entry.peerId || id });
            }
            return Array.from(byId.values());
        }
        // Prefer live clients matching this owner OR any first-order peer (gateway view).
        const byId = new Map<string, { id: string; label: string; peerId?: string }>();
        for (const entry of clients.values()) {
            const id = normalizeSocketPeer(entry.peerId || (entry.reverse ? entry.deviceId : entry.userId) || "");
            if (!id) continue;
