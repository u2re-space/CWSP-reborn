...
const handleLocalAirpadPayload = (app: FastifyInstance, frame: any): boolean => {
    ...
    const frameType = String(frame?.type || frame?.what || payload?.type || payload?.what || "").toLowerCase();
    if (!payload) return false;

    const action = String(payload.action || payload.type || payload.what || "").toLowerCase();
    const isClipboardFrame =
        frameType === "clipboard" ||
        frameType.startsWith("clipboard:") ||
        frameType.startsWith("airpad:clipboard:") ||
        action === "clipboard" ||
        action.startsWith("clipboard:") ||
        action.startsWith("airpad:clipboard:");

    // WHY: Android CwspWsClient sends canonical clipboard:update; old matcher only
    // accepted plain "clipboard" and ignored payload.asset → images never hit DST OS clip.
    if (isClipboardFrame) {
        const asset =
            (payload && typeof payload === "object" && !Array.isArray(payload)
                ? (payload.asset || payload.dataAsset || payload.file || payload.image)
                : null) ||
            (frame && typeof frame === "object" ? (frame as any).asset : null);
        if (asset && typeof asset === "object") {
            app.log?.info?.("Clipboard image write via tunnel");
            import("../../io/clipboard.ts").then(({ writeClipboardImage }) => {
                writeClipboardImage(asset as Record<string, unknown>).catch((err) => {
                    app.log?.error?.({ err }, "Failed to write clipboard image");
                });
            });
            return true;
        }
        ...
        // Asset/text missing — still acknowledge so routing does not warn as unhandled.
...
const buildBridgeRouter = (app: FastifyInstance, hub: WsHub, fallbackUserId: string) => {
    ...
    return (message: any) => {
        ...
        const clipboardType = String(msg.type || msg.action || msg.what || "").toLowerCase();
        const isClipboardMsg =
            clipboardType === "clipboard" ||
            clipboardType.startsWith("clipboard:") ||
            clipboardType.startsWith("airpad:clipboard:");
        // WHY: reverse/bridge peers receive wildcard fan-out without a concrete target;
        // apply clipboard (incl. image asset) on this host before re-multicast.
        if (isClipboardMsg) {
            const clipPayload = (payload && typeof payload === "object" ? payload : msg) as Record<string, unknown>;
            const asset = clipPayload.asset || clipPayload.dataAsset || clipPayload.file || clipPayload.image;
            if (asset && typeof asset === "object") {
                void import("../../io/clipboard.ts").then(({ writeClipboardImage }) =>
                    writeClipboardImage(asset as Record<string, unknown>)
                );
            } else {
                const text =
                    (typeof clipPayload.text === "string" && clipPayload.text) ||
                    (typeof clipPayload.content === "string" && clipPayload.content) ||
                    (typeof clipPayload.body === "string" && clipPayload.body) ||
                    "";
                if (text && !text.startsWith("content://") && !text.startsWith("file://")) {
                    void import("../../io/clipboard.ts").then(({ writeClipboard }) => writeClipboard(text));
                }
            }
        }
        if (isTunnelDebug) {
