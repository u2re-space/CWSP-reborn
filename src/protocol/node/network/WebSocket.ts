 ...
 * FullPath: apps/CWSP-reborn/src/protocol/node/network/WebSocket.ts
 * Change date and time: 17.06.00_10.07.2026
 * Reason for changes: Provide thin canonical WebSocket helpers for the NodeJS endpoint.
 *
 * NOTE: Implemented against `globalThis.WebSocket` (Node 22+ ships a native client).
 * On older Node runtimes callers must provide a WebSocket polyfill (undici/ws) before
 * use. The canonical /ws path is the realtime transport (see network.mdc).
 */
...
export function openCwspWebSocket(url: string, handlers?: CwspWsHandlers): WebSocket {
    ...
    if (!WS) {
        throw new Error(
            "openCwspWebSocket: globalThis.WebSocket is undefined. On Node < 22 provide a WebSocket polyfill (undici/ws).",
        );
    }
    const ws = new WS(url);
